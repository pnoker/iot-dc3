...
public final class S7SerializerImpl implements S7Serializer {
    ...
    public static Object extractBytes(PlcS7PointVariable plcs7PointVariable, final byte[] buffer,
    ...
    public static <T> T extractBytes(final Class<T> beanClass, final byte[] buffer, final int byteOffset) {
        log.trace("Extracting type {} from buffer with size: {} at offset {}", beanClass.getName(), buffer.length,
                byteOffset);
    ...
    public Object dispense(PlcS7PointVariable plcs7PointVariable) throws S7Exception {
        try {
            final byte[] buffer = this.connector.read(DaveArea.DB, plcs7PointVariable.getDbNum(),
                    plcs7PointVariable.getSize(), plcs7PointVariable.getByteOffset());
            return extractBytes(plcs7PointVariable, buffer, 0);
        } catch (final Exception e) {
            throw new S7Exception(
                    "dispense dbnum(" + plcs7PointVariable.getDbNum() + ") byteoffset("
                            + plcs7PointVariable.getByteOffset() + ") blocksize(" + plcs7PointVariable.getSize() + ")",
                    e);
        }
    }
