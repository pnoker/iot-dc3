...
public final class PDU {
    ...
    /**
     * Add data after parameters, set dlen as needed. Needs valid header and parameters
     */
    ...
    /**
     * Add len bytes of len after parameters from a maybe longer block of bytes. Set dlen
    ...
    /**
     * Add values after value header in data, adjust dlen and data count. Needs valid
     * header,parameters,data,dlen
     */
    ...
    public int addVarToReadRequest(final DaveArea area, final int DBnum, int start, final int len) {
        ...
        Nodave.setUSBEWord(this.mem, this.header + 6, this.plen);
        /**
         * TODO calc length of result. Do not add variable if it would exceed max. result
         * length.
         */
        return 0;
    }
     ...
     */
    /*
     * void constructWriteRequest( int area, int DBnum, int start, int len, byte[] buffer)
     * { byte pa[] = new byte[14]; byte da[] = { 0, 4, 0, 0 }; pa[0] = PDU.FUNC_WRITE;
     * pa[1] = (byte) 0x01; pa[2] = (byte) 0x12; pa[3] = (byte) 0x0a; pa[4] = (byte) 0x10;
     * pa[5] = (byte) 0x02;
     *
     * Nodave.setUSBEWord(pa, 6, len); Nodave.setUSBEWord(pa, 8, DBnum);
     * Nodave.setUSBELong(pa, 10, 8 * start); // the bit address Nodave.setUSByte(pa, 10,
     * area); initHeader(1); addParam(pa); addData(da); addValue(buffer); if
     * ((Nodave.Debug & Nodave.DEBUG_PDU) != 0) { dump(); } }
     */
    ...
    /*
     * typedef struct { uc P; // allways 0x32 uc type; // a type? type 2 and 3 headers are
     * two bytes longer. uc a,b; // currently unknown us number; // Number, can be used to
     * identify answers corresponding to requests us plen; // length of parameters which
     * follow this header us dlen; // length of data which follows the parameters uc x[2];
     * // only present in type 2 and 3 headers. This may contain error information. }
     * PDUHeader;
     */
    ...
    /**
     * Set up a PDU instance to reflect the structure of data present in the memory area
     * given to initHeader. Needs valid header.
     *
     * @return Result
     */
