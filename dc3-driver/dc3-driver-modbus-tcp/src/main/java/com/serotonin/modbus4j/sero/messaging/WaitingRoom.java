...
/**
 * The waiting room is a place for request messages to hang out while awaiting their
 * responses.
 *
...
class WaitingRoom {
	...
	/**
	 * The request message should be sent AFTER entering the waiting room so that the
	 * (vanishingly small) chance of a response being returned before the thread is
	 * waiting for it is eliminated.
	 * @return
	 */
	void enter(WaitingRoomKey key) {
		...
		synchronized (this) {
			...
			// if (dup != null) {
			//
			// throw new WaitingRoomException("Waiting room too crowded. Already contains
			// the key " + key);
			// }
	...
	void leave(WaitingRoomKey key) {
		...
		synchronized (this) {
			...
			// Notify any threads that are waiting to get in. This could probably be just
			// a notify() call.
			notifyAll();
		}
	}
	...
	/**
	 * This method is used by the data listening thread to post responses as they are
	...
	/**
	 * This class is used by network message controllers to manage the blocking of threads
	 * sending confirmed messages. The instance itself serves as a monitor upon which the
	 * sending thread can wait (with a timeout). When a response is received, the message
	 * controller can set it in here, automatically notifying the sending thread that the
