...
public class WriteMaskRegisterRequest extends ModbusRequest {
	...
	/**
	 * The andMask determines which bits we want to change. If a bit in the andMask is 1,
	 * it indicates that the value should not be changed. If it is zero, it should be
	 * changed according to the orMask value for that bit.
	 */
	private int andMask;

	/**
	 * The orMask determines what value a bit will have after writing if the andMask
	 * allows that bit to be changed. If a changable bit in the orMask is 0, the bit in
	 * the result will be zero. Ditto for 1.
	 */
	private int orMask;

	/**
	 * Constructor that defaults the masks to have no effect on the register. Use the
	 * setBit function to modify mask values.
	 * @param slaveId a int.
