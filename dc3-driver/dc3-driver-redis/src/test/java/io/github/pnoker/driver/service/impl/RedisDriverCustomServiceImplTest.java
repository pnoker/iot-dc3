...
class RedisDriverCustomServiceImplTest {
    ...
    private RedisDriverCustomServiceImpl service;

    @BeforeEach
    void setUp() {
        service = new RedisDriverCustomServiceImpl(driverMetadata, driverSenderService, stringRedisTemplate);
    }

    @Test
    void validatePassesForEmptyDriverConfig() {
        ValidationReport report = service.validate(new HashMap<>());
        assertThat(report.isPassed()).isTrue();
    }

    @Test
    void validatePointFlagsMissingKey() {
