...
values are received asynchronously through a `@KafkaListener` and cached by message key; a point read returns the latest
cached value, and a point write produces a message to the configured topic.
...
The broker connection is configured through Spring Boot `spring.kafka.*` properties in `application.yml`, driven by the
