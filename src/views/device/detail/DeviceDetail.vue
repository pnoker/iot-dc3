...
<template>
	<div>
			...
			<el-tabs v-model="reactiveData.active" @tab-click="changeActive">
					...
					<detail-card>
						<ul>
							...
							<li>
								...
								包含模板 [{{ reactiveData.listProfileData.length }} 个]: {{ profileName }}
							</li>
							<li>
								<el-icon>
									<CollectionTag />
				...
				<el-tab-pane label="包含模板" name="profile">
					<el-row>
						...
						<el-col v-for="data in reactiveData.listProfileData" :key="data.id" :xs="24" :sm="12" :md="8" :lg="6" :xl="4">
							<profile-card :data="data" :point-table="reactiveData.pointTable" :embedded="true"></profile-card>
						</el-col>
					</el-row>
				</el-tab-pane>
				...
				<el-tab-pane label="设备数据" name="pointValue">
					<el-row>
						...
						<el-col v-for="data in reactiveData.listPointValueData" :key="data.id" :xs="24" :sm="12" :md="8" :lg="6" :xl="4">
							<point-value-card
								:data="data"
		...
		<point-value-detail ref="pointValueDetailRef" :detail-data="reactiveData.pointValueDetailData"> </point-value-detail>
	</div>
</template>
