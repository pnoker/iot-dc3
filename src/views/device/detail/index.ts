...
export default defineComponent({
    ...
    setup() {
        ...
        const profileName = computed(() => {
            return reactiveData.listProfileData.map(profile => profile.name).join(", ") || "-"
        })
        ...
        const profiles = () => {
            profileByDeviceIdApi(reactiveData.id).then(res => {
        ...
        const points = () => {
            pointByDeviceIdApi(reactiveData.id).then(res => {
        ...
        const pointValues = () => {
            pointValueByDeviceIdApi(reactiveData.id, true).then(res => {
                reactiveData.listPointValueData = res.data.data

                // unit
                const pointIds = Array.from(new Set(reactiveData.listPointValueData.map(pointValue => pointValue.pointId)))
                if (pointIds.length > 0) {
                ...
                reactiveData.listPointValueData.forEach(pointValue => {
                    if (pointValue.type === "string") {
                        ...
                        reactiveData.listPointValueHistoryData[pointValue.pointId] = pointValue.children.reverse().map(pointValue => pointValue.value === "true" ? 1 : 0)
                    } else {
                        reactiveData.listPointValueHistoryData[pointValue.pointId] = pointValue.children.reverse().map(pointValue => +pointValue.value)
                    }
                })
