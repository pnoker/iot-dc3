...
import { driverInfoAddApi, driverInfoByDeviceIdApi, driverInfoUpdateApi, pointInfoAddApi, pointInfoByDeviceIdApi, pointInfoUpdateApi } from "@/api/info";
...
export default defineComponent({
    ...
    setup() {
        ...
        const point = () => {
            pointByDeviceIdApi(reactiveData.id).then(res1 => {
                reactiveData.pointTable = res1.data.data.reduce((pre, cur) => {
                    pre[cur.id] = cur.name;
                    return pre
                }, {})

                pointAttributeByDriverIdApi(reactiveData.driverId).then(res2 => {
        ...
        const changeDriver = (driverId) => {
            driverAttributeByDriverIdApi(driverId).then(res => {
                ...
                driverInfoByDeviceIdApi(reactiveData.id).then(res => {
                    const formData = {};
        ...
        const changePoint = () => {
            pointAttributeByDriverIdApi(reactiveData.driverId).then(res => {
                ...
                pointInfoByDeviceIdApi(reactiveData.id).then(res => {
                    res.data.data.forEach(info => {
        ...
        const deviceUpdate = () => {
            ...
            form?.validate((valid) => {
                if (valid) {
                    deviceUpdateApi(reactiveData.deviceFormData).then(res => {
                        changeDriver(reactiveData.driverId);
                        changePoint();
                        reactiveData.oldDeviceFormData = { ...res.data.data };
                    }).catch(() => {
