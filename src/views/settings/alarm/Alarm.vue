      ...
      <template #filters="{ formData: fd }">
        ...
        <el-form-item :label="$t('settings.event.confirmFlag')" prop="confirmFlag">
          <el-segmented
            ...
            :options="[
              { label: $t('common.all'), value: '' },
              { label: $t('settings.alarm.unconfirmed'), value: 0 },
              { label: $t('settings.alarm.confirmed'), value: 1 },
            ]"
          />
        </el-form-item>
      </template>
      ...
      >
        ...
        <el-table-column :label="t('common.operation')" width="140" fixed="right">
          <template #default="{ row }">
            ...
            >
              <template #reference>
                <el-button link type="primary">{{ t('settings.alarm.confirm') }}</el-button>
              </template>
            </el-popconfirm>
            <el-popconfirm
              v-else
...
<script lang="ts" setup>
  ...
  const rows = ref<Row[]>([]);
  // Alarm page always pins eventTypeFlag=1 so it's not part of formData.
  const formData = reactive<{ source: string; confirmFlag: number | '' }>({
    source: '',
    confirmFlag: '',
  });
