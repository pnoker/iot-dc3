  ...
  <div>
    ...
    <blank-card>
      <el-table v-loading="reactiveData.loading" :data="reactiveData.listData" class="settings-table" stripe>
        <el-table-column :label="$t('common.name')" min-width="160" prop="eventName" />
        <el-table-column label="Code" min-width="150" prop="eventCode" show-overflow-tooltip />
...
<script lang="ts" setup>
  ...
  const openDetail = (row: EventRecord) => {
    router.push({ name: 'settingsEventDefinitionDetail', query: { id: String(row.id) } }).catch(() => {});
  };
