...
<template>
  <div>
    <base-card>
      <template #header>
        <span>Command Definition Detail</span>
      </template>
      <detail-card v-loading="loading">
        <el-descriptions v-if="record" :column="2" border>
          <el-descriptions-item :label="$t('common.name')">{{ record.commandName }}</el-descriptions-item>
          <el-descriptions-item label="Code">{{ record.commandCode }}</el-descriptions-item>
          <el-descriptions-item label="Command Type">{{ record.commandTypeFlag }}</el-descriptions-item>
          <el-descriptions-item label="Call Type">{{ record.callTypeFlag }}</el-descriptions-item>
          <el-descriptions-item label="Timeout (ms)">{{ record.timeout }}</el-descriptions-item>
...
<script lang="ts" setup>
  ...
  import { getCommandById } from '@/api/command';
  ...
  const record = ref<CommandRecord | null>(null);
