    ...
    <div>
        ...
        <blank-card>
            <el-row>
                ...
                <el-col>
                    <el-empty v-if="listData.length<1" description="暂无数据，请选择设备和位号！"></el-empty>
                </el-col>
    ...
    methods: {
        list() {
            pointValueList({
                ...
                this.listData = res.data.data.records.map(record => {
                ...
                // point unit
                let pointIds = Array.from(new Set(res.data.data.records.map(pointValue => pointValue.pointId)));
                if (pointIds.length > 0) {
                    pointUnit(pointIds).then(res => {
                        this.unitTable = res.data.data;
                    });
                }
            }).catch(() => {
            }).finally(() => {
        ...
        device() {
            deviceDictionary().then(res => {
                this.deviceDictionary = res.data.data;
        ...
        point() {
            pointDictionary("profile").then(res => {
                this.pointDictionary = res.data.data;
