...
Point 读写不在 `dc3_command` 中重复建模，由 `Point.rw_flag` 和现有 `PointValueCommand` 链路表达。新增 `Command` 只用于描述读写
...
设备状态和驱动状态当前不依赖 Redis，而是由 `LocalCacheService` 基于 Caffeine 的本地缓存维护 TTL 状态；心跳刷新缓存，状态翻转或
TTL 过期时再派生告警事件记录。因此，在线/离线状态不建议建成 `Profile` 下的通用 `dc3_event` 定义，避免把平台生命周期和设备业务能力混在一起。
