...
- **influxdb**：v3 HTTP API 直连（行协议写入 + query_sql CSV 读取，零客户端 依赖）；整型字段必须带 `i` 后缀（否则列绑 Float64
  永久丢 ns 精度）；行删除 不存在，租户注销走分区工具。
- **iotdb**：路径节点不能纯数字（`t1/d10/p20` 前缀）；`WHERE time` 的裸数字 按 **毫秒**解释（与库精度无关，µs 字面量会静默匹配空集）——一律用
  `2026-08-20T12:59:59+00:00` 形态；session 必须关重定向 （`enableRedirection(false)`，节点发现会发容器内地址）。
