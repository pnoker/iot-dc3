...
sequenceDiagram
    ...
    participant Tools as 🔧 10x @Tool Bean
    participant Manager as 📋 dc3-center-manager
    participant Data as 💾 dc3-center-data
    participant DB as 🗄️ PostgreSQL + MongoDB

    User->>GW: POST /api/v3/agentic/v1/chat/completions<br/>"读取锅炉温度和风机转速"
    GW->>Auth: 验证 X-Auth-Tenant + X-Auth-Token
    Auth-->>GW: Principal + tenant_id + 权限
    GW->>Agentic: 转发聊天请求

    Agentic->>Agentic: ChatClient.call(prompt, tools)
    Note over Agentic: 模型决策：调用 getLatestPointValue() × 2
    ...
    Data->>DB: MongoDB 查询 (point_id=42, tenant_id=acme-corp)
    DB-->>Data: {value: 86.4, unit: "°C", ts: "2026-06-28T10:00:00Z"}
    Data-->>Tools: PointValueBO
    Tools-->>Agentic: 工具结果

    Agentic->>Agentic: 模型整合回答
    Agentic-->>GW: ChatCompletion (自然语言回答)
    GW-->>User: "3号锅炉温度：86.4°C（2分钟前采集）。<br/>风机转速：60%。"

    User->>GW: "将风机调到100%"
    GW->>Auth: 验证
    GW->>Agentic: 转发

    Agentic->>Agentic: 模型意图调用 writePointValue()
    Note over Agentic: ⚠️ 写入工具 → 生成待确认 Action
    Agentic->>Agentic: ActionService.createWritePointValueAction()
    Agentic-->>User: ⚠️ 待确认：Action #abc123<br/>"将3号锅炉风机设为100%"

    User->>GW: POST /action/confirm (action_id=abc123)
    GW->>Agentic: 确认 Action
    Agentic->>Data: PointCommandFacade.submitWrite()
    Data-->>Agentic: 指令已接收
    Agentic-->>User: ✅ 风机已设为 100%。指令已下发。
```
