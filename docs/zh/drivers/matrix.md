...
物联网与无线类。`mqtt` 走发布/订阅，值由订阅被动到达（无主动读），命令可下发；`lwm2m` 内嵌服务端、收设备注册与通知，读写订阅俱全；
`coap`、`http`、`ble`、`can` 为请求-响应式主动读写；`zigbee` 当前为骨架实现，仅监听协调器网络状态、尚未监听节点入网与属性上报，订阅能力未实现。
...
| [Zigbee](./zigbee) (`ZigbeeDriver`) | IoT/无线 | ✓ | ✓ | —     | 骨架实现，订阅（入网/上报）未实现 |
...
的主动读），也可能是骨架未补齐（如 `ethernet-ip`、`iec104`、`dlms`）。最终行为请以对应模块的 `read()` / `write()` / `initial()`
源码为准；驱动迭代后此表会同步更新。
:::
