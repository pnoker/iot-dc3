...
*写命令**（`write()` / `execute()` 发布报文、QoS 回退、模板渲染）均已实现；`read()` 按 pub/sub 语义恒返回 `null`
（数据靠订阅被动到达，非缺陷）。仍为参考桩的是：`health()` 恒返回在线（broker 连接健康检查标 TODO 未做）、`initial()`
为空的初始化模板。把它作为接入与配置参考时，请注意在线状态目前不反映真实 broker 连接。
:::
...
2. 给设备绑定的[物模型](../introduction/concepts/profile)加一个可写[位号](../introduction/concepts/point)，point 属性填
