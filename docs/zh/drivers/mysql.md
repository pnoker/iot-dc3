...
> **`dc3-driver-mysql` 把一个 MySQL 数据库当作数据源接入 IoT DC3**——周期性执行 `SELECT` 查询把查到的值当采集值，并支持用位号上配的 `UPDATE`/`INSERT` 写查询向库里写值。

不是所有数据都来自现场协议设备：很多业务数据、历史数据、第三方系统沉淀的结果，本身就躺在一张 MySQL 表里。本驱动作为数据库客户端（[驱动](../introduction/concepts/driver) 类型 `DRIVER_CLIENT`），用 JDBC（`mysql-connector-j`）连到一个 MySQL 库，按[位号](../introduction/concepts/point)上配置的 SQL 去查值、写值。JDBC 连接的建立与查询执行由共享的 `dc3-common-sql` 抽象基类负责。适用于：把已有业务库里的字段当位号采集、对接只提供数据库视图的上游系统、把外部系统结果定时拉进平台。
...
- **驱动名 / code**：`MySQL Driver` / `MysqlDriver`
- **类型**：`DRIVER_CLIENT`（驱动主动连库并发起查询）

## 驱动配置（设备级 `driver-attribute`）

把一个 MySQL 库接进来时，在[设备](../introduction/concepts/device)上填这些[属性](../introduction/concepts/attribute-config)。它们决定连到哪个库、用什么账号、查询超时多久：

| 属性 | code | 类型 | 默认值 | 说明 |
|---|---|---|---|---|
| Host | `host` | STRING | `localhost` | MySQL host |
| Port | `port` | INT | `3306` | MySQL port |
| Database | `database` | STRING | （空）| MySQL database name |
| Username | `username` | STRING | `root` | MySQL username |
| Password | `password` | STRING | （空）| MySQL password |
| Query Timeout | `queryTimeout` | INT | `30` | SQL query timeout in seconds |

驱动用 `host`、`port`、`database` 拼出 JDBC URL（形如 `jdbc:mysql://host:port/database?...`），`host`、`port`、`database`、`username`、`password` 五项均为必填——缺任一项，设备配置校验都不会通过。
...
写值走的是位号上的 `writeQuery`：`write()` 取 `point-attribute` 的 `writeQuery`，用预编译参数绑定执行 `UPDATE`/`INSERT`。`command-attribute` 上的 `executeQuery` 仅作为配置项保留，当前驱动代码中没有任何地方读取或执行它——不存在"按命令直接执行一段 SQL"的独立路径，写值一律走 `writeQuery`。
:::
...
- [HTTP 驱动](./http) — 另一种把非协议数据源（REST 接口）接入的方式
