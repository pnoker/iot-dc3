...
- **协议驱动 Drivers（`dc3-driver-*`）**——驱动目录共 28 个协议适配实现，`docker-compose.yml` 默认内置其中 8 个驱动容器（
  `listening-virtual`/`modbus-tcp`/`modbus-rtu`/`mqtt`/`opc-da`/`opc-ua`/`plcs7`/`virtual`）；南向接设备、北向经 RabbitMQ
  与数据中心解耦。
