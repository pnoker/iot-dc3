...
- **查询层**：MyBatis-Plus 的租户行处理器对所有 `TenantOwned` 实体自动追加 `WHERE tenant_id = ?`，覆盖四个中心的全库查询。
- **接口层**：`BaseController.requireTenant()` 对跨租户的单条 ID 查询返回 404（而非泄露数据），`filterTenant()` 在批量结果里剔除非本租户记录。
