...
RBAC 决定"能不能做这类操作"，租户隔离决定"能不能碰这条数据"。两者正交，缺一不可——有 `device:get` 权限不代表你能 get 别家租户的设备。隔离落在两层：
...
**持久层 MyBatis-Plus 租户行处理器**：对实现了 `TenantOwned` 的实体，自动在 SQL 上追加 `WHERE tenant_id = ?`。这一层库级生效、跨四个中心（鉴权/管理/数据/智能）统一兜底，不依赖每个查询手写租户条件——开发者忘了写，处理器也会补上。
