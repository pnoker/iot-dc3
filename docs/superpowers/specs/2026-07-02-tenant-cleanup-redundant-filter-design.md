...
- **B**（join 原生 SQL 手 `tenant_id`，如 `and dgb.tenant_id = dd.tenant_id`）→ **逐处审查**（拦截器用 JSqlParser 解析整 SQL
  含 join，对每张表注入过滤谓词；但跨表关联 `dgb.tenant_id = dd.tenant_id` 是表间一致不是过滤，需审查是否冗余）
