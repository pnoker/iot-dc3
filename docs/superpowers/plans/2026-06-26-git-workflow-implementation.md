...
**Goal:** 把 iot-dc3 与 iot-dc3-web 统一改造为简化 Git Flow（main 生产+部署+tag、develop 集成、feature 单线、release 归档），tag 改 semver，CI 去重，补齐 PR/Issue 模板与分支保护。

**Architecture:** 每个仓库从 develop 切 `feature/git-workflow` 分支，在该分支上改 workflow/tag 脚本/模板/CONTRIBUTING，PR 回 develop；两仓库 develop 就绪后各自 promote 到 main，最后做不可逆的 GitHub 设置（默认分支 release→main、分支保护、release 归档）。
...
- [ ] **Step 3: 前端 main + develop 保护**（同 Step 1/2，--repo pnoker/iot-dc3-web，status check context 用前端实际名，先 `gh run list` 确认）
- [ ] **Step 4: 验证保护生效**
```bash
