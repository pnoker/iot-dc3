...
- **AI coding tools** — let Claude Code, Codex, Gemini CLI drive the platform through the shell; every command supports `--format json`, so the output is safe for programs to parse.
...
> Want a model rather than a script at the wheel? See the [AI section](../ai/) (conversational Agentic Center + MCP for external agents).
