...
> You are here: you hit a term while reading [Core Concepts](./concepts) or the operations docs, and came back to look it up. Every platform term links to its detail page.
...
- [Core Concepts](./concepts) — a one-sentence mental model plus an entity-relationship diagram that ties the platform terms into an object model
