...
When onboarding a MySQL database, fill in these [attributes](../introduction/concepts/attribute-config) on the [Device](../introduction/concepts/device). They decide which database to connect to, which account to use, and the query timeout:
...
::: tip Query timeout applies in `queryTimeout` seconds
...
- [HTTP Driver](./http) — another way to onboard a non-protocol data source (a REST endpoint)
