...
- **Device marked offline**: the device health check defaults to cron `0/15 * * * * ?` with a `45`-second lease timeout. Each successful push refreshes the lease; if nothing is pushed before the timeout, the device is marked offline. A device shown offline while it is actually sending usually means one of the causes above dropped its frames before the lease could be refreshed. See [device](../introduction/concepts/device) for the online-state mechanism.
...
- **Read / write / subscribe capability** (aligned with the [driver capability matrix](./matrix)):
  - **Read**: `—`, no active polling. `schedule.read.enable=false`, and `read()` returns `null` directly; data is entirely push-triggered by the device. The driver does have an internal `0/5 * * * * ?` job (`schedule.custom`) for its own periodic upkeep; it issues no reads to devices.
  - **Write**: `✓`, `write()` is implemented. When a device's TCP frame arrives the driver registers that device's `Channel` into `DEVICE_CHANNEL_MAP`; on a write command it looks up the active channel by `deviceId` and writes the value bytes back to the device (5-second flush timeout). If the channel is missing or inactive the write fails and returns `false`. Note: UDP is connectionless, so write-back goes only over the TCP channel.
