...
const DICT = {
  zh: {aria:'Oracle 驱动读写', drv:'dc3-driver-oracle', drvSub:'JDBC · SID / ServiceName',
    db:'Oracle 库', dbSub:'业务表 / 历史表', pv:'位号值 PointValue',
    e1:'readQuery (SELECT)', e2:'writeQuery (UPDATE/INSERT, ?)', e3:'结果第一行第一列'},
  en: {aria:'Oracle driver read/write', drv:'dc3-driver-oracle', drvSub:'JDBC · SID / ServiceName',
...
<template>
  <DiagramFrame><div class="dc3-diagram">
    <svg :aria-label="s.aria" role="img" viewBox="0 0 1040 300">
      <defs><marker id="or-ah" markerHeight="7" markerWidth="10" orient="auto" refX="9" refY="3.5"><polygon fill="var(--dc3-arrow)" points="0 0,10 3.5,0 7"/></marker></defs>
      <line marker-end="url(#or-ah)" stroke="var(--dc3-arrow)" stroke-width="1.5" x1="260" x2="400" y1="130" y2="130"/><text fill="var(--dc3-arrow-label)" font-size="10" text-anchor="middle" x="330" y="122">{{ s.e1 }}</text>
      <line marker-end="url(#or-ah)" stroke="var(--dc3-arrow)" stroke-width="1.5" stroke-dasharray="5,4" x1="260" x2="400" y1="170" y2="180"/><text fill="var(--dc3-arrow-label)" font-size="10" text-anchor="middle" x="330" y="192">{{ s.e2 }}</text>
      <line marker-end="url(#or-ah)" stroke="var(--dc3-arrow)" stroke-width="1.5" x1="600" x2="760" y1="150" y2="150"/><text fill="var(--dc3-arrow-label)" font-size="10" text-anchor="middle" x="680" y="142">{{ s.e3 }}</text>
