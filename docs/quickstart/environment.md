...
本地源码运行 Java 服务时，应使用 `dc3/env/dev.env.sh` 或 `dc3/env/dev.env`。根目录 `.env` 主要给 Compose 插值，不会自动注入到所有本地
Java 进程。
...
如果不加载，本地 Java 进程可能继续使用容器内服务名或默认端口，例如 `dc3-postgres`、`dc3-rabbitmq`、`dc3-center-manager`
，从而无法连接本机依赖。
...
| `AUTH_HMAC_SECRET` | Runtime | Gateway 与后端服务之间用于签名 `X-Auth-Principal` 的共享 HMAC-SHA256 密钥；生产环境应设置强随机值 |
