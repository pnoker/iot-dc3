...
public class DriverGrpcFacade implements DriverFacade {
    ...
    @Override public Flux<FacadeDriverBO> listByIdsReactive(Long tenantId, Collection<Long> ids) {
        if (tenantId == null || ids == null || ids.isEmpty()) return Flux.empty(); List<Long> values = ids.stream().filter(Objects::nonNull).distinct().toList(); if (values.isEmpty()) return Flux.empty();
        GrpcDriverIdsQuery request = GrpcDriverIdsQuery.newBuilder().addAllDriverIds(values).setTenantId(tenantId).build();
        DriverApiGrpc.DriverApiStub stub = grpcFacadeSupport.withDeadline(driverApiStub);
        return ReactiveGrpcClientSupport.<GrpcDriverIdsQuery, GrpcDriverListDTO>unary(
                ...
                .flatMapMany(response -> Flux.fromIterable(response.getItemsList()).map(facadeGrpcDriverBuilder::toFacadeBO));
    }

    @Override public Mono<OffsetPage<FacadeDriverBO>> listReactive(FacadeDriverOffsetQuery query) {
        ...
        return ReactiveGrpcClientSupport.<GrpcOffsetDriverQuery, GrpcOffsetPageDriverDTO>unary(
                ...
                .map(response -> {
                    ...
                    return OffsetPage.of(response.getItemsList().stream().map(facadeGrpcDriverBuilder::toFacadeBO).toList(),
