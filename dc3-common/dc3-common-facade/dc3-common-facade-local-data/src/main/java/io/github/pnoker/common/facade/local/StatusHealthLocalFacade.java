...
public class StatusHealthLocalFacade implements StatusHealthFacade {
    ...
    public Mono<Map<Long, String>> listDeviceStatusesByIdsReactive(Long tenantId, Collection<Long> deviceIds) {
        return deviceFacade
                ...
                .collectList()
                .flatMap(ids -> stateFlags(tenantId, EntityTypeEnum.DEVICE, ids).map(flags -> statusCodes(ids, flags)));
    }
    ...
    public Mono<Map<Long, String>> listDriverStatusesByIdsReactive(Long tenantId, Collection<Long> driverIds) {
        return driverFacade
                ...
                .collectList()
                .flatMap(ids -> stateFlags(tenantId, EntityTypeEnum.DRIVER, ids).map(flags -> statusCodes(ids, flags)));
    }
    ...
    public Mono<FacadeDriverDeviceStatusSummaryBO> getDriverDeviceStatusSummaryReactive(Long tenantId, Long driverId) {
        return driverFacade
                ...
                .flatMap(driver -> deviceFacade
                        .listByDriverIdReactive(tenantId, driverId)
                        .map(FacadeDeviceBO::getId)
                        .collectList()
                        .flatMap(ids -> stateFlags(tenantId, EntityTypeEnum.DEVICE, ids)
                                .map(flags -> {
                                    int online = (int) ids.stream()
                                            .filter(id -> EntityStatusEnum.ONLINE
                                                    .getIndex()
                                                    .equals(flags.get(id)))
                                            .count();
                                    return new FacadeDriverDeviceStatusSummaryBO(
                                            driverId, ids.size(), online, ids.size() - online);
                                })));
    }
    ...
    private Map<Long, String> statusCodes(Collection<Long> ids, Map<Long, Byte> flags) {
        ...
        ids.forEach(id -> {
            ...
            result.put(id, status == null ? EntityStatusEnum.OFFLINE.getCode() : status.getCode());
        });
    ...
    private FacadeSystemHealthBO toFacadeHealth(SystemHealthVO source) {
        ...
        target.setDrivers(summary(source.getDrivers()));
        target.setDevices(summary(source.getDevices()));
        return target;
    }

    private FacadeSystemHealthBO.FleetSummary summary(SystemHealthVO.FleetSummary source) {
        return source == null
                ? new FacadeSystemHealthBO.FleetSummary()
                : new FacadeSystemHealthBO.FleetSummary(source.getTotal(), source.getOnline());
    }
}
