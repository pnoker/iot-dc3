...
/**
 * Batch consumption tuning for the highest-throughput path (point values). Binds the
 * same {@code dc3.data.point.batch.*} keys the pre-port configuration used, so existing
 * deployments keep their tuning; the prefix is kept for compatibility, not because the
 * settings are data-center specific.
 *
 * @author pnoker
 * @since 2026.8.19
 */
...
public class BatchConsumerProperties {
