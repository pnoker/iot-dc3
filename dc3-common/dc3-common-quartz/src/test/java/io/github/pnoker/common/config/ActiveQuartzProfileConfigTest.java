...
class ActiveQuartzProfileConfigTest {
    ...
    private final ActiveQuartzProfileConfig config = new ActiveQuartzProfileConfig();

    @Test
    void postProcessEnvironmentAddsQuartzProfileByDefault() {
        StandardEnvironment environment = new StandardEnvironment();

        config.postProcessEnvironment(environment, new SpringApplication());

        assertThat(environment.getActiveProfiles()).contains(EnvironmentConstant.QUARTZ_PROFILE);
    }
