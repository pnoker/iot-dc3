 ...
 * {@link FacadeScannedApiBO}. One handler method with multiple HTTP methods or
...
public class ApiEndpointScanner {
    ...
    private static final Set<RequestMethod> SUPPORTED_METHODS = Set.of(
            RequestMethod.GET, RequestMethod.POST, RequestMethod.PUT, RequestMethod.DELETE);

    private static final List<String> DEFAULT_EXCLUDES = List.of(
            "/actuator/**", "/error", "/error/**", "/favicon.ico");
    ...
    private void expand(RequestMappingInfo info, HandlerMethod handler, Map<String, FacadeScannedApiBO> out) {
        ...
        for (PathPattern pattern : patterns) {
            ...
            for (RequestMethod method : methods) {
                ...
                out.put(key, FacadeScannedApiBO.builder()
                        .method(method.name())
                        .path(path)
