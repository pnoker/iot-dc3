 ...
 * {@code dc3_device_event} / {@code dc3_driver_event}. Without this a driver
 * or device that stops sending heartbeats would silently drop off the
 * dashboard — the badge would show the right online/offline split thanks to
 * the cache miss, but nothing would land in the alert log explaining why.
 *
 * <p>Hooks into {@link LocalCacheService}'s expiry listener so we don't need
 * a polling thread. Callbacks run off the Caffeine removal path onto the
 * common ForkJoinPool so a slow gRPC facade lookup can't stall the cache.</p>
 *
 * @author pnoker
 * @since 2026.5.2
 */
...
public class OfflineExpiryListener {
    ...
    private void handleDriverExpiry(String key, String lastStatus) {
        if (Objects.equals(lastStatus, DriverStatusEnum.OFFLINE.getCode())) return;
    ...
    private void handleDeviceExpiry(String key, String lastStatus) {
        if (Objects.equals(lastStatus, DeviceStatusEnum.OFFLINE.getCode())) return;
