...
public class DriverStatusServiceImpl implements DriverStatusService {
    ...
    public Mono<Map<String, String>> list(FacadeDriverOffsetQuery query) {
        return driverFacade.listReactive(query)
                .flatMap(page -> statuses(query.tenantId(), page.items().stream()
                        .map(driver -> driver.getId()).filter(id -> id != null).toList(), EntityTypeEnum.DRIVER));
    }

    @Override
    public Mono<Long> countOnlineDevices(Long tenantId, Long driverId) {
        return driverFacade.getByIdReactive(tenantId, driverId)
                .switchIfEmpty(Mono.error(new NotFoundException("Driver does not exist")))
                .flatMap(ignored -> deviceFacade.listByDriverIdReactive(tenantId, driverId).collectList())
                .flatMap(devices -> {
                    List<Long> ids = devices.stream().map(FacadeDeviceBO::getId).filter(id -> id != null).toList();
                    if (ids.isEmpty()) return Mono.just(0L);
                    return stateStore.listStateFlags(tenantId, EntityTypeEnum.DEVICE, ids)
                            .map(flags -> ids.stream().filter(id -> online(flags.get(id))).count());
                });
    }
    ...
    private Mono<Map<String, String>> statuses(Long tenantId, List<Long> ids, EntityTypeEnum type) {
        ...
        return stateStore.listStateFlags(tenantId, type, ids)
                .map(flags -> ids.stream().collect(Collectors.toUnmodifiableMap(String::valueOf,
                        id -> statusCode(flags.get(id)))));
    }
