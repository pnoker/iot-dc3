...
public class R2dbcPointCommandContext implements ReactivePointCommandContext {
    ...
    public Mono<FacadeDeviceBO> device(Long tenantId, Long deviceId) {
        ...
        return databaseClient.sql("SELECT id, device_name, device_code, driver_id, profile_id, enable_flag, tenant_id, "
                        + "signature, version FROM dc3_manager.dc3_device WHERE tenant_id = :tenant_id AND id = :id "
                        + "AND deleted = 0 LIMIT 1")
                .bind("tenant_id", tenantId).bind("id", deviceId).map((row, metadata) -> {
                    FacadeDeviceBO value = new FacadeDeviceBO();
                    value.setId(row.get("id", Long.class)); value.setDeviceName(row.get("device_name", String.class));
                    value.setDeviceCode(row.get("device_code", String.class)); value.setDriverId(row.get("driver_id", Long.class));
                    value.setProfileId(row.get("profile_id", Long.class)); value.setTenantId(row.get("tenant_id", Long.class));
                    value.setSignature(row.get("signature", String.class)); value.setVersion(row.get("version", Integer.class));
    ...
    public Mono<FacadePointBO> point(Long tenantId, Long pointId) {
        ...
        return databaseClient.sql("SELECT id, point_name, point_code, point_type_flag, rw_flag, profile_id, enable_flag, "
                        + "tenant_id, signature, version FROM dc3_manager.dc3_point WHERE tenant_id = :tenant_id AND id = :id "
                        + "AND deleted = 0 LIMIT 1")
                .bind("tenant_id", tenantId).bind("id", pointId).map((row, metadata) -> {
                    FacadePointBO value = new FacadePointBO();
                    value.setId(row.get("id", Long.class)); value.setPointName(row.get("point_name", String.class));
                    value.setPointCode(row.get("point_code", String.class)); value.setProfileId(row.get("profile_id", Long.class));
                    value.setTenantId(row.get("tenant_id", Long.class)); value.setSignature(row.get("signature", String.class));
    ...
    public Mono<FacadeDriverBO> driverByDevice(Long tenantId, Long deviceId) {
        if (tenantId == null || deviceId == null) return Mono.empty();
        return databaseClient.sql("SELECT d.id, d.driver_name, d.driver_code, d.service_name, d.service_host, "
                ...
                .bind("tenant_id", tenantId).bind("device_id", deviceId).map((row, metadata) -> {
    ...
    public Mono<FacadeDeviceOwnerBO> activeOwner(Long tenantId, Long deviceId) {
        if (tenantId == null || deviceId == null) return Mono.empty();
        return databaseClient.sql("SELECT l.driver_id, l.owner_node, l.fencing_token FROM dc3_manager.dc3_device_lease l "
                ...
                .map((row, metadata) -> new FacadeDeviceOwnerBO(row.get("driver_id", Long.class),
                        row.get("owner_node", String.class), row.get("fencing_token", Long.class))).one();
    }

    private Byte index(Number value) { return value == null ? null : value.byteValue(); }
}
