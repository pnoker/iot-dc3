...
public class EventHistoryServer extends EventHistoryApiGrpc.EventHistoryApiImplBase {
    ...
    public void reportEvent(GrpcEventReportVO request, StreamObserver<GrpcEventAccepted> responseObserver) {
        ...
        ReactiveGrpcServerSupport.subscribe(
                eventHistoryService
                        .report(request.getTenantId(), report)
                        .map(id ->
                                GrpcEventAccepted.newBuilder().setRecordId(id).build()),
                responseObserver);
    }
        ...
        ReactiveGrpcServerSupport.subscribe(
                eventHistoryService
                        ...
                        .switchIfEmpty(Mono.error(new io.github.pnoker.common.exception.NotFoundException(
                                "event history record does not exist")))
                        .map(this::toGrpcDTO),
                responseObserver);
    }
    ...
    public void list(GrpcEventHistoryQuery request, StreamObserver<GrpcEventHistoryOffsetPage> responseObserver) {
        ...
        if (request.hasEventTypeFlag())
            query.setEventTypeFlag(EventTypeFlagEnum.ofIndex((byte) request.getEventTypeFlag()));
        query.setOffset(page.offset());
        query.setLimit(page.limit());
        query.setSort(page.sort());
        ReactiveGrpcServerSupport.subscribe(
                eventHistoryService.list(request.getTenantId(), query).map(this::toGrpcPage), responseObserver);
    }
    ...
    private GrpcEventHistoryDTO toGrpcDTO(EventHistoryVO record) {
        return GrpcEventHistoryDTO.newBuilder()
                .setId(parse(record.getId()))
                .setRecordId(text(record.getRecordId()))
                .setTenantId(parse(record.getTenantId()))
                .setDeviceId(parse(record.getDeviceId()))
                .setEventId(parse(record.getEventId()))
                .setEventCode(text(record.getEventCode()))
                .setEventTypeFlag(
                        record.getEventTypeFlag() == null
                                ? 0
                                : record.getEventTypeFlag().getIndex())
                .setEventLevelFlag(
                        record.getEventLevelFlag() == null
                                ? 0
                                : record.getEventLevelFlag().getIndex())
                .putAllParamValues(toStringMap(record.getParamValues()))
                .setConfigSnapshot(text(record.getConfigSnapshot()))
                .setMessage(text(record.getMessage()))
                .setOccurTime(toEpochSecond(record.getOccurTime()))
                .setReceiveTime(toEpochSecond(record.getReceiveTime()))
                .setAcknowledgeFlag(
                        record.getAcknowledgeFlag() == null
                                ? 0
                                : record.getAcknowledgeFlag().getIndex())
                ...
                .setAcknowledgeUserId(parse(record.getAcknowledgeUserId()))
                .build();
    }
    ...
    private Map<String, String> toStringMap(String json) {
        ...
        return Objects.requireNonNullElse(
                JsonUtil.parseObject(json, new TypeReference<Map<String, String>>() {}), Map.of());
    }
}
