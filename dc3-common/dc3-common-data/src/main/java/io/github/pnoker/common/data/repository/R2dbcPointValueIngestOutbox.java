...
public class R2dbcPointValueIngestOutbox implements ReactivePointValueIngestOutbox {
    ...
    public Mono<List<PointValueDO>> enqueue(List<PointValueDO> values, String owner) {
        ...
        return transactionalOperator.transactional(Flux.fromIterable(rows).concatMap(value -> insert(value, owner)
                .filter(Boolean::booleanValue)
                .map(ignored -> value)).collectList());
    }

    @Override
    public Flux<PointValueDO> findPersisted(List<PointValueDO> values) {
        if (values == null || values.isEmpty()) return Flux.empty();
        return Flux.fromIterable(values).concatMap(value -> databaseClient.sql("SELECT " + COLUMNS + " FROM " + table()
                        + " WHERE tenant_id=:tenant_id AND message_id=:message_id AND status='PERSISTED'")
                .bind("tenant_id", value.getTenantId()).bind("message_id", value.getMessageId()).map(this::map).one());
    }
    ...
    public Mono<Integer> markPersisted(PointValueDO value, String owner) {
        ...
        return databaseClient.sql("UPDATE " + table() + " SET status='PERSISTED',claimed_at=CURRENT_TIMESTAMP,last_error=NULL "
                        + "WHERE tenant_id=:tenant_id AND message_id=:message_id AND status='CLAIMED' AND claimed_by=:owner")
                .bind("tenant_id", value.getTenantId()).bind("message_id", value.getMessageId()).bind("owner", owner)
                .fetch().rowsUpdated().map(Long::intValue);
    }

    private Mono<Boolean> insert(PointValueDO value, String owner) {
        require(value);
        String sql = "INSERT INTO " + table() + " (" + COLUMNS + ",status,attempts,available_at,claimed_at,claimed_by) VALUES "
                ...
                + (postgres() ? "ON CONFLICT (tenant_id,message_id) DO NOTHING" : "ON DUPLICATE KEY UPDATE message_id=message_id");
        DatabaseClient.GenericExecuteSpec spec = databaseClient.sql(sql)
                .bind("tenant_id", value.getTenantId()).bind("message_id", value.getMessageId())
                ...
                .bind("device_id", value.getDeviceId()).bind("point_id", value.getPointId())
        ...
        spec = value.getNumValue() == null ? spec.bindNull("num_value", Double.class) : spec.bind("num_value", value.getNumValue());
        spec = spec.bind("claimed_by", owner);
        return spec.fetch().rowsUpdated().then(databaseClient.sql("SELECT status,claimed_by FROM " + table()
                        + " WHERE tenant_id=:tenant_id AND message_id=:message_id")
                .bind("tenant_id", value.getTenantId()).bind("message_id", value.getMessageId())
                .map((row, metadata) -> "CLAIMED".equals(row.get("status", String.class))
                        && owner.equals(row.get("claimed_by", String.class))).one().defaultIfEmpty(false));
    }
    ...
    public Flux<PointValueDO> claim(String owner, int limit) {
        ...
        String persistedLeaseExpired = postgres()
                ? "CURRENT_TIMESTAMP - INTERVAL '30 seconds'" : "CURRENT_TIMESTAMP - INTERVAL 30 SECOND";
        ...
        Mono<List<PointValueDO>> claimed = databaseClient.sql(expired).fetch().rowsUpdated().then(
                databaseClient.sql(select).bind("limit", bounded).map(this::map).all()
                        .concatMap(value -> databaseClient.sql("UPDATE " + table() + " SET status='CLAIMED',attempts=attempts+1,claimed_at=CURRENT_TIMESTAMP,claimed_by=:owner "
                                        + "WHERE tenant_id=:tenant_id AND message_id=:message_id AND status IN ('PENDING','PERSISTED')")
                                .bind("owner", owner).bind("tenant_id", value.getTenantId()).bind("message_id", value.getMessageId())
                                .fetch().rowsUpdated().filter(rows -> rows == 1).thenReturn(value))
                        .collectList());
        return transactionalOperator.transactional(claimed)
    ...
    public Mono<Integer> markProcessed(PointValueDO value) {
        if (value == null || value.getTenantId() == null || value.getMessageId() == null) return Mono.just(0);
                return databaseClient.sql("UPDATE " + table() + " SET status='PROCESSED',processed_at=CURRENT_TIMESTAMP,claimed_at=NULL,claimed_by=NULL,last_error=NULL "
                        + "WHERE tenant_id=:tenant_id AND message_id=:message_id AND status='PERSISTED'")
                .bind("tenant_id", value.getTenantId()).bind("message_id", value.getMessageId()).fetch().rowsUpdated().map(Long::intValue);
    }
    ...
    public Mono<Integer> markFailed(PointValueDO value, String owner, String error) {
        ...
        return databaseClient.sql(sql).bind("tenant_id", value.getTenantId()).bind("message_id", value.getMessageId())
                .bind("owner", owner).bind("last_error", Objects.requireNonNullElse(error, "unknown error"))
                .fetch().rowsUpdated().map(Long::intValue);
    }

    private PointValueDO map(io.r2dbc.spi.Row row, io.r2dbc.spi.RowMetadata metadata) {
        PointValueDO value = new PointValueDO();
        value.setTenantId(number(row, "tenant_id", Long.class)); value.setMessageId(row.get("message_id", String.class));
        ...
        value.setDeviceId(number(row, "device_id", Long.class)); value.setPointId(number(row, "point_id", Long.class));
    ...
    private boolean postgres() { return "postgres".equalsIgnoreCase(dialect.name()); }

    private void require(PointValueDO value) {
        if (value.getTenantId() == null || value.getTenantId() <= 0 || value.getMessageId() == null || value.getMessageId().isBlank()) {
    ...
    private <T> T number(io.r2dbc.spi.Row row, String name, Class<T> type) {
        Object raw = row.get(name); if (raw == null) return null; if (type.isInstance(raw)) return (T) raw;
