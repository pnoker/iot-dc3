...
public class R2dbcRuleStateStore implements ReactiveRuleStateStore {
    ...
    public Mono<RuleStateDO> get(long tenantId, long stateId) {
        ...
        return databaseClient.sql("SELECT " + COLUMNS + " FROM " + TABLE + " WHERE tenant_id=:tenant_id AND id=:id LIMIT 1")
                .bind("tenant_id", tenantId).bind("id", stateId).map(this::map).one();
    }

    @Override
    public Mono<OffsetPage<RuleStateDO>> list(long tenantId, Long ruleId, Byte alarmTargetTypeFlag, Long entityId,
        ...
        if (ruleId != null) { where.append(" AND rule_id=:rule_id"); binds.add("rule_id"); }
        if (alarmTargetTypeFlag != null) { where.append(" AND alarm_target_type_flag=:target_type"); binds.add("target_type"); }
        if (entityId != null) { where.append(" AND entity_id=:entity_id"); binds.add("entity_id"); }
        if (fingerprint != null && !fingerprint.isBlank()) { where.append(" AND fingerprint LIKE :fingerprint"); binds.add("fingerprint"); }
        if (entityStateFlag != null) { where.append(" AND entity_state_flag=:state_flag"); binds.add("state_flag"); }
        if (alarmId != null) { where.append(" AND alarm_id=:alarm_id"); binds.add("alarm_id"); }
        ...
        for (String bind : binds) {
            ...
            countSpec = countSpec.bind(bind, value); rowsSpec = rowsSpec.bind(bind, value);
        }
    ...
    public Mono<Boolean> delete(long tenantId, long stateId) {
        ...
        return databaseClient.sql("DELETE FROM " + TABLE + " WHERE tenant_id=:tenant_id AND id=:id")
                .bind("tenant_id", tenantId).bind("id", stateId).fetch().rowsUpdated().map(rows -> rows == 1);
    }

    @Override
    public Mono<RuleStateDO> find(long tenantId, long ruleId, byte alarmTargetTypeFlag, long entityId,
                                  String fingerprint) {
        ...
        return databaseClient.sql("SELECT " + COLUMNS + " FROM " + TABLE
                ...
                .bind("entity_id", entityId).bind("fingerprint", fingerprint)
                .map(this::map).one();
    }

    @Override
    public Mono<RuleStateDO> transition(RuleStateDO state, boolean recovery) {
        if (state == null || !valid(state.getTenantId()) || !valid(state.getRuleId())
                || !valid(state.getEntityId()) || state.getFingerprint() == null) {
        ...
        DatabaseClient.GenericExecuteSpec updateSpec = databaseClient.sql(update)
                .bind("state_flag", value(state.getEntityStateFlag()))
                .bind("alarm_id", state.getAlarmId())
                .bind("state_ext", state.getEntityStateExt() == null ? "{}" : JsonUtil.toJsonString(state.getEntityStateExt()))
                .bind("trigger_delta", recovery ? 0L : 1L)
                .bind("tenant_id", state.getTenantId()).bind("rule_id", state.getRuleId())
                .bind("target_type", value(state.getAlarmTargetTypeFlag())).bind("entity_id", state.getEntityId())
        ...
        return transactionalOperator.transactional(updateSpec.fetch().rowsUpdated())
                .flatMap(rows -> rows == 1 ? find(state.getTenantId(), state.getRuleId(), value(state.getAlarmTargetTypeFlag()),
                                state.getEntityId(), state.getFingerprint())
                        : insert(state, recovery))
                .onErrorResume(DataIntegrityViolationException.class,
                        error -> find(state.getTenantId(), state.getRuleId(), value(state.getAlarmTargetTypeFlag()),
                                        state.getEntityId(), state.getFingerprint())
    ...
    private Mono<RuleStateDO> insert(RuleStateDO state, boolean recovery) {
        ...
        String sql = "INSERT INTO " + TABLE + " (" + COLUMNS + ") VALUES "
                ...
                + ":last_recover_time,:last_notify_time,:trigger_count,:alarm_id," + dialect.jsonWriteExpression(":state_ext")
                + ",:tenant_id,:remark,:creator_id,:creator_name,:create_time,:operator_id,:operator_name,:operate_time)";
        DatabaseClient.GenericExecuteSpec spec = databaseClient.sql(sql)
                .bind("id", state.getId()).bind("rule_id", state.getRuleId())
                .bind("target_type", value(state.getAlarmTargetTypeFlag())).bind("entity_id", state.getEntityId())
                .bind("fingerprint", state.getFingerprint()).bind("state_flag", value(state.getEntityStateFlag()))
                .bind("trigger_count", state.getTriggerCount()).bind("alarm_id", state.getAlarmId())
                .bind("tenant_id", state.getTenantId()).bind("remark", text(state.getRemark()))
                .bind("creator_id", longValue(state.getCreatorId())).bind("creator_name", text(state.getCreatorName()))
                .bind("create_time", state.getCreateTime()).bind("operator_id", longValue(state.getOperatorId()))
                .bind("operator_name", text(state.getOperatorName())).bind("operate_time", state.getOperateTime());
        ...
        spec = bindNullable(spec, "state_ext", state.getEntityStateExt() == null ? "{}" : JsonUtil.toJsonString(state.getEntityStateExt()), String.class);
        return transactionalOperator.transactional(spec.fetch().rowsUpdated()).flatMap(rows -> rows == 1 ? Mono.just(state)
    ...
    public Mono<Boolean> updateLastNotifyTime(long tenantId, long stateId, LocalDateTime lastNotifyTime) {
        ...
        return databaseClient.sql("UPDATE " + TABLE + " SET last_notify_time=:last_notify_time"
                        + " WHERE tenant_id=:tenant_id AND id=:id")
                .bind("last_notify_time", lastNotifyTime).bind("tenant_id", tenantId).bind("id", stateId)
    ...
    private RuleStateDO map(io.r2dbc.spi.Row row, io.r2dbc.spi.RowMetadata metadata) {
        ...
        value.setEntityId(row.get("entity_id", Long.class)); value.setFingerprint(row.get("fingerprint", String.class));
        value.setEntityStateFlag(number(row.get("entity_state_flag", Number.class)));
        value.setFirstTriggerTime(time(row.get("first_trigger_time"))); value.setLastTriggerTime(time(row.get("last_trigger_time")));
        value.setLastRecoverTime(time(row.get("last_recover_time"))); value.setLastNotifyTime(time(row.get("last_notify_time")));
        value.setTriggerCount(row.get("trigger_count", Long.class)); value.setAlarmId(row.get("alarm_id", Long.class));
        value.setEntityStateExt(json(row.get("entity_state_ext", String.class))); value.setTenantId(row.get("tenant_id", Long.class));
