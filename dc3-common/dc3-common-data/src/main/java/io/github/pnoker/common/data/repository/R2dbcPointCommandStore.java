...
public class R2dbcPointCommandStore implements ReactivePointCommandStore {
    ...
    private static final String COLUMNS = "id, command_id, tenant_id, type, device_id, point_id, request_value, response_value,"
    ...
    public Mono<PointCommandHistoryDO> find(Long tenantId, String commandId) {
        ...
        return databaseClient.sql("SELECT " + COLUMNS + " FROM " + TABLE
                ...
                .bind("tenant_id", tenantId).bind("command_id", commandId)
                .map(this::map).one();
    }

    @Override
    public Mono<PointCommandHistoryDO> insert(PointCommandHistoryDO command) {
        if (command == null || command.getTenantId() == null || command.getCommandId() == null
        ...
        DatabaseClient.GenericExecuteSpec spec = databaseClient.sql("INSERT INTO " + TABLE + " (" + COLUMNS + ") VALUES "
                ...
                .bind("id", command.getId()).bind("command_id", command.getCommandId())
                .bind("tenant_id", command.getTenantId()).bind("type", index(command.getType()))
                .bind("device_id", command.getDeviceId()).bind("point_id", command.getPointId())
                .bind("status", index(command.getStatus())).bind("source", index(command.getSource()))
                .bind("occur_time", command.getOccurTime());
        spec = bindNullable(spec, "expire_time", command.getExpireTime(), LocalDateTime.class);
        spec = spec
                .bind("schema_version", command.getSchemaVersion() == null ? (short) 1 : command.getSchemaVersion())
                .bind("create_time", command.getCreateTime()).bind("operate_time", command.getOperateTime());
        ...
        return spec.fetch().rowsUpdated().flatMap(rows -> rows == 1
    ...
    public Mono<Boolean> markSent(Long tenantId, String commandId, Instant sentAt) {
        return update(tenantId, commandId, "status = :status, send_time = :send_time", spec -> spec
                .bind("status", PointCommandStatusEnum.SENT.getIndex())
                .bind("send_time", local(sentAt)));
    }

    @Override
    public Mono<Boolean> markPublishFailed(Long tenantId, String commandId, String errorCode,
                                           String errorMessage, Instant finishedAt) {
        return update(tenantId, commandId, "status = :status, error_code = :error_code, error_message = :error_message, finish_time = :finish_time", spec -> {
            spec = spec.bind("status", PointCommandStatusEnum.FAILED.getIndex()).bind("finish_time", local(finishedAt));
    ...
    @Override
    public Mono<Boolean> complete(Long tenantId, String commandId, PointCommandStatusEnum status,
                                  String responseValue, String errorCode, String errorMessage, Instant finishedAt) {
        return update(tenantId, commandId, "status = :status, response_value = :response_value, error_code = :error_code, error_message = :error_message, finish_time = :finish_time", spec -> {
    ...
    @Override
    public Mono<Boolean> markDead(Long tenantId, String commandId, String errorCode,
                                  String errorMessage, Instant finishedAt) {
        return update(tenantId, commandId, "status = :status, error_code = :error_code, error_message = :error_message, finish_time = :finish_time", spec -> {
            spec = spec.bind("status", PointCommandStatusEnum.DEAD.getIndex()).bind("finish_time", local(finishedAt));
            spec = bindNullable(spec, "error_code", errorCode, String.class);
            return bindNullable(spec, "error_message", errorMessage, String.class);
        });
    }
    ...
    @Override
    public Mono<OffsetPage<PointCommandHistoryDO>> list(Long tenantId, Long deviceId, Long pointId,
                                                        PointCommandStatusEnum status, PointCommandTypeEnum type,
        ...
        DatabaseClient.GenericExecuteSpec count = bindFilters(databaseClient.sql("SELECT COUNT(*) AS total FROM " + TABLE + where), tenantId, deviceId, pointId, status, type);
        DatabaseClient.GenericExecuteSpec rows = bindFilters(databaseClient.sql("SELECT " + COLUMNS + " FROM " + TABLE + where
                + " ORDER BY " + orderBy(sort) + " LIMIT :limit OFFSET :offset"), tenantId, deviceId, pointId, status, type)
    ...
    private Mono<Boolean> update(Long tenantId, String commandId, String assignments,
    ...
    private DatabaseClient.GenericExecuteSpec bindFilters(DatabaseClient.GenericExecuteSpec spec, Long tenantId,
                                                           Long deviceId, Long pointId, PointCommandStatusEnum status,
    ...
    private PointCommandHistoryDO map(io.r2dbc.spi.Row row, io.r2dbc.spi.RowMetadata metadata) {
        ...
        value.setId(row.get("id", Long.class)); value.setCommandId(row.get("command_id", String.class));
        value.setTenantId(row.get("tenant_id", Long.class)); value.setDeviceId(row.get("device_id", Long.class));
        value.setPointId(row.get("point_id", Long.class)); value.setRequestValue(row.get("request_value", String.class));
        value.setResponseValue(row.get("response_value", String.class)); value.setErrorCode(row.get("error_code", String.class));
        value.setErrorMessage(row.get("error_message", String.class)); value.setSourceUserId(row.get("source_user_id", Long.class));
        value.setSchemaVersion(row.get("schema_version", Short.class)); value.setType(enumType(row.get("type", Number.class)));
        value.setStatus(enumStatus(row.get("status", Number.class))); value.setSource(enumSource(row.get("source", Number.class)));
    ...
    private Byte index(PointCommandTypeEnum value) { return value == null ? PointCommandTypeEnum.READ.getIndex() : value.getIndex(); }
    private Byte index(PointCommandStatusEnum value) { return value == null ? PointCommandStatusEnum.PENDING.getIndex() : value.getIndex(); }
    private Byte index(PointCommandSourceEnum value) { return value == null ? PointCommandSourceEnum.HTTP.getIndex() : value.getIndex(); }
    private PointCommandTypeEnum enumType(Number value) { return PointCommandTypeEnum.ofIndex(value == null ? null : value.byteValue()); }
    private PointCommandStatusEnum enumStatus(Number value) { return PointCommandStatusEnum.ofIndex(value == null ? null : value.byteValue()); }
    private PointCommandSourceEnum enumSource(Number value) { return PointCommandSourceEnum.ofIndex(value == null ? null : value.byteValue()); }
