...
public class R2dbcEntityAlarmStore implements ReactiveEntityAlarmStore {
    ...
    public Mono<EntityAlarmDO> insert(EntityAlarmDO alarm) {
        ...
        return transactionalOperator.transactional(spec.fetch().rowsUpdated())
                .flatMap(rows -> alarm.getDedupeKey() == null || alarm.getDedupeKey().isBlank()
                        ? (rows == 1 ? Mono.just(alarm)
                        : Mono.error(new IllegalStateException("entity alarm insert affected " + rows + " rows")))
                        : findByDedupe(alarm.getTenantId(), alarm.getDedupeKey())
                        .switchIfEmpty(Mono.error(new IllegalStateException("entity alarm dedupe row is missing"))));
    }
    ...
    public Mono<Boolean> delete(Long tenantId, Long alarmId) {
        ...
        return databaseClient.sql("DELETE FROM " + TABLE + " WHERE tenant_id=:tenant_id AND id=:id")
                .bind("tenant_id", tenantId).bind("id", alarmId).fetch().rowsUpdated()
                .map(rows -> rows == 1);
    }
    ...
    public Mono<Boolean> updateConfirm(Long tenantId, Long alarmId, byte confirmFlag) {
        ...
        return databaseClient.sql("UPDATE " + TABLE + " SET confirm_flag=:confirm_flag,operate_time=CURRENT_TIMESTAMP"
                        + " WHERE tenant_id=:tenant_id AND id=:id")
                .bind("confirm_flag", confirmFlag).bind("tenant_id", tenantId).bind("id", alarmId)
    ...
    private long stableId(EntityAlarmDO alarm) {
        ...
        UUID stable = UUID.nameUUIDFromBytes((alarm.getTenantId() + ":" + alarm.getDedupeKey())
                .getBytes(StandardCharsets.UTF_8));
        long id = (stable.getMostSignificantBits() ^ Long.rotateLeft(stable.getLeastSignificantBits(), 29)) & Long.MAX_VALUE;
        return id == 0 ? 1 : id;
    }

    private <T> DatabaseClient.GenericExecuteSpec bindNullable(DatabaseClient.GenericExecuteSpec spec, String name,
    ...
    private Mono<EntityAlarmDO> findByDedupe(Long tenantId, String dedupeKey) {
        ...
        return databaseClient.sql("SELECT " + COLUMNS + " FROM " + TABLE
                ...
                .bind("tenant_id", tenantId).bind("dedupe_key", dedupeKey)
                .map(this::map).one();
    }
}
