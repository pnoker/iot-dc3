...
/**
 * gRPC server implementation for the PointValue service.
 * Delegates to {@link PointValueService} and {@link PointValueCommandService}.
 */
@Slf4j
@Service
public class PointValueServer extends PointValueApiGrpc.PointValueApiImplBase {
    ...
    public PointValueServer(PointValueService pointValueService,
    ...
    public void lastValue(GrpcPointValueQuery request, StreamObserver<GrpcRPointValueDTO> responseObserver) {
        try {
            // latest() with a page query — simplified: query by device+point, return first result
            ...
            com.baomidou.mybatisplus.extension.plugins.pagination.Page<io.github.pnoker.common.entity.bo.PointValueBO> page =
                    pointValueService.latest(query);

            GrpcRPointValueDTO.Builder response = GrpcRPointValueDTO.newBuilder();
            response.setResult(GrpcR.newBuilder().setOk(true)
                    .setCode(ResponseEnum.OK.getCode())
                    .setMessage(ResponseEnum.OK.getText()).build());
            ...
            if (page != null && !page.getRecords().isEmpty()) {
                ...
                response.setData(GrpcPointValueDTO.newBuilder()
                        ...
                        .setCreateTime(bo.getCreateTime() != null ? bo.getCreateTime().toEpochSecond(java.time.ZoneOffset.UTC) : 0)
                        .build());
            }
            ...
            responseObserver.onNext(GrpcRPointValueDTO.newBuilder()
                    .setResult(GrpcR.newBuilder().setOk(false)
    ...
    @Override
    public void historyValue(GrpcPointValueHistoryQuery request, StreamObserver<GrpcRPointValueStringList> responseObserver) {
        try {
            List<String> history = pointValueService.history(
            ...
            GrpcRPointValueStringList.Builder response = GrpcRPointValueStringList.newBuilder()
                    .setResult(GrpcR.newBuilder().setOk(true)
                            .setCode(ResponseEnum.OK.getCode())
                            .setMessage(ResponseEnum.OK.getText()).build());
