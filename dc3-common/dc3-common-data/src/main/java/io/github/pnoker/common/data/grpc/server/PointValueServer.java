...
public class PointValueServer extends PointValueApiGrpc.PointValueApiImplBase {
    ...
    public void getLastValue(GrpcPointValueQuery request, StreamObserver<GrpcPointValueDTO> responseObserver) {
        io.github.pnoker.common.entity.query.PointValueQuery query =
                new io.github.pnoker.common.entity.query.PointValueQuery();
                ...
                pointValueService
                        ...
                        .flatMap(page -> page.items().isEmpty()
                                ? reactor.core.publisher.Mono.error(
                                        new io.github.pnoker.common.exception.NotFoundException("Point value"))
                                : reactor.core.publisher.Mono.just(
                                        toGrpcDTO(page.items().getFirst()))),
                responseObserver);
    }

    @Override
    public void listHistoryValues(
            GrpcPointValueHistoryQuery request, StreamObserver<GrpcPointValueCursorPage> responseObserver) {
        ReactiveGrpcServerSupport.subscribe(
                pointValueService
                        .history(
                                request.getTenantId(),
                        ...
                        .map(history -> {
                            GrpcPointValueCursorPage.Builder response = GrpcPointValueCursorPage.newBuilder()
                                    ...
                                    .addAllData(history.items().stream()
                                            .map(this::toGrpcDTO)
                                            .toList());
    ...
    @Override
    public void listSeriesVolumes(
            io.github.pnoker.api.center.data.GrpcPointVolumeQuery request,
            StreamObserver<GrpcPointVolumeList> responseObserver) {
        ReactiveGrpcServerSupport.subscribe(
                pointValueService
                        .seriesVolumes(request.getTenantId(), java.time.Instant.ofEpochMilli(request.getFromTime()))
                        .map(volumes -> {
                            return GrpcPointVolumeList.newBuilder()
    ...
    private GrpcPointValueDTO toGrpcDTO(io.github.pnoker.common.entity.bo.PointValueBO bo) {
        return GrpcPointValueDTO.newBuilder()
                ...
                .setCreateTime(
                        bo.getCreateTime() == null ? 0L : bo.getCreateTime().toEpochSecond(java.time.ZoneOffset.UTC))
                .build();
    }

    @Override
    public void readCommand(
            GrpcPointValueCommandQuery request, StreamObserver<GrpcPointCommandAccepted> responseObserver) {
        ...
        ReactiveGrpcServerSupport.subscribe(
                pointCommandService
                        .read(request.getTenantId(), entityBO)
                        .map(commandId -> GrpcPointCommandAccepted.newBuilder()
    ...
    public void writeCommand(
            GrpcPointValueWriteCommand request, StreamObserver<GrpcPointCommandAccepted> responseObserver) {
        ...
        ReactiveGrpcServerSupport.subscribe(
                pointCommandService
                        .write(request.getTenantId(), entityBO)
                        .map(commandId -> GrpcPointCommandAccepted.newBuilder()
                                .setCommandId(commandId)
                                .build()),
                responseObserver);
