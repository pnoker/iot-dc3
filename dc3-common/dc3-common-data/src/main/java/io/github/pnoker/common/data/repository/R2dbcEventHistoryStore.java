...
public class R2dbcEventHistoryStore implements ReactiveEventHistoryStore {
    ...
    private static final String COLUMNS = "id,record_id,tenant_id,device_id,event_id,event_code,event_type_flag,event_level_flag,"
    ...
    public Mono<EventHistoryDO> insert(EventHistoryDO event) {
        if (event == null || event.getTenantId() == null || event.getTenantId() <= 0
                || event.getRecordId() == null || event.getRecordId().isBlank()) {
        ...
        String sql = "INSERT INTO " + TABLE + " (" + COLUMNS + ") VALUES (:id,:record_id,:tenant_id,:device_id,:event_id,:event_code,:event_type_flag,:event_level_flag,"
        ...
        DatabaseClient.GenericExecuteSpec spec = databaseClient.sql(sql)
                .bind("id", event.getId()).bind("record_id", event.getRecordId()).bind("tenant_id", event.getTenantId())
                .bind("device_id", event.getDeviceId()).bind("event_id", event.getEventId()).bind("event_code", event.getEventCode())
                .bind("event_type_flag", value(event.getEventTypeFlag())).bind("event_level_flag", value(event.getEventLevelFlag()))
                .bind("message", event.getMessage()).bind("occur_time", event.getOccurTime()).bind("receive_time", event.getReceiveTime())
                .bind("acknowledge_flag", value(event.getAcknowledgeFlag())).bind("schema_version", event.getSchemaVersion() == null ? (short) 1 : event.getSchemaVersion())
                .bind("create_time", event.getCreateTime()).bind("operate_time", event.getOperateTime());
        ...
        return transactionalOperator.transactional(spec.fetch().rowsUpdated()).flatMap(rows -> rows == 1 ? Mono.just(event)
    ...
    public Mono<EventHistoryDO> findByRecordId(Long tenantId, String recordId) {
        ...
        return databaseClient.sql("SELECT " + COLUMNS + " FROM " + TABLE + " WHERE tenant_id=:tenant_id AND record_id=:record_id LIMIT 1")
                .bind("tenant_id", tenantId).bind("record_id", recordId).map(this::map).one();
    }

    @Override
    public Mono<OffsetPage<EventHistoryDO>> list(Long tenantId, Long deviceId, Long eventId, String eventCode,
        ...
        if (offset < 0 || limit < 1 || limit > PageRequest.MAX_LIMIT) return Mono.error(new IllegalArgumentException("invalid page bounds"));
        ...
        if (deviceId != null) { predicate.append(" AND device_id=:device_id"); base = base.bind("device_id", deviceId); }
        if (eventId != null) { predicate.append(" AND event_id=:event_id"); base = base.bind("event_id", eventId); }
        if (eventCode != null && !eventCode.isBlank()) { predicate.append(" AND event_code=:event_code"); base = base.bind("event_code", eventCode); }
        if (eventTypeFlag != null) { predicate.append(" AND event_type_flag=:event_type_flag"); base = base.bind("event_type_flag", eventTypeFlag); }
        String order = orderBy(sort);
        Mono<Long> total = databaseClient.sql("SELECT COUNT(*) AS total FROM " + TABLE + predicate)
                .bind("tenant_id", tenantId)
                .map((row, metadata) -> row.get("total", Number.class).longValue()).one();
        String query = "SELECT " + COLUMNS + " FROM " + TABLE + predicate + " ORDER BY " + order + " LIMIT :limit OFFSET :offset";
        DatabaseClient.GenericExecuteSpec rowsSpec = databaseClient.sql(query).bind("tenant_id", tenantId).bind("limit", limit).bind("offset", offset);
    ...
    private EventHistoryDO map(io.r2dbc.spi.Row row, io.r2dbc.spi.RowMetadata metadata) {
        ...
        value.setId(row.get("id", Long.class)); value.setRecordId(row.get("record_id", String.class)); value.setTenantId(row.get("tenant_id", Long.class));
        value.setDeviceId(row.get("device_id", Long.class)); value.setEventId(row.get("event_id", Long.class)); value.setEventCode(row.get("event_code", String.class));
        value.setEventTypeFlag(number(row.get("event_type_flag"))); value.setEventLevelFlag(number(row.get("event_level_flag")));
        value.setParamValues(text(row.get("param_values", String.class))); value.setConfigSnapshot(text(row.get("config_snapshot", String.class))); value.setMessage(row.get("message", String.class));
        value.setOccurTime(time(row.get("occur_time"))); value.setReceiveTime(time(row.get("receive_time"))); value.setAcknowledgeFlag(number(row.get("acknowledge_flag")));
        value.setAcknowledgeTime(time(row.get("acknowledge_time"))); value.setAcknowledgeUserId(row.get("acknowledge_user_id", Long.class)); value.setSchemaVersion(row.get("schema_version", Short.class));
        value.setCreateTime(time(row.get("create_time"))); value.setOperateTime(time(row.get("operate_time"))); return value;
    }
    ...
    private int value(Byte value) { return value == null ? 0 : value; }
    ...
    private <T> DatabaseClient.GenericExecuteSpec bindNullable(DatabaseClient.GenericExecuteSpec spec, String name, T value, Class<T> type) { return value == null ? spec.bindNull(name, type) : spec.bind(name, value); }
}
