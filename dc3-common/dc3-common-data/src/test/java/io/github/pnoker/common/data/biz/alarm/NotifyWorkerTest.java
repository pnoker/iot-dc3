...
class NotifyWorkerTest {
    ...
    @Mock private NotifyChannelAdapterRegistry notifyChannelAdapterRegistry;
    ...
    @InjectMocks private NotifyWorker worker;
    ...
    void marksSuccessAndAcks() {
        ...
        when(notifyHistoryStore.updateDelivery(anyLong(), anyLong(), anyByte(), any(), any(), any(), anyInt())).thenReturn(Mono.just(true));
    ...
    void rejectsMissingTenantBeforeDatabaseAccess() {
        NotifyTaskDTO invalid = NotifyTaskDTO.builder().notifyHistoryId(50L).channelId(2L).build();
    ...
    void rejectsWhenHistoryUpdateAffectsNoRows() {
        ...
        when(notifyChannelAdapterRegistry.find(NotifyChannelTypeEnum.WEBHOOK)).thenReturn(Optional.of(adapter));
        when(adapter.send(any(), any())).thenReturn(Mono.just(NotifySendResult.success("target", 200, "OK", Map.of())));
        when(notifyHistoryStore.updateDelivery(anyLong(), anyLong(), anyByte(), any(), any(), any(), anyInt())).thenReturn(Mono.just(false));
    ...
    private NotifyTaskDTO task(int retry) {
        return NotifyTaskDTO.builder().notifyHistoryId(50L).tenantId(7L).channelId(2L)
                .channelTypeFlag(NotifyChannelTypeEnum.WEBHOOK.getIndex()).payloadType("text")
                .payload(Map.of("text", "hello")).retryCount(retry).build();
    }

    private void stubChannel(boolean enabled) {
        NotifyChannelBO channel = new NotifyChannelBO(); channel.setId(2L); channel.setTenantId(7L);
        channel.setEnableFlag(enabled ? EnableFlagEnum.ENABLE : EnableFlagEnum.DISABLE);
        channel.setChannelTypeFlag(NotifyChannelTypeEnum.WEBHOOK); channel.setCredentialRef("target");
