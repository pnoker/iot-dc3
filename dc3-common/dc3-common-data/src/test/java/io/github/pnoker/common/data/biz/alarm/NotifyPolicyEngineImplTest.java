...
class NotifyPolicyEngineImplTest {
    ...
    void sendsWhenPolicyAllowsSeverityAndRateLimit() {
        NotifyDecision decision = engine.decide(
                match("P1", AlarmConstant.MATCH_TYPE_FIRING),
                notifyPolicy(),
                bind(List.of("P0", "P1"), true, 300000L),
                new RuleStateBO(),
                LocalDateTime.now());
    ...
    void skipsSeverityNotAllowedByBinding() {
        NotifyDecision decision = engine.decide(
                match("P3", AlarmConstant.MATCH_TYPE_FIRING),
                notifyPolicy(),
                bind(List.of("P0", "P1"), true, 300000L),
                new RuleStateBO(),
                LocalDateTime.now());
    ...
    void skipsRecoveryWhenBindingDisablesRecovery() {
        NotifyDecision decision = engine.decide(
                match("P1", AlarmConstant.MATCH_TYPE_RECOVERY),
                notifyPolicy(),
                bind(List.of("P0", "P1"), false, 300000L),
                new RuleStateBO(),
                LocalDateTime.now());
    ...
    void silenceWindowBoundaryMatrix(String label, LocalDateTime now, String start, String end, boolean silenced) {
        ...
        NotifyDecision decision = engine.decide(
                match("P1", AlarmConstant.MATCH_TYPE_FIRING), notify, bind(null, true, null), new RuleStateBO(), now);
    ...
    void silenceWindowRespectsDaysOfWeek() {
        ...
        NotifyDecision decisionThu = engine.decide(
                match("P1", AlarmConstant.MATCH_TYPE_FIRING),
                notify,
                bind(null, true, null),
                new RuleStateBO(),
        ...
        NotifyDecision decisionMon = engine.decide(
                match("P1", AlarmConstant.MATCH_TYPE_FIRING),
                notify,
                bind(null, true, null),
                new RuleStateBO(),
    ...
    @ParameterizedTest(
            name =
                    "bind.rateLimitOverrideMs={0}, notify.rateLimit.intervalMs={1}, notifyInterval={2} => intervalMs={3}")
    @CsvSource({
    ...
    void rateLimitIntervalFallsBackInPriorityOrder(
            Long bindOverride, Long notifyIntervalMs, Long entityInterval, Long expectedInterval) {
        RuleStateBO state = new RuleStateBO();
        ...
        NotifyDecision decision = engine.decide(
                match("P1", AlarmConstant.MATCH_TYPE_FIRING), notify, bind(null, true, bindOverride), state, now);
