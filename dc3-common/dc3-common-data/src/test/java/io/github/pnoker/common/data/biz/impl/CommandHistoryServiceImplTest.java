...
class CommandHistoryServiceImplTest {
    ...
    @Mock
    ReactiveCommandHistoryStore historyStore;

    @Mock
    ReactiveMessageSender sender;

    @Mock
    CommandHistoryBuilder builder;
    ...
    @BeforeEach
    void setUp() {
        service = new CommandHistoryServiceImpl(
                deviceFacade, driverFacade, commandFacade, context, historyStore, sender, builder);
        device = new FacadeDeviceBO();
        device.setId(10L);
        device.setDriverId(40L);
    ...
    void callPersistsAndPublishesReactively() {
        ...
        StepVerifier.create(service.call(100L, request))
                .assertNext(recordId -> assertThat(recordId).isNotBlank())
                .verifyComplete();
    ...
    void publishFailureMarksHistoryAndPropagates() {
        ...
        StepVerifier.create(service.call(100L, request(null)))
                .expectErrorMessage("Failed to route custom command to active driver owner")
                .verify();
    ...
    private CommandCallBO request(Map<String, String> values) {
        ...
        request.setCommandId(20L);
        request.setParamValues(values);
        return request;
    }
}
