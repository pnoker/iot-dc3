...
public class R2dbcResourceStore implements ReactiveResourceStore {
    ...
    public Mono<ResourceDO> getByTypeAndEntity(ResourceTypeEnum resourceType, Long entityId) {
        ...
        return databaseClient.sql("SELECT " + COLUMNS + " FROM " + TABLE
                ...
                .bind("resource_type_flag", resourceType.getIndex()).bind("entity_id", entityId)
                .map(this::map).one();
    }
    ...
    public Mono<Boolean> delete(Long id, Long operatorId, String operatorName) {
        ...
        DatabaseClient.GenericExecuteSpec resourceDelete = databaseClient.sql("UPDATE " + TABLE
                ...
                .bind("operate_time", now());
        DatabaseClient.GenericExecuteSpec bindingDelete = databaseClient.sql("UPDATE dc3_auth.dc3_role_resource_bind"
                        + " SET deleted=1,operator_id=:operator_id,operator_name=:operator_name,operate_time=:operate_time"
                ...
                .bind("operator_name", text(operatorName))
                .bind("operate_time", now());
        return transactionalOperator.transactional(resourceDelete.fetch().rowsUpdated()
                        .flatMap(updated -> updated == 1
                                ? bindingDelete.fetch().rowsUpdated().thenReturn(true)
                                : Mono.just(false)));
    }
    ...
    public Mono<Boolean> hasChildren(Long id) {
        ...
        return databaseClient.sql("SELECT 1 FROM " + TABLE
                        + " WHERE parent_resource_id=:id AND deleted=0 LIMIT 1")
                .bind("id", id)
    ...
    private String whereClause(ResourceFilter filter) {
        ...
        if (filter.resourceName() != null) where.append(" AND ").append(dialect.caseInsensitiveLike("resource_name", ":resource_name"));
        ...
        if (!filter.resourceTypeFlags().isEmpty()) where.append(" AND resource_type_flag IN (")
                .append(placeholders("resource_type_flag", filter.resourceTypeFlags().size())).append(')');
        if (!filter.resourceScopeFlags().isEmpty()) where.append(" AND resource_scope_flag IN (")
                .append(placeholders("resource_scope_flag", filter.resourceScopeFlags().size())).append(')');
    ...
    private DatabaseClient.GenericExecuteSpec bind(DatabaseClient.GenericExecuteSpec spec, ResourceFilter filter) {
        ...
        for (int i = 0; i < filter.resourceTypeFlags().size(); i++) {
            spec = spec.bind("resource_type_flag" + i, filter.resourceTypeFlags().get(i).getIndex());
        }
        for (int i = 0; i < filter.resourceScopeFlags().size(); i++) {
            spec = spec.bind("resource_scope_flag" + i, filter.resourceScopeFlags().get(i).getIndex());
        }
        return spec;
    }
        ...
        if (sort != null) {
            for (SortSpec spec : sort) {
                String column = switch (spec.field()) {
                    ...
                    default -> throw new IllegalArgumentException("unsupported resource sort field: " + spec.field());
                };
