...
public class R2dbcResourceRegistryStore implements ReactiveResourceRegistryStore {
    ...
    public Flux<ApiDO> listApis(String serviceName) {
        return databaseClient.sql("SELECT " + API_COLUMNS + " FROM " + API_TABLE
    ...
    public Mono<ApiDO> insertApi(ApiDO api) {
        ...
        return query.fetch().rowsUpdated().flatMap(updated -> updated == 1
                ? getApiById(id)
    ...
    public Mono<Boolean> deleteApi(Long id, Long operatorId, String operatorName) {
        ...
        return databaseClient.sql("UPDATE " + API_TABLE
                ...
                .bind("operate_time", now())
                .fetch().rowsUpdated().map(updated -> updated == 1);
    }

    @Override
    public Flux<ResourceDO> listApiResources(String serviceName) {
        return databaseClient.sql("SELECT " + RESOURCE_COLUMNS + " FROM " + RESOURCE_TABLE
    ...
    public Mono<ResourceDO> insertResource(ResourceDO resource) {
        long id = newId();
        LocalDateTime now = now();
        String sql = "INSERT INTO " + RESOURCE_TABLE + " (id,parent_resource_id,resource_name,resource_code,service_name,"
                + "resource_type_flag,resource_scope_flag,entity_id,resource_ext,enable_flag,remark,creator_id,creator_name,"
    ...
    public Mono<ResourceDO> updateResource(ResourceDO resource) {
        ...
        String sql = "UPDATE " + RESOURCE_TABLE + " SET parent_resource_id=:parent_resource_id,resource_name=:resource_name,"
    ...
    public Mono<Boolean> deleteResource(Long id, Long operatorId, String operatorName) {
        if (id == null) return Mono.just(false);
        LocalDateTime now = now();
        DatabaseClient.GenericExecuteSpec resource = databaseClient.sql("UPDATE " + RESOURCE_TABLE
                        + " SET deleted=1,operator_id=:operator_id,operator_name=:operator_name,operate_time=:operate_time"
                        + " WHERE id=:id AND deleted=0")
                .bind("id", id).bind("operator_id", longValue(operatorId))
                .bind("operator_name", text(operatorName)).bind("operate_time", now);
        DatabaseClient.GenericExecuteSpec bindings = databaseClient.sql("UPDATE dc3_auth.dc3_role_resource_bind"
                        + " SET deleted=1,operator_id=:operator_id,operator_name=:operator_name,operate_time=:operate_time"
                        + " WHERE resource_id=:resource_id AND deleted=0")
                .bind("resource_id", id).bind("operator_id", longValue(operatorId))
                .bind("operator_name", text(operatorName)).bind("operate_time", now);
        return resource.fetch().rowsUpdated().flatMap(updated -> updated == 1
                ? bindings.fetch().rowsUpdated().thenReturn(true)
                : Mono.just(false));
    }

    @Override
    public Mono<Long> countChildren(Long parentId) {
        return databaseClient.sql("SELECT COUNT(*) AS total FROM " + RESOURCE_TABLE
    ...
    public Mono<Long> acquireLock(String lockName) {
        ...
        return databaseClient.sql(insert).bind("lock_name", lockName)
                .bind("expires_at", dialect.bindInstant(expires)).fetch().rowsUpdated()
                .then(databaseClient.sql("SELECT fencing_token FROM dc3_platform_lock WHERE lock_name=:lock_name FOR UPDATE")
                        .bind("lock_name", lockName)
                        .map((row, metadata) -> ((Number) row.get("fencing_token")).longValue())
                        .one())
                .flatMap(current -> databaseClient.sql("UPDATE dc3_platform_lock SET fencing_token=:next_token,expires_at=:expires_at"
    ...
    private Mono<ApiDO> getApiById(Long id) {
        return databaseClient.sql("SELECT " + API_COLUMNS + " FROM " + API_TABLE + " WHERE id=:id AND deleted=0")
                .bind("id", id).map(this::mapApi).one();
    }

    private Mono<ResourceDO> getResourceById(Long id) {
        return databaseClient.sql("SELECT " + RESOURCE_COLUMNS + " FROM " + RESOURCE_TABLE + " WHERE id=:id AND deleted=0")
                .bind("id", id).map(this::mapResource).one();
    }
    ...
    private ApiDO mapApi(io.r2dbc.spi.Row row, io.r2dbc.spi.RowMetadata metadata) {
        ApiDO api = new ApiDO();
        api.setId(row.get("id", Long.class)); api.setServiceName(row.get("service_name", String.class));
    ...
    private ResourceDO mapResource(io.r2dbc.spi.Row row, io.r2dbc.spi.RowMetadata metadata) {
        ...
        resource.setId(row.get("id", Long.class)); resource.setParentResourceId(row.get("parent_resource_id", Long.class));
        resource.setResourceName(row.get("resource_name", String.class)); resource.setResourceCode(row.get("resource_code", String.class));
        resource.setServiceName(row.get("service_name", String.class)); resource.setResourceTypeFlag(number(row.get("resource_type_flag")));
        resource.setResourceScopeFlag(number(row.get("resource_scope_flag"))); resource.setEntityId(row.get("entity_id", Long.class));
        resource.setResourceExt(parseJson(row.get("resource_ext", String.class))); resource.setEnableFlag(number(row.get("enable_flag")));
        resource.setRemark(row.get("remark", String.class)); resource.setCreatorId(row.get("creator_id", Long.class));
        resource.setCreatorName(row.get("creator_name", String.class)); resource.setCreateTime(time(row.get("create_time")));
        resource.setOperatorId(row.get("operator_id", Long.class)); resource.setOperatorName(row.get("operator_name", String.class));
        resource.setOperateTime(time(row.get("operate_time"))); resource.setDeleted(number(row.get("deleted"))); return resource;
    }

    private JsonExt parseJson(String value) {
        if (value == null) return null;
        try { return JsonUtil.parseObject(value, JsonExt.class); } catch (RuntimeException ignored) { return null; }
    }
    ...
    private byte value(Byte value) { return value == null ? 0 : value; }
