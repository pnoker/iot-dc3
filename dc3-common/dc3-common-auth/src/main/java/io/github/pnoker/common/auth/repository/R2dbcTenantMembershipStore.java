...
public class R2dbcTenantMembershipStore implements ReactiveTenantMembershipStore, ReactiveTenantMembershipCommandStore {
    ...
    private static final String COLUMNS = "m.id,m.tenant_id,m.principal_id,m.principal_type,m.membership_status,m.joined_time,m.membership_ext,m.remark,m.creator_id,m.creator_name,m.create_time,m.operator_id,m.operator_name,m.operate_time,m.deleted";
    ...
    public Mono<TenantMembershipDO> getById(Long tenantId, Long id) {
        ...
        return databaseClient.sql("SELECT " + COLUMNS + " FROM " + TABLE + " m WHERE m.tenant_id=:tenant_id AND m.id=:id AND m.deleted=0 LIMIT 1")
    ...
    public Mono<TenantMembershipDO> getByTenantAndPrincipal(Long tenantId, Long principalId) {
        ...
        return databaseClient.sql("SELECT " + COLUMNS + " FROM " + TABLE + " m WHERE m.tenant_id=:tenant_id AND m.principal_id=:principal_id AND m.membership_status='ACTIVE' AND m.deleted=0 LIMIT 1")
    ...
    public Flux<Long> listPrincipalIds(Long tenantId) {
        ...
        return databaseClient.sql("SELECT m.principal_id FROM " + TABLE + " m WHERE m.tenant_id=:tenant_id AND m.membership_status='ACTIVE' AND m.deleted=0 ORDER BY m.principal_id")
                .bind("tenant_id", tenantId).map((row, metadata) -> row.get("principal_id", Long.class)).all();
    }
    ...
    public Mono<OffsetPage<TenantMembershipDO>> list(TenantMembershipFilter filter) {
        ...
        DatabaseClient.GenericExecuteSpec count = bind(databaseClient.sql("SELECT COUNT(*) AS total FROM " + TABLE + " m" + condition), filter);
        DatabaseClient.GenericExecuteSpec rows = bind(databaseClient.sql("SELECT " + COLUMNS + " FROM " + TABLE + " m" + condition
    ...
    public Mono<TenantMembershipDO> insert(TenantMembershipDO membership) {
        ...
        DatabaseClient.GenericExecuteSpec statement = databaseClient.sql("INSERT INTO " + TABLE
                ...
                .bind("id", membership.getId()).bind("tenant_id", membership.getTenantId())
                .bind("principal_id", membership.getPrincipalId()).bind("joined_time", membership.getJoinedTime())
                .bind("create_time", membership.getCreateTime()).bind("operate_time", membership.getOperateTime())
    ...
    public Mono<Boolean> delete(Long tenantId, Long id, Long operatorId, String operatorName) {
        ...
        return transactionalOperator.transactional(databaseClient.sql("UPDATE " + TABLE
                ...
                .bind("tenant_id", tenantId).bind("id", id).bind("operator_id", operatorId)
                .bind("operator_name", operatorName).bind("operate_time", LocalDateTime.now(ZoneOffset.UTC))
                .fetch().rowsUpdated().map(rows -> rows == 1));
    }

    private DatabaseClient.GenericExecuteSpec bind(DatabaseClient.GenericExecuteSpec spec, TenantMembershipFilter filter) {
        spec = spec.bind("tenant_id", filter.tenantId());
        if (filter.principalId() != null) spec = spec.bind("principal_id", filter.principalId());
        if (filter.principalType() != null) spec = spec.bind("principal_type", filter.principalType().getValue());
        if (filter.membershipStatus() != null) spec = spec.bind("membership_status", filter.membershipStatus().getValue());
        return spec;
    }
    ...
    private TenantMembershipDO map(io.r2dbc.spi.Row row, io.r2dbc.spi.RowMetadata metadata) {
        ...
        value.setPrincipalId(row.get("principal_id", Long.class)); value.setPrincipalType(row.get("principal_type", String.class));
        value.setMembershipStatus(row.get("membership_status", String.class)); value.setJoinedTime(time(row.get("joined_time")));
        value.setMembershipExt(json(row.get("membership_ext", String.class))); value.setRemark(row.get("remark", String.class));
    ...
    private String orderBy(List<SortSpec> sort) {
        ...
        for (SortSpec spec : sort) {
            String column = switch (spec.field()) {
                case "id" -> "m.id"; case "principalId" -> "m.principal_id"; case "principalType" -> "m.principal_type";
                case "membershipStatus" -> "m.membership_status"; case "joinedTime" -> "m.joined_time";
                case "createTime" -> "m.create_time"; case "operateTime" -> "m.operate_time";
                default -> throw new IllegalArgumentException("unsupported tenant membership sort field: " + spec.field());
