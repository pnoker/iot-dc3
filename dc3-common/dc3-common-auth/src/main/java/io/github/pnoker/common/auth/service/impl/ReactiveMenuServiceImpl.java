...
public class ReactiveMenuServiceImpl implements ReactiveMenuService {
    ...
    public Flux<MenuTreeBO> listTree(MenuFilter filter) {
        return Mono.defer(() -> store.listTree(filter).map(builder::buildBOByDO)
                        .map(MenuTreeBO::fromBO).collectList())
            ...
            return requireParent(menu.getParentMenuId())
                    ...
                    .flatMap(duplicate -> duplicate
                            ...
                            .flatMap(saved -> syncMenuResource(saved)
                                    .doOnSuccess(ignored -> invalidateAll()).thenReturn(saved)))
                    .onErrorMap(DuplicateKeyException.class, error -> new DuplicateException("Menu code is already in use"));
        });
    ...
    public Mono<MenuBO> update(MenuBO menu) {
        Mono<MenuBO> operation = Mono.defer(() -> {
            ...
            Mono<Void> cycle = parentId == null || parentId == 0L ? Mono.empty()
                    : Mono.defer(() -> store.isDescendant(menu.getId(), parentId))
                    .flatMap(found -> found
                            ? Mono.<Void>error(new RequestException("Menu parent would create a cycle"))
                            : Mono.empty());
            return requireParent(parentId).then(cycle)
                    .then(Mono.defer(() -> store.existsDuplicate(menu)))
                    .flatMap(duplicate -> duplicate
                            ? Mono.<MenuBO>error(new DuplicateException("Menu has been duplicated"))
                            : Mono.defer(() -> store.update(menu)).map(builder::buildBOByDO)
                            .flatMap(saved -> syncMenuResource(saved)
                                    .doOnSuccess(ignored -> invalidateAll()).thenReturn(saved)))
                    .switchIfEmpty(Mono.error(new NotFoundException("Menu")))
                    .onErrorMap(DuplicateKeyException.class, error -> new DuplicateException("Menu code is already in use"));
    ...
    public Mono<Void> delete(Long id, Long operatorId, String operatorName) {
        Mono<Void> operation = getById(id).then(Mono.defer(() -> store.hasChildren(id)))
                ...
                .flatMap(deleted -> Boolean.TRUE.equals(deleted)
                        ...
                        .doOnSuccess(ignored -> invalidateAll()).thenReturn(true)
                        : Mono.just(false))
                .flatMap(deleted -> Boolean.TRUE.equals(deleted)
                        ? Mono.<Void>empty() : Mono.error(new NotFoundException("Menu")));
        return transactional(operation);
    }
    ...
    private Mono<Void> requireParent(Long parentId) {
        ...
        return Mono.defer(() -> store.getById(parentId))
                .switchIfEmpty(Mono.error(new NotFoundException("Menu parent"))).then();
    }
    ...
    private Mono<Void> syncMenuResource(MenuBO menu) {
        ...
        return parent.flatMap(parentId -> Mono.defer(() -> resourceStore
                        .getByTypeAndEntity(ResourceTypeEnum.MENU, menu.getId()))
                .flatMap(existing -> resourceStore.update(resourceMirror(menu, parentId, existing.getId()))
    ...
    private io.github.pnoker.common.auth.entity.bo.ResourceBO resourceMirror(MenuBO menu, Long parentId, Long id) {
        io.github.pnoker.common.auth.entity.bo.ResourceBO resource = new io.github.pnoker.common.auth.entity.bo.ResourceBO();
        resource.setId(id);
    ...
    private void validate(MenuBO menu, boolean update) {
        ...
        if (menu.getMenuName() == null || menu.getMenuName().isBlank()) throw new RequestException("Menu name is required");
