...
public class ReactiveRoleResourceBindServiceImpl implements ReactiveRoleResourceBindService {
    ...
    public Mono<RoleResourceBindBO> add(RoleResourceBindBO binding, Long tenantId) {
        ...
        return roleService.getById(tenantId, binding.getRoleId())
                .then(resourceStore.listEnabledByIds(java.util.List.of(binding.getResourceId())).hasElements()
                        .flatMap(enabled -> enabled ? Mono.<Void>empty() : Mono.error(new NotFoundException("Resource"))))
    ...
    public Mono<Void> delete(Long tenantId, Long id, Long operatorId, String operatorName) {
        if (!valid(tenantId) || !valid(id)) return Mono.error(new RequestException("Role resource binding ID is required"));
        return store.delete(tenantId, id, operatorId, operatorName)
                .flatMap(deleted -> Boolean.TRUE.equals(deleted) ? Mono.<Void>empty() : Mono.error(new NotFoundException("Role resource bind")))
    ...
    public Mono<OffsetPage<RoleResourceBindBO>> list(RoleResourceBindFilter filter) {
        return store.list(filter).map(page -> OffsetPage.of(page.items().stream().map(bindingBuilder::buildBOByDO).toList(), page.offset(), page.limit(), page.total()));
    }

    @Override
    public Flux<ResourceBO> listResourcesByRole(Long tenantId, Long roleId) {
        return roleService.getById(tenantId, roleId).thenMany(store.listResourceIds(tenantId, roleId)).distinct()
                .collectList().flatMapMany(resourceStore::listEnabledByIds).map(resourceBuilder::buildBOByDO);
    }
    ...
    public Flux<ResourceBO> listResourcesByPrincipal(Long tenantId, Long principalId) {
        return membershipService.requireTenantMember(tenantId, principalId)
                .thenMany(store.listResourceIdsByPrincipal(tenantId, principalId)).distinct()
                .collectList().flatMapMany(resourceStore::listEnabledByIds).map(resourceBuilder::buildBOByDO);
    }

    @Override
    public Flux<RoleBO> listRolesByResource(Long tenantId, Long resourceId) {
        if (!valid(tenantId) || !valid(resourceId)) return Flux.error(new RequestException("Tenant and resource IDs are required"));
        return resourceStore.listEnabledByIds(java.util.List.of(resourceId)).hasElements()
                .flatMapMany(enabled -> enabled
                        ? store.listRoleIdsByResource(tenantId, resourceId).distinct().flatMap(id -> roleService.getById(tenantId, id))
