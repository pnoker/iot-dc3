...
public class ReactiveLocalCredentialCommandServiceImpl implements ReactiveLocalCredentialCommandService {
    ...
    @Override
    public Mono<LocalCredentialBO> add(
            Long tenantId, LocalCredentialBO credential, Long operatorId, String operatorName) {
        if (!validTenant(tenantId)
                || credential == null
                || credential.getPrincipalId() == null
                || credential.getPrincipalId() <= 0
                || blank(credential.getLoginName())
                || blank(credential.getRawPassword())) {
        ...
        return requireMember(tenantId, credential.getPrincipalId())
                ...
                .flatMap(prepared -> {
                    ...
                    DatabaseClient.GenericExecuteSpec insert = databaseClient
                            .sql(
                                    "INSERT INTO " + TABLE
                                            ...
                                            + " VALUES (:id,:principal_id,:login_name,:login_name_normalized,:credential_type,:password_hash,:password_algorithm,"
                                            + dialect.jsonWriteExpression(":password_params")
                                            + ",:password_updated_time,:password_expire_time,:failed_attempts,NULL,:require_password_change,:enable_flag,"
                                            + dialect.jsonWriteExpression(":credential_ext")
                                            + ",:remark,:creator_id,:creator_name,:create_time,:operator_id,:operator_name,:operate_time,0)")
                            .bind("id", id)
                            .bind("principal_id", prepared.getPrincipalId())
                            .bind("login_name", text(prepared.getLoginName()))
                            .bind("login_name_normalized", normalize(prepared.getLoginName()))
                            .bind("credential_type", CredentialTypeEnum.PASSWORD.getValue())
                            .bind("password_hash", prepared.getPasswordHash())
                            .bind(
                                    "password_algorithm",
                                    prepared.getPasswordAlgorithm().getValue())
                            .bind("password_params", json(row.getPasswordParams()))
                            .bind("password_updated_time", now)
                            .bind("failed_attempts", 0)
                            .bind("require_password_change", flag(prepared.getRequirePasswordChange()))
                            .bind("enable_flag", flag(prepared.getEnableFlag()))
                            .bind("credential_ext", json(row.getCredentialExt()))
                            .bind("remark", text(prepared.getRemark()))
                    ...
                    insert = bindNullable(
                            insert, "password_expire_time", prepared.getPasswordExpireTime(), LocalDateTime.class);
                    return transactionalOperator
                            .transactional(insert.fetch()
                                    .rowsUpdated()
                                    .flatMap(rows -> rows == 1
                                            ? Mono.empty()
                                            : Mono.error(new IllegalStateException("credential insert failed"))))
                            .then(reactiveLocalCredentialService.getById(tenantId, id));
                })
                .onErrorMap(
                        DataIntegrityViolationException.class,
                        error -> new DuplicateException("Local credential has been duplicated"));
    }

    @Override
    public Mono<LocalCredentialBO> update(
            Long tenantId, LocalCredentialBO credential, Long operatorId, String operatorName) {
        ...
        return reactiveLocalCredentialService
                .getById(tenantId, credential.getId())
    ...
    public Mono<Boolean> delete(Long tenantId, Long id, Long operatorId, String operatorName) {
        if (!validTenant(tenantId) || id == null || id <= 0)
            return Mono.error(new EmptyException("Credential id is required"));
        return databaseClient
                .sql("UPDATE " + TABLE
                        + " c SET deleted=1,operator_id=:operator_id,operator_name=:operator_name,operate_time=:operate_time"
                        + " WHERE c.id=:id AND c.deleted=0 AND EXISTS (SELECT 1 FROM " + MEMBERSHIP
                        + " m WHERE m.tenant_id=:tenant_id"
                ...
                .flatMap(rows -> rows == 1
                        ...
                        : Mono.error(new NotFoundException("Local credential does not exist")))
                .as(transactionalOperator::transactional);
    }

    @Override
    public Mono<LocalCredentialBO> resetPassword(
    ...
    @Override
    public Mono<LocalCredentialBO> changePassword(
            Long tenantId,
            String loginName,
    ...
    public Mono<Void> recordSuccessfulLogin(Long tenantId, Long id) {
        return databaseClient
                .sql("UPDATE " + TABLE + " c SET failed_attempts=0,locked_until=NULL"
                        + " WHERE c.id=:id AND c.deleted=0 AND EXISTS (SELECT 1 FROM " + MEMBERSHIP
                        + " m WHERE m.tenant_id=:tenant_id"
                        + " AND m.principal_id=c.principal_id AND m.membership_status='ACTIVE' AND m.deleted=0)")
                .bind("id", id)
    ...
    public Mono<Void> recordFailedLogin(Long tenantId, Long id) {
        return databaseClient
                .sql("UPDATE " + TABLE
                        + " c SET failed_attempts=c.failed_attempts+1,locked_until=CASE WHEN c.failed_attempts+1>=:max_attempts THEN CURRENT_TIMESTAMP + (:lock_minutes * INTERVAL '1 minute') ELSE c.locked_until END"
                        + " WHERE c.id=:id AND c.deleted=0 AND EXISTS (SELECT 1 FROM " + MEMBERSHIP
                        + " m WHERE m.tenant_id=:tenant_id"
                        + " AND m.principal_id=c.principal_id AND m.membership_status='ACTIVE' AND m.deleted=0)")
                .bind("max_attempts", MAX_FAILED_ATTEMPTS)
                .bind("lock_minutes", LOCK_MINUTES)
                .bind("id", id)
                .bind("tenant_id", tenantId)
                .fetch()
                .rowsUpdated()
    ...
    private Mono<Void> requireMember(Long tenantId, Long principalId) {
        return databaseClient
                .sql("SELECT 1 FROM " + MEMBERSHIP + " WHERE tenant_id=:tenant_id AND principal_id=:principal_id"
                        + " AND membership_status='ACTIVE' AND deleted=0 LIMIT 1")
                .bind("tenant_id", tenantId)
                .bind("principal_id", principalId)
                .map((row, metadata) -> Boolean.TRUE)
                .one()
                .switchIfEmpty(Mono.error(new NotFoundException("Tenant membership does not exist")))
                .then();
    }
    ...
    private Mono<Void> updateRow(Long tenantId, LocalCredentialBO credential, Long operatorId, String operatorName) {
        ...
        DatabaseClient.GenericExecuteSpec update = databaseClient
                .sql(
                        "UPDATE " + TABLE
                                + " c SET login_name=:login_name,login_name_normalized=:login_name_normalized,password_hash=:password_hash,password_algorithm=:password_algorithm,password_params="
                                + dialect.jsonWriteExpression(":password_params")
                                + ",password_updated_time=:password_updated_time,password_expire_time=:password_expire_time,failed_attempts=:failed_attempts,locked_until=:locked_until,require_password_change=:require_password_change,enable_flag=:enable_flag,credential_ext="
                                + dialect.jsonWriteExpression(":credential_ext")
                                + ",remark=:remark,operator_id=:operator_id,operator_name=:operator_name,operate_time=:operate_time WHERE c.id=:id AND c.deleted=0 AND EXISTS (SELECT 1 FROM "
                                + MEMBERSHIP
                                + " m WHERE m.tenant_id=:tenant_id AND m.principal_id=c.principal_id AND m.membership_status='ACTIVE' AND m.deleted=0)")
                .bind("login_name", text(credential.getLoginName()))
                .bind("login_name_normalized", normalize(credential.getLoginName()))
                .bind("password_hash", credential.getPasswordHash())
                .bind("password_algorithm", credential.getPasswordAlgorithm().getValue())
                .bind("password_params", json(row.getPasswordParams()))
                .bind("password_updated_time", credential.getPasswordUpdatedTime())
                .bind("failed_attempts", credential.getFailedAttempts() == null ? 0 : credential.getFailedAttempts())
                .bind("require_password_change", flag(credential.getRequirePasswordChange()))
                .bind("enable_flag", flag(credential.getEnableFlag()))
                .bind("credential_ext", json(row.getCredentialExt()))
                .bind("remark", text(credential.getRemark()))
                .bind("operator_id", value(operatorId))
                .bind("operator_name", text(operatorName))
                .bind("operate_time", now())
                .bind("id", credential.getId())
                .bind("tenant_id", tenantId);
        ...
        return update.fetch()
                ...
                .flatMap(rows -> rows == 1
                        ? Mono.empty()
                        : Mono.error(new UpdateException("The local credential update failed")));
    }
    ...
    private Mono<LocalCredentialBO> prepareForCreate(LocalCredentialBO credential) {
        return encode(credential.getRawPassword()).map(hash -> {
            ...
            credential.setRequirePasswordChange(
                    credential.getRequirePasswordChange() == null
                            ? RequirePasswordChangeFlagEnum.REQUIRED
                            : credential.getRequirePasswordChange());
            credential.setEnableFlag(
                    credential.getEnableFlag() == null ? EnableFlagEnum.ENABLE : credential.getEnableFlag());
            return credential;
        });
    }
    ...
    private Mono<LocalCredentialBO> prepareForUpdate(LocalCredentialBO credential, LocalCredentialBO current) {
        ...
        credential.setCredentialType(CredentialTypeEnum.PASSWORD);
        credential.setLoginName(
                blank(credential.getLoginName())
                        ? current.getLoginName()
                        : credential.getLoginName().trim());
        credential.setLoginNameNormalized(normalize(credential.getLoginName()));
        credential.setPasswordParams(current.getPasswordParams());
        credential.setCredentialExt(
                credential.getCredentialExt() == null ? current.getCredentialExt() : credential.getCredentialExt());
        ...
        credential.setFailedAttempts(
                credential.getFailedAttempts() == null ? current.getFailedAttempts() : credential.getFailedAttempts());
        credential.setLockedUntil(current.getLockedUntil());
        credential.setRequirePasswordChange(
                credential.getRequirePasswordChange() == null
                        ? current.getRequirePasswordChange()
                        : credential.getRequirePasswordChange());
        credential.setEnableFlag(
                credential.getEnableFlag() == null ? current.getEnableFlag() : credential.getEnableFlag());
    ...
    private LocalDateTime expireAt() {
        return passwordExpireDays > 0 ? now().plusDays(passwordExpireDays) : null;
    }
    ...
    private byte flag(RequirePasswordChangeFlagEnum value) {
        return value == null ? RequirePasswordChangeFlagEnum.REQUIRED.getIndex() : value.getIndex();
    }
