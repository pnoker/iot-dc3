...
public class R2dbcTenantStore implements ReactiveTenantStore {
    ...
    private static final String COLUMNS = "id,tenant_name,tenant_code,tenant_ext,enable_flag,remark,creator_id,creator_name,"
    ...
    public Mono<TenantDO> getEnabledByCode(String code) {
        ...
        return databaseClient.sql("SELECT " + COLUMNS + " FROM " + TABLE
                ...
                .bind("tenant_code", code.trim()).map(this::map).one();
    }
    ...
    public Mono<TenantDO> getByNameAndCode(String tenantName, String tenantCode) {
        ...
        return databaseClient.sql("SELECT " + COLUMNS + " FROM " + TABLE
                ...
                .bind("tenant_name", tenantName.trim()).bind("tenant_code", tenantCode.trim())
                .map(this::map).one();
    }
    ...
    public Mono<OffsetPage<TenantDO>> list(TenantFilter filter) {
        ...
        DatabaseClient.GenericExecuteSpec count = databaseClient.sql("SELECT COUNT(*) AS total FROM " + TABLE + condition);
        DatabaseClient.GenericExecuteSpec rows = databaseClient.sql("SELECT " + COLUMNS + " FROM " + TABLE + condition
    ...
    public Mono<TenantDO> insert(TenantDO tenant) {
        ...
        DatabaseClient.GenericExecuteSpec insert = databaseClient.sql("INSERT INTO " + TABLE + " (" + COLUMNS
                        + ") VALUES (:id,:tenant_name,:tenant_code," + dialect.jsonWriteExpression(":tenant_ext")
                        + ",:enable_flag,:remark,:creator_id,:creator_name,:create_time,:operator_id,:operator_name,:operate_time,0)")
                .bind("id", tenant.getId()).bind("tenant_name", text(tenant.getTenantName()))
                .bind("tenant_code", text(tenant.getTenantCode())).bind("enable_flag", flag(tenant.getEnableFlag()))
                .bind("remark", text(tenant.getRemark())).bind("creator_id", value(tenant.getCreatorId()))
                .bind("creator_name", text(tenant.getCreatorName())).bind("create_time", now)
                .bind("operator_id", value(tenant.getOperatorId())).bind("operator_name", text(tenant.getOperatorName()))
        ...
        return transactionalOperator.transactional(insert.fetch().rowsUpdated()
                .flatMap(rows -> rows == 1 ? getById(tenant.getId()) : Mono.error(
                        new IllegalStateException("tenant insert affected " + rows + " rows"))));
    }

    @Override
    public Mono<TenantDO> update(TenantDO tenant) {
        if (tenant.getId() == null) return Mono.empty();
        DatabaseClient.GenericExecuteSpec update = databaseClient.sql("UPDATE " + TABLE + " SET tenant_name=:tenant_name,"
                ...
                .bind("tenant_name", text(tenant.getTenantName())).bind("tenant_code", text(tenant.getTenantCode()))
                .bind("enable_flag", flag(tenant.getEnableFlag())).bind("remark", text(tenant.getRemark()))
                .bind("operator_id", value(tenant.getOperatorId())).bind("operator_name", text(tenant.getOperatorName()))
                .bind("operate_time", LocalDateTime.now(ZoneOffset.UTC)).bind("id", tenant.getId());
        update = bindJson(update, "tenant_ext", tenant.getTenantExt());
        return transactionalOperator.transactional(update.fetch().rowsUpdated()
                .flatMap(rows -> rows == 1 ? getById(tenant.getId()) : Mono.empty()));
    }
    ...
    public Mono<Boolean> delete(Long id, Long operatorId, String operatorName) {
        ...
        return transactionalOperator.transactional(databaseClient.sql("UPDATE " + TABLE
                ...
                .bind("id", id).bind("operator_id", value(operatorId)).bind("operator_name", text(operatorName))
                .bind("operate_time", LocalDateTime.now(ZoneOffset.UTC)).fetch().rowsUpdated().map(rows -> rows == 1));
    }
    ...
    private DatabaseClient.GenericExecuteSpec bindJson(DatabaseClient.GenericExecuteSpec spec, String name, Object value) {
        String json = value == null ? "{}" : JsonUtil.toJsonString(value);
        return spec.bind(name, json);
    }

    private byte flag(Byte value) { return value == null ? EnableFlagEnum.ENABLE.getIndex() : value; }
