...
public class R2dbcMenuStore implements ReactiveMenuStore {
    ...
    public Flux<MenuDO> listTree(MenuFilter filter) {
        ...
        return bind(databaseClient.sql("SELECT " + COLUMNS + " FROM " + TABLE + whereClause(filter)
                                + " ORDER BY " + orderBy(filter.page().sort())), filter)
                .map(this::map).all();
    }
    ...
    public Mono<MenuDO> insert(MenuBO menu) {
        ...
        DatabaseClient.GenericExecuteSpec query = databaseClient.sql(sql)
                .bind("id", id).bind("parent_menu_id", value(menu.getParentMenuId()))
                .bind("menu_type_flag", index(menu.getMenuTypeFlag()))
                .bind("menu_name", text(menu.getMenuName())).bind("menu_code", text(menu.getMenuCode()))
                .bind("menu_level", index(menu.getMenuLevel())).bind("menu_index", value(menu.getMenuIndex()))
                .bind("menu_ext", json(menu.getMenuExt())).bind("enable_flag", index(menu.getEnableFlag()))
                .bind("remark", text(menu.getRemark())).bind("creator_id", value(menu.getCreatorId()))
                .bind("creator_name", text(menu.getCreatorName())).bind("create_time", now)
                .bind("operator_id", value(menu.getOperatorId())).bind("operator_name", text(menu.getOperatorName()))
    ...
    public Mono<MenuDO> update(MenuBO menu) {
        ...
        DatabaseClient.GenericExecuteSpec query = databaseClient.sql(sql)
                .bind("id", menu.getId()).bind("parent_menu_id", value(menu.getParentMenuId()))
                .bind("menu_type_flag", index(menu.getMenuTypeFlag())).bind("menu_name", text(menu.getMenuName()))
                .bind("menu_code", text(menu.getMenuCode())).bind("menu_level", index(menu.getMenuLevel()))
                .bind("menu_index", value(menu.getMenuIndex())).bind("menu_ext", json(menu.getMenuExt()))
                .bind("enable_flag", index(menu.getEnableFlag())).bind("remark", text(menu.getRemark()))
                .bind("operator_id", value(menu.getOperatorId())).bind("operator_name", text(menu.getOperatorName()))
                .bind("operate_time", now());
    ...
    public Mono<Boolean> delete(Long id, Long operatorId, String operatorName) {
        ...
        return transactionalOperator.transactional(databaseClient.sql("UPDATE " + TABLE
                        ...
                        .bind("id", id).bind("operator_id", value(operatorId))
    ...
    public Mono<Boolean> hasChildren(Long id) {
        ...
        return databaseClient.sql("SELECT 1 FROM " + TABLE
                        + " WHERE parent_menu_id=:id AND deleted=0 LIMIT 1")
                .bind("id", id).fetch().first().hasElement();
    }
    ...
    public Mono<Boolean> isDescendant(Long rootId, Long candidateId) {
        ...
        return databaseClient.sql(sql).bind("root_id", rootId).bind("candidate_id", candidateId)
                .fetch().first().hasElement();
    }
    ...
    private String whereClause(MenuFilter filter) {
        ...
        if (filter.menuName() != null) where.append(" AND ").append(dialect.caseInsensitiveLike("menu_name", ":menu_name"));
    ...
    private DatabaseClient.GenericExecuteSpec bind(DatabaseClient.GenericExecuteSpec spec, MenuFilter filter) {
        ...
        if (filter.menuTypeFlag() != null) spec = spec.bind("menu_type_flag", filter.menuTypeFlag().getIndex());
        ...
        if (sort != null) {
            for (SortSpec spec : sort) {
                String column = switch (spec.field()) {
                    ...
                    default -> throw new IllegalArgumentException("unsupported menu sort field: " + spec.field());
                };
    ...
    private MenuDO map(io.r2dbc.spi.Row row, io.r2dbc.spi.RowMetadata metadata) {
        ...
        menu.setId(row.get("id", Long.class)); menu.setParentMenuId(row.get("parent_menu_id", Long.class));
        menu.setMenuTypeFlag(number(row.get("menu_type_flag"))); menu.setMenuName(row.get("menu_name", String.class));
        menu.setMenuCode(row.get("menu_code", String.class)); menu.setMenuLevel(number(row.get("menu_level")));
        menu.setMenuIndex(number(row.get("menu_index"))); menu.setMenuExt(parseJson(row.get("menu_ext", String.class)));
        menu.setEnableFlag(number(row.get("enable_flag"))); menu.setRemark(row.get("remark", String.class));
        menu.setCreatorId(row.get("creator_id", Long.class)); menu.setCreatorName(row.get("creator_name", String.class));
        menu.setCreateTime(time(row.get("create_time"))); menu.setOperatorId(row.get("operator_id", Long.class));
        menu.setOperatorName(row.get("operator_name", String.class)); menu.setOperateTime(time(row.get("operate_time")));
    ...
    private short value(Integer value) { return value == null ? 0 : value.shortValue(); }
