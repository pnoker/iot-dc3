...
public class ReactiveProfileServiceImpl implements ReactiveProfileService {
    ...
    @Override
    public Mono<ProfileBO> add(ProfileBO value) {
        return Mono.defer(() -> {
            validate(value, false);
            value.setProfileCode(
                    value.getProfileCode() == null || value.getProfileCode().isBlank()
                            ? CodeUtil.getCode()
                            : value.getProfileCode().trim());
            return profileStore
                    .existsByName(value.getTenantId(), value.getProfileName(), null)
                    .flatMap(duplicate -> duplicate
                            ? Mono.<ProfileBO>error(new DuplicateException("Profile has been duplicated"))
                            : profileStore.insert(normalize(value, false)))
                    .onErrorMap(
                            DataIntegrityViolationException.class,
                            error -> new DuplicateException("Profile has been duplicated"));
        });
    }

    @Override
    public Mono<ProfileBO> update(ProfileBO value) {
        return Mono.defer(() -> {
            validate(value, true);
            return profileStore
                    ...
                    .flatMap(current -> profileStore
                            .existsByName(value.getTenantId(), value.getProfileName(), value.getId())
                            .flatMap(duplicate -> duplicate
                                    ? Mono.<ProfileBO>error(new DuplicateException("Profile has been duplicated"))
                                    : profileStore.update(normalize(value, true), value.getVersion())))
    ...
    public Mono<Boolean> delete(Long tenantId, Long id, int expectedVersion, Long operatorId, String operatorName) {
        if (tenantId == null || id == null)
            return Mono.error(new RequestException("Tenant ID and profile ID are required"));
        return profileStore
                .get(tenantId, id)
                .switchIfEmpty(Mono.error(new NotFoundException("Profile does not exist")))
                .flatMap(profile -> profileStore
                        .hasAssociations(tenantId, id)
                        .flatMap(associated -> associated
                                ? Mono.<Boolean>error(new RequestException("Profile is still referenced"))
                                : profileStore.delete(tenantId, id, expectedVersion, operatorId, operatorName)))
                .filter(Boolean.TRUE::equals)
                .switchIfEmpty(Mono.error(new ConflictException("Profile version conflict")));
    }

    @Override
    public Mono<ProfileBO> getById(Long tenantId, Long id) {
        if (tenantId == null || id == null)
            return Mono.error(new RequestException("Tenant ID and profile ID are required"));
        return profileStore
                .get(tenantId, id)
    ...
    private void validate(ProfileBO value, boolean update) {
        if (value == null
            ...
            throw new RequestException("Tenant ID and profile name are required");
        if (update && (value.getId() == null || value.getVersion() == null || value.getVersion() < 0))
            throw new RequestException("Profile ID and version are required for update");
        value.setProfileName(value.getProfileName().trim());
    }

    private ProfileBO normalize(ProfileBO value, boolean update) {
        value.setProfileShareFlag(
                value.getProfileShareFlag() == null ? ProfileShareTypeEnum.TENANT : value.getProfileShareFlag());
        value.setProfileTypeFlag(
                value.getProfileTypeFlag() == null ? ProfileTypeEnum.USER : value.getProfileTypeFlag());
