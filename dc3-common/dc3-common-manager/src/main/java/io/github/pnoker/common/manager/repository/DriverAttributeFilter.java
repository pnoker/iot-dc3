...
/** Tenant-scoped offset query for driver attributes. */
public record DriverAttributeFilter(
        Long tenantId,
        String attributeName,
        String attributeCode,
        AttributeTypeEnum attributeTypeFlag,
        Long driverId,
    ...
    /** Driver attribute filter compact constructor: normalizes the filter record. */
    public DriverAttributeFilter {
        ...
        if (sort.stream().anyMatch(spec -> spec == null || !SORT_FIELDS.contains(spec.field()))) {
            throw new IllegalArgumentException("unsupported driver attribute sort field");
        }
    }
}
