 ...
 * profile distribution) and the topology Sankey.
 *
 * <p>All methods return typed Row DTOs from {@code entity/bo/dashboard/};
 * {@code map-underscore-to-camel-case: true} in application.yml handles
 * the snake→camel column mapping automatically. SQL lives in
 * {@code resources/mapping/DashboardMapper.xml}.</p>
 *
 * @author pnoker
 * @since 2026.5.2
 */
...
public interface DashboardMapper {
     ...
     */
    List<BucketRow> countDeviceByDriver(@Param("tenantId") Long tenantId,
                                        @Param("limit") int limit);

    /**
     * Device counts grouped by {@code profile_id} (aliased to {@code key}). Top-N.
     */
    List<BucketRow> countDeviceByProfile(@Param("tenantId") Long tenantId,
                                         @Param("limit") int limit);

    /**
     * Daily new-row count for the given entity table. {@code table} is
     * interpolated (service layer whitelists it). Missing days are padded
     * with zero service-side so the frontend always gets a fixed-length array.
     */
    List<DailyGrowthRow> dailyGrowth(@Param("tenantId") Long tenantId,
     ...
     * queried against {@code dc3_history.v_dc3_point_value_all} (cross-schema).
     */
    List<PointVolumeRow> topologyPointVolumes(@Param("tenantId") Long tenantId,
                                              @Param("from") LocalDateTime from);
}
