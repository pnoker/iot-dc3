...
public class ReactiveDriverAttributeServiceImpl implements ReactiveDriverAttributeService {
    ...
    public Mono<DriverAttributeBO> add(DriverAttributeBO value) {
        return Mono.defer(() -> {
            ...
            return ensureDriver(value).then(store.getByCodeAndDriver(value.getTenantId(), value.getAttributeCode(), value.getDriverId()))
                    .flatMap(existing -> Mono.<DriverAttributeBO>error(new DuplicateException("Command attribute has been duplicated")))
    ...
    @Override public Flux<DriverAttributeBO> listByDriverId(Long tenantId, Long driverId) { return store.listByDriverId(tenantId, driverId); }
    @Override public Mono<OffsetPage<DriverAttributeBO>> list(DriverAttributeFilter filter) { return store.list(filter); }
