...
public class ReactiveLabelServiceImpl implements ReactiveLabelService {
    private static final Set<EntityTypeEnum> SUPPORTED = Set.of(EntityTypeEnum.DRIVER, EntityTypeEnum.PROFILE,
            EntityTypeEnum.POINT, EntityTypeEnum.DEVICE);
    ...
    public Mono<LabelBO> add(LabelBO label) {
        return Mono.defer(() -> {
            ...
            return labelStore.getByName(label.getTenantId(), label.getLabelName(), label.getEntityTypeFlag().getIndex())
    ...
    public Mono<LabelBO> update(LabelBO label) {
        return Mono.defer(() -> {
            validate(label, true);
            return labelStore.get(label.getTenantId(), label.getId())
                    .switchIfEmpty(Mono.error(new NotFoundException("Label does not exist")))
                    .flatMap(current -> labelStore.getByName(label.getTenantId(), label.getLabelName(), label.getEntityTypeFlag().getIndex())
                            .filter(existing -> !existing.getId().equals(label.getId()))
                            .flatMap(existing -> Mono.<LabelBO>error(new DuplicateException("Label has been duplicated")))
                            .switchIfEmpty(Mono.defer(() -> {
                                if (label.getLabelCode() == null || label.getLabelCode().isBlank()) label.setLabelCode(current.getLabelCode());
                                if (label.getLabelColor() == null || label.getLabelColor().isBlank()) label.setLabelColor(current.getLabelColor());
    ...
    public Mono<Boolean> delete(Long tenantId, Long id, Long operatorId, String operatorName) {
        if (tenantId == null || id == null) return Mono.error(new RequestException("Tenant ID and label ID are required"));
        return labelStore.get(tenantId, id).switchIfEmpty(Mono.error(new NotFoundException("Label does not exist")))
                ...
                .flatMap(bound -> bound ? Mono.error(new AssociatedException("The label has been bound by another entity"))
                        : labelStore.delete(tenantId, id, operatorId, operatorName))
                .filter(Boolean.TRUE::equals).switchIfEmpty(Mono.error(new RequestException("Label was already deleted")));
    }
    ...
    private void validate(LabelBO label, boolean update) {
        if (label == null || label.getTenantId() == null || label.getTenantId() <= 0
                || label.getLabelName() == null || label.getLabelName().isBlank()
                || label.getEntityTypeFlag() == null || !SUPPORTED.contains(label.getEntityTypeFlag())
