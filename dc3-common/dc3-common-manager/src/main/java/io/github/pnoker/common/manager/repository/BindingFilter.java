...
/** Immutable tenant-scoped polymorphic binding filter. */
public record BindingFilter(
    ...
    public BindingFilter {
        ...
        if (sort.stream().anyMatch(v -> v == null || !SORT.contains(v.field())))
            throw new IllegalArgumentException("unsupported binding sort field");
    }
}
