...
public class R2dbcDriverLeaseStore implements ReactiveDriverLeaseStore {
    ...
    public Mono<Void> acquireDriverLock(Long tenantId, Long driverId) {
        ...
        return databaseClient.sql("SELECT id FROM " + DRIVER_TABLE
    ...
    @Override
    public Mono<Void> renewInstance(Long tenantId, Long driverId, String node, String client,
        ...
        return databaseClient.sql(sql)
                .bind("tenant_id", tenantId).bind("driver_id", driverId).bind("node_id", node)
                .bind("client_id", client).bind("service_host", host).bind("lease_until", leaseUntil)
    ...
    public Flux<String> listActiveNodes(Long tenantId, Long driverId) {
        ...
        return databaseClient.sql("SELECT node_id FROM " + INSTANCE_TABLE
                        + " WHERE tenant_id=:tenant_id AND driver_id=:driver_id AND lease_until > CURRENT_TIMESTAMP "
                        + "ORDER BY node_id ASC")
                .bind("tenant_id", tenantId).bind("driver_id", driverId)
                .map((row, metadata) -> row.get("node_id", String.class)).all();
    }
    ...
    public Flux<Long> listDriverDeviceIds(Long tenantId, Long driverId, long afterDeviceId, int limit) {
        ...
        return databaseClient.sql("SELECT id FROM " + DEVICE_TABLE
                ...
                .bind("tenant_id", tenantId).bind("driver_id", driverId)
                .bind("after_device_id", afterDeviceId).bind("limit", limit)
    ...
    public Mono<DriverLeaseStateDO> getLeaseState(Long tenantId, Long driverId) {
        ...
        return databaseClient.sql("SELECT tenant_id, driver_id, membership_hash, device_revision, assignment_version "
                        + "FROM " + STATE_TABLE + " WHERE tenant_id=:tenant_id AND driver_id=:driver_id LIMIT 1")
                .bind("tenant_id", tenantId).bind("driver_id", driverId)
                .map((row, metadata) -> {
    ...
    public Mono<Long> getDeviceRevision(Long tenantId, Long driverId) {
        ...
        return databaseClient.sql("SELECT revision FROM " + REVISION_TABLE
                        + " WHERE tenant_id=:tenant_id AND driver_id=:driver_id LIMIT 1")
                .bind("tenant_id", tenantId).bind("driver_id", driverId)
                .map((row, metadata) -> row.get("revision", Long.class)).one()
                .defaultIfEmpty(0L);
    }

    @Override
    public Mono<Long> advanceAssignmentVersion(Long tenantId, Long driverId,
                                               String membershipHash, long deviceRevision) {
        ...
        String sql = "INSERT INTO " + STATE_TABLE
                        ...
                        + " device_revision=EXCLUDED.device_revision, assignment_version=" + STATE_TABLE + ".assignment_version + 1,"
        ...
        DatabaseClient.GenericExecuteSpec statement = databaseClient.sql(sql)
                .bind("tenant_id", tenantId).bind("driver_id", driverId)
                .bind("membership_hash", membershipHash).bind("device_revision", deviceRevision);
        if (postgres()) {
            return statement.map((row, metadata) -> row.get("assignment_version", Long.class)).one();
        }
        return transactionalOperator.transactional(statement.fetch().rowsUpdated()
                .then(databaseClient.sql("SELECT assignment_version FROM " + STATE_TABLE
                                + " WHERE tenant_id=:tenant_id AND driver_id=:driver_id LIMIT 1")
                        .bind("tenant_id", tenantId).bind("driver_id", driverId)
                        .map((row, metadata) -> row.get("assignment_version", Long.class)).one()));
    }
    ...
    public Mono<Void> deleteExpiredInstances(Long tenantId, Long driverId, Instant expiredBefore) {
        ...
        return databaseClient.sql("DELETE FROM " + INSTANCE_TABLE
                        + " WHERE tenant_id=:tenant_id AND driver_id=:driver_id AND lease_until < :expired_before")
                .bind("tenant_id", tenantId).bind("driver_id", driverId).bind("expired_before", expiredBefore)
                .fetch().rowsUpdated().then();
    }
    ...
    public Mono<Void> reconcileDeviceLeases(List<DeviceLeaseDO> leases) {
        ...
        StringBuilder sql = new StringBuilder("INSERT INTO ").append(LEASE_TABLE)
        ...
        for (int index = 0; index < leases.size(); index++) {
            ...
            sql.append(" (:tenant_").append(index).append(", :driver_").append(index)
                    .append(", :device_").append(index).append(", :owner_").append(index).append(')');
        ...
        if (postgres()) {
            sql.append(" ON CONFLICT (tenant_id, device_id) DO UPDATE SET ")
                    .append("fencing_token = CASE WHEN ").append(LEASE_TABLE).append(".driver_id <> EXCLUDED.driver_id "
                            + "OR ").append(LEASE_TABLE).append(".owner_node <> EXCLUDED.owner_node THEN ")
                    .append(LEASE_TABLE).append(".fencing_token + 1 ELSE ").append(LEASE_TABLE).append(".fencing_token END, ")
                    .append("operate_time = CASE WHEN ").append(LEASE_TABLE).append(".driver_id <> EXCLUDED.driver_id "
                            + "OR ").append(LEASE_TABLE).append(".owner_node <> EXCLUDED.owner_node THEN CURRENT_TIMESTAMP ELSE ")
                    .append(LEASE_TABLE).append(".operate_time END, driver_id=EXCLUDED.driver_id, owner_node=EXCLUDED.owner_node");
        } else {
            sql.append(" ON DUPLICATE KEY UPDATE fencing_token = IF(driver_id <> VALUES(driver_id) OR owner_node <> VALUES(owner_node),"
    ...
    public Mono<Void> deleteOrphanedLeases(Long tenantId, Long driverId) {
        if (!valid(tenantId) || !valid(driverId)) return Mono.empty();
        return databaseClient.sql("DELETE FROM " + LEASE_TABLE + " lease WHERE lease.tenant_id=:tenant_id "
                        ...
                        + "AND device.deleted=0 AND device.enable_flag=0) OR NOT EXISTS (SELECT 1 FROM " + INSTANCE_TABLE + " instance "
    ...
    @Override
    public Flux<DeviceLeaseDO> listOwnedLeases(Long tenantId, Long driverId, String node,
                                               long afterDeviceId, int limit) {
        if (!valid(tenantId) || !valid(driverId) || blank(node) || afterDeviceId < 0 || limit < 1) return Flux.empty();
        return databaseClient.sql("SELECT lease.tenant_id, lease.driver_id, lease.device_id, lease.owner_node, lease.fencing_token "
                        + "FROM " + LEASE_TABLE + " lease JOIN " + INSTANCE_TABLE + " instance ON instance.tenant_id=lease.tenant_id "
                ...
                .bind("tenant_id", tenantId).bind("driver_id", driverId).bind("owner_node", node)
                .bind("after_device_id", afterDeviceId).bind("limit", limit)
                .map(this::mapLease).all();
    }
    ...
    public Mono<DeviceLeaseDO> getActiveLease(Long tenantId, Long deviceId) {
        ...
        return databaseClient.sql("SELECT lease.tenant_id, lease.driver_id, lease.device_id, lease.owner_node, lease.fencing_token "
                ...
                .map(this::mapLease).one();
    }

    private DeviceLeaseDO mapLease(io.r2dbc.spi.Row row, io.r2dbc.spi.RowMetadata metadata) {
        return new DeviceLeaseDO(row.get("tenant_id", Long.class), row.get("driver_id", Long.class),
                row.get("device_id", Long.class), row.get("owner_node", String.class),
