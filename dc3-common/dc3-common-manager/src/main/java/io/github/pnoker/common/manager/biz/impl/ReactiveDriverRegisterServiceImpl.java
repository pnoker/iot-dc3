...
public class ReactiveDriverRegisterServiceImpl implements ReactiveDriverRegisterService {
    ...
    public Mono<Registration> register(GrpcDriverRegisterDTO request) {
        ...
        return tenantFacade.getByCode(request.getTenant())
    ...
    private Mono<Registration> reconcileDriver(DriverBO incoming, GrpcDriverRegisterDTO request) {
        return driverService.getByServiceName(incoming.getTenantId(), incoming.getServiceName())
                        ...
                        .flatMap(driverAttributes -> reconcilePointAttributes(request, driver)
                                .flatMap(pointAttributes -> reconcileCommandAttributes(request, driver)
                                        .flatMap(commandAttributes -> reconcileEventAttributes(request, driver)
                                                .map(eventAttributes -> new Registration(driver, driverAttributes,
                                                        pointAttributes, commandAttributes, eventAttributes))))));
    }
    ...
    private Mono<List<DriverAttributeBO>> reconcileDriverAttributes(GrpcDriverRegisterDTO request, DriverBO driver) {
        Map<String, DriverAttributeBO> incoming = request.getDriverAttributesList().stream()
                .collect(Collectors.toMap(value -> value.getAttributeCode(), grpcDriverAttributeBuilder::buildBOByGrpcDTO));
        return driverAttributeService.listByDriverId(driver.getTenantId(), driver.getId()).collectList()
                .flatMap(existing -> reconcile(incoming, existing, driver,
                        DriverAttributeBO::getAttributeCode,
                        attribute -> { attribute.setDriverId(driver.getId()); attribute.setTenantId(driver.getTenantId()); },
                        driverAttributeService::saveBatch, driverAttributeService::updateBatch,
                        ids -> driverAttributeService.deleteByIds(driver.getTenantId(), ids, null, null),
                        () -> driverAttributeService.listByDriverId(driver.getTenantId(), driver.getId()).collectList()));
    }
    ...
    private Mono<List<PointAttributeBO>> reconcilePointAttributes(GrpcDriverRegisterDTO request, DriverBO driver) {
        Map<String, PointAttributeBO> incoming = request.getPointAttributesList().stream()
                .collect(Collectors.toMap(value -> value.getAttributeCode(), grpcPointAttributeBuilder::buildBOByGrpcDTO));
        return pointAttributeService.listByDriverId(driver.getTenantId(), driver.getId()).collectList()
                .flatMap(existing -> reconcile(incoming, existing, driver,
                        PointAttributeBO::getAttributeCode,
                        attribute -> { attribute.setDriverId(driver.getId()); attribute.setTenantId(driver.getTenantId()); },
                        pointAttributeService::saveBatch, pointAttributeService::updateBatch,
                        ids -> pointAttributeService.deleteByIds(driver.getTenantId(), ids, null, null),
                        () -> pointAttributeService.listByDriverId(driver.getTenantId(), driver.getId()).collectList()));
    }
    ...
    private Mono<List<CommandAttributeBO>> reconcileCommandAttributes(GrpcDriverRegisterDTO request, DriverBO driver) {
        Map<String, CommandAttributeBO> incoming = request.getCommandAttributesList().stream()
                .collect(Collectors.toMap(value -> value.getAttributeCode(), grpcCommandAttributeBuilder::buildBOByGrpcDTO));
        return commandAttributeService.listByDriverId(driver.getTenantId(), driver.getId()).collectList()
                .flatMap(existing -> reconcile(incoming, existing, driver,
                        CommandAttributeBO::getAttributeCode,
                        attribute -> { attribute.setDriverId(driver.getId()); attribute.setTenantId(driver.getTenantId()); },
                        commandAttributeService::saveBatch, commandAttributeService::updateBatch,
                        ids -> commandAttributeService.deleteByIds(driver.getTenantId(), ids, null, null),
                        () -> commandAttributeService.listByDriverId(driver.getTenantId(), driver.getId()).collectList()));
    }
    ...
    private Mono<List<EventAttributeBO>> reconcileEventAttributes(GrpcDriverRegisterDTO request, DriverBO driver) {
        Map<String, EventAttributeBO> incoming = request.getEventAttributesList().stream()
                .collect(Collectors.toMap(value -> value.getAttributeCode(), grpcEventAttributeBuilder::buildBOByGrpcDTO));
        return eventAttributeService.listByDriverId(driver.getTenantId(), driver.getId()).collectList()
                .flatMap(existing -> reconcile(incoming, existing, driver,
                        EventAttributeBO::getAttributeCode,
                        attribute -> { attribute.setDriverId(driver.getId()); attribute.setTenantId(driver.getTenantId()); },
                        eventAttributeService::saveBatch, eventAttributeService::updateBatch,
                        ids -> eventAttributeService.deleteByIds(driver.getTenantId(), ids, null, null),
                        () -> eventAttributeService.listByDriverId(driver.getTenantId(), driver.getId()).collectList()));
    }

    private <T> Mono<List<T>> reconcile(Map<String, T> incoming, List<T> existing, DriverBO driver,
                                         Function<T, String> key, java.util.function.Consumer<T> normalize,
        ...
        Set<Long> removed = current.entrySet().stream()
                ...
                .map(entry -> id(entry.getValue())).filter(Objects::nonNull)
                .collect(Collectors.toCollection(HashSet::new));
        return save.apply(inserts).then(update.apply(updates)).then(delete.apply(removed)).then(reload.get());
    }
    ...
    private void setIdAndVersion(Object value, Object old) {
        ...
        if (value instanceof DriverAttributeBO target && old instanceof DriverAttributeBO source && target.getVersion() == null) target.setVersion(source.getVersion());
        if (value instanceof PointAttributeBO target && old instanceof PointAttributeBO source && target.getVersion() == null) target.setVersion(source.getVersion());
        if (value instanceof CommandAttributeBO target && old instanceof CommandAttributeBO source && target.getVersion() == null) target.setVersion(source.getVersion());
        if (value instanceof EventAttributeBO target && old instanceof EventAttributeBO source && target.getVersion() == null) target.setVersion(source.getVersion());
    }
}
