...
public class ManagerDeviceServer extends DeviceApiGrpc.DeviceApiImplBase {
    ...
    @Autowired
    public ManagerDeviceServer(GrpcDeviceBuilder grpcDeviceBuilder,
    ...
    public void list(GrpcOffsetDeviceQuery request, StreamObserver<GrpcOffsetPageDeviceDTO> observer) {
        ReactiveGrpcServerSupport.subscribe(reactiveDeviceService.list(offsetFilter(request))
                .map(page -> offsetResponse(page)), observer);
    }

    @Override
    public void getActiveOwner(GrpcDeviceQuery request, StreamObserver<GrpcDeviceOwnerDTO> observer) {
        ReactiveGrpcServerSupport.subscribe(driverLeaseService.getActiveOwner(request.getTenantId(), request.getDeviceId())
                .map(owner -> GrpcDeviceOwnerDTO.newBuilder()
                        .setDriverId(owner.driverId()).setOwnerNode(owner.ownerNode()).setFencingToken(owner.fencingToken()).build())
                .switchIfEmpty(Mono.error(new NotFoundException("device owner does not exist"))), observer);
    }
    ...
    public void listByDeviceIds(GrpcDeviceIdsQuery request, StreamObserver<GrpcDeviceListDTO> observer) {
        if (request.getDeviceIdsList().stream().anyMatch(id -> id <= 0)) {
            ReactiveGrpcServerSupport.subscribe(Mono.error(new IllegalArgumentException("device ids must be positive")), observer);
            return;
        }
    ...
    public void getByDeviceId(GrpcDeviceQuery request, StreamObserver<GrpcDeviceDTO> observer) {
        ReactiveGrpcServerSupport.subscribe(reactiveDeviceService.getById(request.getTenantId(), request.getDeviceId())
                .map(grpcDeviceBuilder::buildGrpcDTOByBO)
                .switchIfEmpty(Mono.error(new NotFoundException("device does not exist"))), observer);
    }
    ...
    private GrpcOffsetPageDeviceDTO offsetResponse(OffsetPage<DeviceBO> page) {
        return GrpcOffsetPageDeviceDTO.newBuilder()
                .setPage(io.github.pnoker.api.common.OffsetPage.newBuilder().setOffset(page.offset()).setLimit(page.limit())
                        .setTotal(page.total()).setHasNext(page.hasNext()))
                .addAllItems(page.items().stream().map(grpcDeviceBuilder::buildGrpcDTOByBO).toList()).build();
    }

    private void listResponse(Flux<DeviceBO> values, StreamObserver<GrpcDeviceListDTO> observer) {
        ReactiveGrpcServerSupport.subscribe(values.map(grpcDeviceBuilder::buildGrpcDTOByBO).collectList()
                .map(devices -> GrpcDeviceListDTO.newBuilder().addAllItems(devices).build()), observer);
    }
