...
public class R2dbcEventAttributeStore implements ReactiveEventAttributeStore {
    ...
    private DatabaseClient.GenericExecuteSpec bind(DatabaseClient.GenericExecuteSpec spec, EventAttributeFilter filter) {
    ...
    private String orderBy(List<SortSpec> sort) {
        ...
        if (sort != null) for (SortSpec spec : sort) {
            String column = switch (spec.field()) {
                ...
                case "version" -> "a.version"; default -> throw new IllegalArgumentException("unsupported event attribute sort field");
            };
    ...
    private EventAttributeBO map(io.r2dbc.spi.Row row, io.r2dbc.spi.RowMetadata metadata) {
        ...
        if (raw != null) try {
            ...
            EventAttributeExt ext = new EventAttributeExt(); ext.setType(json.getType()); ext.setVersion(json.getVersion()); ext.setRemark(json.getRemark());
            ext.setContent(json.getContent() == null ? null : JsonUtil.parseObject(json.getContent(), EventAttributeExt.Content.class));
