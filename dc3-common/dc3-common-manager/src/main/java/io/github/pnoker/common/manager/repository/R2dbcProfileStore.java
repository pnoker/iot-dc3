...
public class R2dbcProfileStore implements ReactiveProfileStore {
    ...
    @Override public Mono<Boolean> existsByName(Long tenantId, String profileName, Long excludingId) {
        if (tenantId == null || profileName == null || profileName.isBlank()) return Mono.just(false);
        String sql = "SELECT 1 FROM " + TABLE + " WHERE tenant_id=:tenant_id AND profile_name=:profile_name AND deleted=0" + (excludingId == null ? "" : " AND id<>:excluding_id") + " LIMIT 1";
        DatabaseClient.GenericExecuteSpec query = databaseClient.sql(sql).bind("tenant_id", tenantId).bind("profile_name", profileName.trim());
        if (excludingId != null) query = query.bind("excluding_id", excludingId);
        return query.map((row, metadata) -> true).one().defaultIfEmpty(false);
    }

    @Override public Mono<Boolean> hasAssociations(Long tenantId, Long id) {
        if (tenantId == null || id == null) return Mono.just(false);
        return databaseClient.sql("SELECT (EXISTS (SELECT 1 FROM dc3_manager.dc3_device WHERE tenant_id=:tenant_id AND profile_id=:id AND deleted=0) OR EXISTS (SELECT 1 FROM dc3_manager.dc3_point WHERE tenant_id=:tenant_id AND profile_id=:id AND deleted=0) OR EXISTS (SELECT 1 FROM dc3_manager.dc3_command WHERE tenant_id=:tenant_id AND profile_id=:id AND deleted=0) OR EXISTS (SELECT 1 FROM dc3_manager.dc3_event WHERE tenant_id=:tenant_id AND profile_id=:id AND deleted=0)) AS associated")
                .bind("tenant_id", tenantId).bind("id", id).map((row, metadata) -> Boolean.TRUE.equals(row.get("associated", Boolean.class))).one().defaultIfEmpty(false);
    }

    @Override public Mono<ProfileBO> insert(ProfileBO value) {
        if (value == null || value.getTenantId() == null || value.getProfileName() == null || value.getProfileName().isBlank()) return Mono.error(new IllegalArgumentException("tenantId and profileName are required"));
        if (value.getId() == null) value.setId(UuidV7.nextLong());
        if (value.getVersion() == null) value.setVersion(0);
        LocalDateTime now = LocalDateTime.now(ZoneOffset.UTC);
        DatabaseClient.GenericExecuteSpec query = databaseClient.sql("INSERT INTO " + TABLE + " (id,profile_name,profile_code,profile_share_flag,profile_type_flag,profile_ext,enable_flag,tenant_id,remark,signature,version,creator_id,creator_name,create_time,operator_id,operator_name,operate_time,deleted) VALUES (:id,:profile_name,:profile_code,:profile_share_flag,:profile_type_flag," + dialect.jsonWriteExpression(":profile_ext") + ",:enable_flag,:tenant_id,:remark,:signature,:version,:creator_id,:creator_name,:create_time,:operator_id,:operator_name,:operate_time,0)")
                .bind("id", value.getId()).bind("profile_name", value.getProfileName().trim()).bind("profile_code", value.getProfileCode() == null ? "" : value.getProfileCode())
                .bind("profile_share_flag", index(value.getProfileShareFlag())).bind("profile_type_flag", index(value.getProfileTypeFlag())).bind("profile_ext", serialize(value.getProfileExt())).bind("enable_flag", index(value.getEnableFlag())).bind("tenant_id", value.getTenantId()).bind("remark", value.getRemark() == null ? "" : value.getRemark()).bind("signature", value.getSignature() == null ? "" : value.getSignature()).bind("version", value.getVersion()).bind("creator_id", value.getCreatorId() == null ? 0L : value.getCreatorId()).bind("creator_name", value.getCreatorName() == null ? "" : value.getCreatorName()).bind("create_time", value.getCreateTime() == null ? now : value.getCreateTime()).bind("operator_id", value.getOperatorId() == null ? 0L : value.getOperatorId()).bind("operator_name", value.getOperatorName() == null ? "" : value.getOperatorName()).bind("operate_time", value.getOperateTime() == null ? now : value.getOperateTime());
        return transactionalOperator.transactional(query.fetch().rowsUpdated().flatMap(rows -> rows == 1 ? get(value.getTenantId(), value.getId()) : Mono.error(new IllegalStateException("profile insert affected " + rows + " rows"))));
    }

    @Override public Mono<ProfileBO> update(ProfileBO value, int expectedVersion) {
        if (value == null || value.getTenantId() == null || value.getId() == null) return Mono.error(new IllegalArgumentException("tenantId and profile id are required"));
        DatabaseClient.GenericExecuteSpec query = databaseClient.sql("UPDATE " + TABLE + " SET profile_name=:profile_name,profile_share_flag=:profile_share_flag,profile_type_flag=:profile_type_flag,profile_ext=" + dialect.jsonWriteExpression(":profile_ext") + ",enable_flag=:enable_flag,remark=:remark,signature=:signature,version=version+1,operator_id=:operator_id,operator_name=:operator_name,operate_time=:operate_time WHERE id=:id AND tenant_id=:tenant_id AND version=:expected_version AND deleted=0")
                .bind("profile_name", value.getProfileName().trim()).bind("profile_share_flag", index(value.getProfileShareFlag())).bind("profile_type_flag", index(value.getProfileTypeFlag())).bind("profile_ext", serialize(value.getProfileExt())).bind("enable_flag", index(value.getEnableFlag())).bind("remark", value.getRemark() == null ? "" : value.getRemark()).bind("signature", value.getSignature() == null ? "" : value.getSignature()).bind("operator_id", value.getOperatorId() == null ? 0L : value.getOperatorId()).bind("operator_name", value.getOperatorName() == null ? "" : value.getOperatorName()).bind("operate_time", LocalDateTime.now(ZoneOffset.UTC)).bind("id", value.getId()).bind("tenant_id", value.getTenantId()).bind("expected_version", expectedVersion);
    ...
    public Mono<ProfileBO> get(Long tenantId, Long id) {
        ...
        return databaseClient.sql("SELECT " + COLUMNS + " FROM " + TABLE + " p"
    ...
    public Mono<ProfileBO> getByNameAndType(Long tenantId, String name, ProfileTypeEnum type) {
        ...
        return databaseClient.sql("SELECT " + COLUMNS + " FROM " + TABLE + " p"
                ...
                .bind("tenant_id", tenantId).bind("profile_name", name.trim())
                .bind("profile_type", type.getIndex()).map(this::map).one();
    }
    ...
    public Flux<ProfileBO> listByIds(Long tenantId, List<Long> ids) {
        ...
        String placeholders = java.util.stream.IntStream.range(0, distinctIds.size())
                .mapToObj(index -> ":id_" + index).reduce((left, right) -> left + "," + right).orElseThrow();
        DatabaseClient.GenericExecuteSpec spec = databaseClient.sql("SELECT " + COLUMNS + " FROM " + TABLE + " p"
                        + " WHERE p.tenant_id=:tenant_id AND p.deleted=0 AND p.id IN (" + placeholders + ")"
                        + " ORDER BY p.id ASC").bind("tenant_id", tenantId);
    ...
    public Mono<OffsetPage<ProfileBO>> list(ProfileFilter filter) {
        ...
        DatabaseClient.GenericExecuteSpec countSpec = databaseClient.sql("SELECT COUNT(*) FROM " + TABLE + " p" + where);
        ...
        return total.flatMap(count -> dataQuery.map(this::map).all().collectList()
    ...
    private DatabaseClient.GenericExecuteSpec bind(DatabaseClient.GenericExecuteSpec spec, ProfileFilter filter) {
        ...
        if (present(filter.profileName())) spec = spec.bind("profile_name", "%" + filter.profileName().trim() + "%");
        if (present(filter.profileCode())) spec = spec.bind("profile_code", filter.profileCode().trim());
        if (filter.profileShareFlag() != null) spec = spec.bind("profile_share_flag", filter.profileShareFlag().getIndex());
        if (filter.profileTypeFlag() != null) spec = spec.bind("profile_type_flag", filter.profileTypeFlag().getIndex());
        ...
        if (sort != null) {
            for (SortSpec spec : sort) {
                String column = switch (spec.field()) {
                    ...
                    case "id" -> "p.id";
                    default -> throw new IllegalArgumentException("unsupported profile sort field: " + spec.field());
                };
    ...
    private ProfileExt profileExt(String raw) {
        ...
        try {
            ...
            ext.setContent(json.getContent() == null ? null : JsonUtil.parseObject(json.getContent(), ProfileExt.Content.class));
            return ext;
    ...
    private Byte index(ProfileShareTypeEnum value) { return value == null ? ProfileShareTypeEnum.TENANT.getIndex() : value.getIndex(); }
    private Byte index(ProfileTypeEnum value) { return value == null ? ProfileTypeEnum.USER.getIndex() : value.getIndex(); }
    ...
    private String serialize(ProfileExt value) {
        ...
        } catch (Exception exception) { throw new IllegalArgumentException("profile_ext is not valid JSON", exception); }
    }
}
