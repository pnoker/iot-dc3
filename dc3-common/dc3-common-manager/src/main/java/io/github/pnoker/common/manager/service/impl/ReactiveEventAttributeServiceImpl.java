...
public class ReactiveEventAttributeServiceImpl implements ReactiveEventAttributeService {
    ...
    public Mono<EventAttributeBO> add(EventAttributeBO value) {
        return Mono.defer(() -> {
            ...
            return ensureDriver(value).then(store.getByCodeAndDriver(value.getTenantId(), value.getAttributeCode(), value.getDriverId()))
                    .flatMap(existing -> Mono.<EventAttributeBO>error(new DuplicateException("Command attribute has been duplicated")))
    ...
    @Override public Flux<EventAttributeBO> listByDriverId(Long tenantId, Long driverId) { return store.listByDriverId(tenantId, driverId); }
    @Override public Mono<OffsetPage<EventAttributeBO>> list(EventAttributeFilter filter) { return store.list(filter); }
