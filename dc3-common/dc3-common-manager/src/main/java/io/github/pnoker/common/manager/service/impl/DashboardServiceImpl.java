...
public class DashboardServiceImpl implements DashboardService {
    ...
    private static String enableKey(Object raw) {
        if (raw == null) return TopologyLimits.UNKNOWN_BUCKET;
    ...
    /**
     * Pads a sparse (day, count) row set into a fixed-length series ending
     * on {@code today}.
     */
    ...
    public DriverStatsVO driverStats(Long tenantId) {
        ...
        List<BucketVO> byEnable = buckets(dashboardMapper.countDriverByEnable(tenantId), DashboardServiceImpl::enableKey);
        List<BucketVO> byType = buckets(dashboardMapper.countDriverByType(tenantId), DashboardServiceImpl::driverTypeKey);
    ...
    public DeviceStatsVO deviceStats(Long tenantId, int topN) {
        ...
        List<BucketVO> byEnable = buckets(dashboardMapper.countDeviceByEnable(tenantId), DashboardServiceImpl::enableKey);
    ...
    private TopologyVO computeTopology(Long tenantId, String mode, String rangeKey) {
        ...
        List<ProfileBindingRow> bindingRows = allDeviceIds.isEmpty()
        ...
        List<TopologyProfileRow> profileRows = allProfileIdSet.isEmpty()
                ? Collections.emptyList()
        ...
        List<TopologyPointRow> pointRows = allProfileIdSet.isEmpty()
                ? Collections.emptyList()
