...
public class R2dbcDeviceImportJobStore implements ReactiveDeviceImportJobStore {
    ...
    public Mono<Void> insert(DeviceImportJob job) {
        ...
        return transactionalOperator.transactional(databaseClient.sql("INSERT INTO " + TABLE + " (" + COLUMNS
                ...
                .bind("operation_id", uuid(job.operationId())).bind("tenant_id", job.tenantId())
                .bind("driver_id", job.driverId()).bind("profile_id", job.profileId())
                .bind("operator_id", job.operatorId() == null ? 0L : job.operatorId())
                .bind("operator_name", value(job.operatorName())).bind("file_name", value(job.fileName()))
                .bind("file_data", job.content()).bind("claimed_by", value(job.claimedBy()))
                .bindNull("claimed_until", Object.class).bind("attempts", job.attempts())
                .bind("created_at", dialect.bindInstant(Instant.now())).fetch().rowsUpdated()
                .flatMap(rows -> rows == 1 ? Mono.empty()
                        : Mono.error(new IllegalStateException("device import job insert affected " + rows + " rows"))));
    }
    ...
    public Mono<DeviceImportJob> claim(UUID operationId, String workerId, Instant now, Instant claimedUntil) {
        ...
        return transactionalOperator.transactional(databaseClient.sql("UPDATE " + TABLE
                ...
                .bind("worker_id", workerId).bind("claimed_until", dialect.bindInstant(claimedUntil))
                .bind("operation_id", uuid(operationId)).bind("now", dialect.bindInstant(now))
                .fetch().rowsUpdated().flatMap(rows -> rows == 1 ? get(operationId) : Mono.empty()));
    }

    @Override
    public Flux<UUID> listRecoverable(Instant now) {
        return databaseClient.sql("SELECT operation_id FROM " + TABLE
                        + " WHERE claimed_until IS NULL OR claimed_until<:now ORDER BY created_at, operation_id")
                .bind("now", dialect.bindInstant(now)).map((row, metadata) -> readUuid(row.get("operation_id"))).all();
    }
    ...
    public Mono<Boolean> renew(UUID operationId, String workerId, Instant claimedUntil) {
        ...
        return databaseClient.sql("UPDATE " + TABLE + " SET claimed_until=:claimed_until"
                        + " WHERE operation_id=:operation_id AND claimed_by=:worker_id")
                .bind("claimed_until", dialect.bindInstant(claimedUntil)).bind("operation_id", uuid(operationId))
                .bind("worker_id", workerId).fetch().rowsUpdated().map(rows -> rows == 1);
    }

    @Override
    public Mono<Void> delete(UUID operationId, Long tenantId) {
        if (operationId == null || tenantId == null) return Mono.empty();
        return databaseClient.sql("DELETE FROM " + TABLE + " WHERE operation_id=:operation_id AND tenant_id=:tenant_id")
                .bind("operation_id", uuid(operationId)).bind("tenant_id", tenantId).fetch().rowsUpdated().then();
    }

    private Mono<DeviceImportJob> get(UUID operationId) {
        return databaseClient.sql("SELECT " + COLUMNS + " FROM " + TABLE + " WHERE operation_id=:operation_id")
                .bind("operation_id", uuid(operationId)).map((row, metadata) -> new DeviceImportJob(
                        readUuid(row.get("operation_id")), row.get("tenant_id", Long.class),
                        row.get("driver_id", Long.class), row.get("profile_id", Long.class),
                        row.get("operator_id", Long.class), row.get("operator_name", String.class),
                        row.get("file_name", String.class), row.get("file_data", byte[].class),
                        row.get("claimed_by", String.class), instant(row.get("claimed_until")),
                        number(row.get("attempts")))).one();
    }
