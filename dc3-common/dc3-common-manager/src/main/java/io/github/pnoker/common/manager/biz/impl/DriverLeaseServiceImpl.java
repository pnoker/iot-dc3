...
public class DriverLeaseServiceImpl implements DriverLeaseService {
    ...
    @Override
    public Mono<DriverLeaseGrantBO> renew(Long tenantId, Long driverId, String node, String client,
        ...
        return Mono.defer(() -> {
            ...
            return transactionalOperator.transactional(
                    store.acquireDriverLock(tenantId, driverId)
                            .then(store.renewInstance(tenantId, driverId, node, client, host, leaseUntil))
                            .then(store.deleteExpiredInstances(tenantId, driverId,
                            ...
                            .then(Mono.zip(
                                    ...
                                    store.getLeaseState(tenantId, driverId).map(Optional::of)
                            ...
                            .flatMap(tuple -> {
                                ...
                                Mono<Long> version = reconcile
                                        ...
                                        .then(store.advanceAssignmentVersion(tenantId, driverId,
                                                membershipHash, deviceRevision))
                                        : Mono.just(state.get().getAssignmentVersion());
                                return version.map(assignmentVersion -> new DriverLeaseGrantBO(
                                        leaseUntil.toEpochMilli(), assignmentVersion,
    ...
    @Override
    public Flux<DeviceLeaseBO> listOwnedLeases(Long tenantId, Long driverId, String node,
                                               long afterDeviceId, int limit) {
        if (!valid(tenantId) || !valid(driverId) || blank(node) || afterDeviceId < 0
        ...
        return store.listOwnedLeases(tenantId, driverId, node, afterDeviceId, limit)
                .map(value -> new DeviceLeaseBO(value.getDriverId(), value.getDeviceId(),
                        value.getOwnerNode(), value.getFencingToken()));
    }
    ...
    public Mono<DeviceLeaseBO> getActiveOwner(Long tenantId, Long deviceId) {
        ...
        return store.getActiveLease(tenantId, deviceId)
                .map(lease -> new DeviceLeaseBO(lease.getDriverId(), lease.getDeviceId(),
                        lease.getOwnerNode(), lease.getFencingToken()));
    }
        ...
        return loadDevicePage(tenantId, driverId, 0L)
                ...
                .concatMap(page -> {
                    List<DeviceLeaseDO> assignments = page.deviceIds.stream()
                            .map(deviceId -> new DeviceLeaseDO(tenantId, driverId, deviceId,
    ...
    private void validate(Long tenantId, Long driverId, String node, String client,
                          String host, int leaseSeconds, long knownAssignmentVersion) {
        if (!valid(tenantId) || !valid(driverId) || blank(node) || blank(client) || blank(host)
