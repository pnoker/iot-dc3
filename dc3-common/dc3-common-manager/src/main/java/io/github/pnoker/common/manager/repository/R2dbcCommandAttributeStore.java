...
public class R2dbcCommandAttributeStore implements ReactiveCommandAttributeStore {
    ...
    private DatabaseClient.GenericExecuteSpec bind(DatabaseClient.GenericExecuteSpec spec, CommandAttributeFilter filter) {
    ...
    private String orderBy(List<SortSpec> sort) {
        ...
        if (sort != null) for (SortSpec spec : sort) {
            String column = switch (spec.field()) {
                ...
                case "version" -> "a.version"; default -> throw new IllegalArgumentException("unsupported command attribute sort field");
            };
    ...
    private CommandAttributeBO map(io.r2dbc.spi.Row row, io.r2dbc.spi.RowMetadata metadata) {
        ...
        if (raw != null) try {
            ...
            CommandAttributeExt ext = new CommandAttributeExt(); ext.setType(json.getType()); ext.setVersion(json.getVersion()); ext.setRemark(json.getRemark());
            ext.setContent(json.getContent() == null ? null : JsonUtil.parseObject(json.getContent(), CommandAttributeExt.Content.class));
