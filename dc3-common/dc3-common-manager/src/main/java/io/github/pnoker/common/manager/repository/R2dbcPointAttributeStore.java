...
public class R2dbcPointAttributeStore implements ReactivePointAttributeStore {
    ...
    private DatabaseClient.GenericExecuteSpec bind(DatabaseClient.GenericExecuteSpec spec, PointAttributeFilter filter) {
    ...
    private String orderBy(List<SortSpec> sort) {
        ...
        if (sort != null) for (SortSpec spec : sort) {
            String column = switch (spec.field()) {
                ...
                case "version" -> "a.version"; default -> throw new IllegalArgumentException("unsupported point attribute sort field");
            };
    ...
    private PointAttributeBO map(io.r2dbc.spi.Row row, io.r2dbc.spi.RowMetadata metadata) {
        ...
        if (raw != null) try {
            ...
            PointAttributeExt ext = new PointAttributeExt(); ext.setType(json.getType()); ext.setVersion(json.getVersion()); ext.setRemark(json.getRemark());
            ext.setContent(json.getContent() == null ? null : JsonUtil.parseObject(json.getContent(), PointAttributeExt.Content.class));
