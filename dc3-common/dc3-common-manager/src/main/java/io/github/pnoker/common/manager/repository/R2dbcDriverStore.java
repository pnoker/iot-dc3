...
public class R2dbcDriverStore implements ReactiveDriverStore {
    ...
    public Mono<OffsetPage<DriverBO>> list(DriverFilter filter) {
        ...
        if (filter.groupId() != null) where.append(" AND EXISTS (SELECT 1 FROM dc3_manager.dc3_group_bind dgb"
        ...
        if (filter.labelId() != null) where.append(" AND EXISTS (SELECT 1 FROM dc3_manager.dc3_label_bind dlb"
        ...
        DatabaseClient.GenericExecuteSpec countSpec = databaseClient.sql("SELECT COUNT(*) AS total FROM " + TABLE + " d" + where)
                .bind("tenant_id", filter.tenantId());
        DatabaseClient.GenericExecuteSpec itemSpec = databaseClient.sql("SELECT " + COLUMNS + " FROM " + TABLE + " d" + where
        ...
        Mono<Long> total = countSpec.map((row, metadata) -> {
    ...
    public Mono<DriverBO> getByNameAndCode(Long tenantId, String driverName, String driverCode) {
        ...
        return databaseClient.sql("SELECT " + COLUMNS + " FROM " + TABLE
                ...
                .bind("tenant_id", tenantId).bind("driver_name", driverName).bind("driver_code", driverCode)
                .map(this::map).one();
    }
    ...
    public Mono<DriverBO> getByServiceName(Long tenantId, String serviceName) {
        ...
        return databaseClient.sql("SELECT " + COLUMNS + " FROM " + TABLE
                ...
                .bind("tenant_id", tenantId).bind("service_name", serviceName).map(this::map).one();
    }

    @Override
    public Mono<DriverBO> getByDeviceId(Long tenantId, Long deviceId) {
        if (tenantId == null || deviceId == null) return Mono.empty();
        return databaseClient.sql("SELECT " + qualifiedColumns("d") + " FROM " + TABLE + " d"
                ...
                .bind("tenant_id", tenantId).bind("device_id", deviceId).map(this::map).one();
    }
    ...
    public Flux<DriverBO> listByIds(Long tenantId, List<Long> ids) {
        ...
        String placeholders = java.util.stream.IntStream.range(0, values.size()).mapToObj(i -> ":id" + i)
    ...
    public Flux<DriverBO> listByProfileId(Long tenantId, Long profileId) {
        ...
        return databaseClient.sql("SELECT DISTINCT " + qualifiedColumns("d")
    ...
    public Flux<DriverBO> listByPointId(Long tenantId, Long pointId) {
        ...
        return databaseClient.sql("SELECT DISTINCT " + qualifiedColumns("d")
                ...
                .bind("tenant_id", tenantId).bind("point_id", pointId).map(this::map).all();
    }
    ...
    public Mono<DriverBO> insert(DriverBO value) {
        ...
        DatabaseClient.GenericExecuteSpec spec = databaseClient.sql("INSERT INTO " + TABLE + " (" + COLUMNS
                        + ", deleted) VALUES (:id, :driver_name, :driver_code, :service_name, :service_host, :driver_type_flag,"
                        + " " + dialect.jsonWriteExpression(":driver_ext") + ", :enable_flag, :tenant_id, :remark, :signature, :version, :creator_id,"
                        + " :creator_name, :create_time, :operator_id, :operator_name, :operate_time, 0)")
                .bind("id", value.getId()).bind("driver_name", valueOrEmpty(value.getDriverName()))
                .bind("driver_code", valueOrEmpty(value.getDriverCode())).bind("service_name", valueOrEmpty(value.getServiceName()))
                .bind("service_host", valueOrEmpty(value.getServiceHost())).bind("driver_type_flag", enumIndex(value.getDriverTypeFlag()))
                .bind("driver_ext", serialize(value.getDriverExt())).bind("enable_flag", enumIndex(value.getEnableFlag()))
                .bind("tenant_id", value.getTenantId()).bind("remark", valueOrEmpty(value.getRemark()))
                .bind("signature", valueOrEmpty(value.getSignature())).bind("version", value.getVersion() == null ? 0 : value.getVersion())
    ...
    public Mono<DriverBO> update(DriverBO value, int expectedVersion) {
        DatabaseClient.GenericExecuteSpec spec = databaseClient.sql("UPDATE " + TABLE + " SET driver_name = :driver_name,"
                        + " driver_code = :driver_code, service_name = :service_name, service_host = :service_host,"
                        + " driver_type_flag = :driver_type_flag, driver_ext = " + dialect.jsonWriteExpression(":driver_ext") + ", enable_flag = :enable_flag,"
                ...
                .bind("driver_name", valueOrEmpty(value.getDriverName())).bind("driver_code", valueOrEmpty(value.getDriverCode()))
                .bind("service_name", valueOrEmpty(value.getServiceName())).bind("service_host", valueOrEmpty(value.getServiceHost()))
                .bind("driver_type_flag", enumIndex(value.getDriverTypeFlag())).bind("driver_ext", serialize(value.getDriverExt()))
                .bind("enable_flag", enumIndex(value.getEnableFlag())).bind("remark", valueOrEmpty(value.getRemark()))
                .bind("signature", valueOrEmpty(value.getSignature())).bind("operator_name", valueOrEmpty(value.getOperatorName()))
                .bind("operate_time", LocalDateTime.now(ZoneOffset.UTC)).bind("id", value.getId()).bind("tenant_id", value.getTenantId())
    ...
    public Mono<Boolean> delete(Long tenantId, Long id, int expectedVersion, Long operatorId, String operatorName) {
        DatabaseClient.GenericExecuteSpec spec = databaseClient.sql("UPDATE " + TABLE + " SET deleted = 1, operator_id = :operator_id,"
    ...
    private DatabaseClient.GenericExecuteSpec bindFilter(DatabaseClient.GenericExecuteSpec spec, DriverFilter filter) {
        if (present(filter.driverName())) spec = spec.bind("driver_name", "%" + filter.driverName().trim() + "%");
        if (present(filter.driverCode())) spec = spec.bind("driver_code", filter.driverCode().trim());
        if (present(filter.serviceName())) spec = spec.bind("service_name", filter.serviceName().trim());
        if (present(filter.serviceHost())) spec = spec.bind("service_host", filter.serviceHost().trim());
        if (filter.driverTypeFlag() != null) spec = spec.bind("driver_type_flag", filter.driverTypeFlag().getIndex());
        ...
        if (filter.groupId() != null || filter.labelId() != null) spec = spec.bind("entity_type", EntityTypeEnum.DRIVER.getIndex());
        return spec;
    }
    ...
    private String orderBy(List<SortSpec> sort) {
        ...
        if (sort != null) for (SortSpec spec : sort) {
            String column = switch (spec.field()) {
                ...
                default -> throw new IllegalArgumentException("unsupported driver sort field: " + spec.field());
            };
    ...
    private DriverBO map(io.r2dbc.spi.Row row, io.r2dbc.spi.RowMetadata metadata) {
        ...
        value.setId(row.get("id", Long.class)); value.setDriverName(row.get("driver_name", String.class));
        value.setDriverCode(row.get("driver_code", String.class)); value.setServiceName(row.get("service_name", String.class));
        value.setServiceHost(row.get("service_host", String.class)); value.setTenantId(row.get("tenant_id", Long.class));
        ...
        value.setCreatorName(row.get("creator_name", String.class)); value.setCreateTime(toLocalDateTime(row.get("create_time")));
        ...
        Number type = row.get("driver_type_flag", Number.class); value.setDriverTypeFlag(DriverTypeEnum.ofIndex(type == null ? null : type.byteValue()));
        Number enabled = row.get("enable_flag", Number.class); value.setEnableFlag(EnableFlagEnum.ofIndex(enabled == null ? null : enabled.byteValue()));
        String ext = row.get("driver_ext", String.class); value.setDriverExt(deserialize(ext == null ? null : ext));
        return value;
    }
    ...
    private DriverExt deserialize(String json) { try { return json == null ? null : objectMapper.readValue(json, DriverExt.class); } catch (Exception e) { throw new IllegalStateException("invalid driver_ext JSON", e); } }
    private String serialize(DriverExt ext) { try { return objectMapper.writeValueAsString(ext == null ? new DriverExt() : ext); } catch (Exception e) { throw new IllegalArgumentException("driver_ext is not valid JSON", e); } }
    private Byte enumIndex(DriverTypeEnum value) { return value == null ? DriverTypeEnum.DRIVER_CLIENT.getIndex() : value.getIndex(); }
    private Byte enumIndex(EnableFlagEnum value) { return value == null ? EnableFlagEnum.ENABLE.getIndex() : value.getIndex(); }
