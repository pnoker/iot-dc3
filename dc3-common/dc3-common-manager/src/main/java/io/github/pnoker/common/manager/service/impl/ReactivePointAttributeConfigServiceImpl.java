...
public class ReactivePointAttributeConfigServiceImpl implements ReactivePointAttributeConfigService {
    ...
    @Override
    public Mono<PointAttributeConfigBO> add(PointAttributeConfigBO value) {
        return validate(value, false)
                .then(Mono.defer(() -> store.getByAttributeDevicePoint(
                        value.getTenantId(), value.getAttributeId(), value.getDeviceId(), value.getPointId())))
                .flatMap(existing -> Mono.<PointAttributeConfigBO>error(
                        new DuplicateException("Point attribute config has been duplicated")))
                .switchIfEmpty(Mono.defer(() -> store.insert(normalize(value, false))))
                .onErrorMap(
                        DataIntegrityViolationException.class,
                        error -> new DuplicateException("Point attribute config has been duplicated"))
                .doOnNext(saved -> publish(saved.getTenantId(), saved.getDeviceId()));
    }

    @Override
    public Mono<PointAttributeConfigBO> update(PointAttributeConfigBO value) {
        return validate(value, true)
                ...
                .flatMap(current -> store.getByAttributeDevicePoint(
                                value.getTenantId(), value.getAttributeId(), value.getDeviceId(), value.getPointId())
    ...
    @Override
    public Mono<PointAttributeConfigBO> getById(Long tenantId, Long id) {
        return store.get(tenantId, id)
                .switchIfEmpty(Mono.error(new NotFoundException("Point attribute config does not exist")));
    }

    @Override
    public Mono<PointAttributeConfigBO> getByAttributeIdAndDeviceIdAndPointId(
            Long tenantId, Long attributeId, Long deviceId, Long pointId) {
        return store.getByAttributeDevicePoint(tenantId, attributeId, deviceId, pointId)
                .switchIfEmpty(Mono.error(new NotFoundException("Point attribute config does not exist")));
    }
    ...
    public Flux<PointAttributeConfigBO> listByDeviceIdAndPointId(Long tenantId, Long deviceId, Long pointId) {
        return store.listByDeviceIdAndPointId(tenantId, deviceId, pointId);
    }
    ...
    private Mono<Void> validate(PointAttributeConfigBO value, boolean update) {
        ...
        return Mono.defer(() -> Mono.zip(
                        ...
                        pointService.getById(value.getTenantId(), value.getPointId())))
                ...
                .flatMap(tuple -> {
                    ...
                    if (!Objects.equals(attribute.getDriverId(), device.getDriverId())
                            || !Objects.equals(point.getProfileId(), device.getProfileId())) {
