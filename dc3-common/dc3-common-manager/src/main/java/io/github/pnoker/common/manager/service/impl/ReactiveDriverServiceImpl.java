...
public class ReactiveDriverServiceImpl implements ReactiveDriverService {
    ...
    public Mono<DriverBO> add(DriverBO driver) {
        return Mono.defer(() -> {
            ...
            if (driver.getDriverCode() == null || driver.getDriverCode().isBlank()) driver.setDriverCode(CodeUtil.getCode());
            return driverStore.getByNameAndCode(driver.getTenantId(), driver.getDriverName(), driver.getDriverCode())
                    .flatMap(existing -> Mono.<DriverBO>error(new DuplicateException("Failed to create driver: driver has been duplicated")))
    ...
    public Mono<DriverBO> update(DriverBO driver) {
        return Mono.defer(() -> {
            ...
            return driverStore.get(driver.getTenantId(), driver.getId())
                .switchIfEmpty(Mono.error(new NotFoundException("Driver does not exist")))
                .flatMap(current -> {
                    if (current.getDriverCode() != null && !current.getDriverCode().equals(driver.getDriverCode())) {
                        return Mono.error(new RequestException("Driver code cannot be changed"));
                    }
                    return driverStore.getByNameAndCode(driver.getTenantId(), driver.getDriverName(), driver.getDriverCode())
                        .filter(existing -> !existing.getId().equals(driver.getId()))
                        .flatMap(existing -> Mono.<DriverBO>error(new DuplicateException("Failed to update driver: driver has been duplicated")))
    ...
    public Mono<Boolean> delete(Long tenantId, Long id, int expectedVersion, Long operatorId, String operatorName) {
        if (tenantId == null || id == null) return Mono.error(new RequestException("Tenant ID and driver ID are required"));
        return driverStore.get(tenantId, id).switchIfEmpty(Mono.error(new NotFoundException("Driver does not exist")))
                ...
                .switchIfEmpty(Mono.error(new ConflictException("Driver version conflict")))
                .doOnSuccess(ignored -> metadataEventPublisher.publishEvent(new MetadataEvent(this, tenantId, id,
                        MetadataTypeEnum.DRIVER, MetadataOperateTypeEnum.DELETE)));
    }
    ...
    public Mono<DriverBO> getByServiceName(Long tenantId, String serviceName) {
        if (tenantId == null || serviceName == null || serviceName.isBlank()) return Mono.error(new RequestException("Tenant ID and service name are required"));
        return driverStore.getByServiceName(tenantId, serviceName).switchIfEmpty(Mono.error(new NotFoundException("Driver does not exist")));
    }
    ...
    public Mono<DriverBO> getByDeviceId(Long tenantId, Long deviceId) {
        ...
        return driverStore.getByDeviceId(tenantId, deviceId).switchIfEmpty(Mono.error(new NotFoundException("Driver does not exist")));
    }

    @Override public Mono<OffsetPage<DriverBO>> list(DriverFilter filter) { return driverStore.list(filter); }
    @Override public Flux<DriverBO> listByIds(Long tenantId, List<Long> ids) { return driverStore.listByIds(tenantId, ids); }
    @Override public Flux<DriverBO> listByProfileId(Long tenantId, Long profileId) { return driverStore.listByProfileId(tenantId, profileId); }
    @Override public Flux<DriverBO> listByPointId(Long tenantId, Long pointId) { return driverStore.listByPointId(tenantId, pointId); }

    private void validateWrite(DriverBO driver, boolean update) {
        if (driver == null || driver.getTenantId() == null || driver.getTenantId() <= 0
                || driver.getDriverName() == null || driver.getDriverName().isBlank()
                || driver.getServiceName() == null || driver.getServiceName().isBlank()
                || driver.getServiceHost() == null || driver.getServiceHost().isBlank()) {
        ...
        driver.setDriverName(driver.getDriverName().trim()); driver.setServiceName(driver.getServiceName().trim());
        driver.setServiceHost(driver.getServiceHost().trim());
    }

    private void publish(DriverBO driver, MetadataOperateTypeEnum operation) {
        if (driver != null) metadataEventPublisher.publishEvent(new MetadataEvent(this, driver.getTenantId(), driver.getId(), MetadataTypeEnum.DRIVER, operation));
    }
}
