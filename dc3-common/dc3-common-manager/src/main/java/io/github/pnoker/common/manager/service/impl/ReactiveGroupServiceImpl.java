...
public class ReactiveGroupServiceImpl implements ReactiveGroupService {
    ...
    private static final Set<EntityTypeEnum> SUPPORTED = Set.of(
            EntityTypeEnum.DRIVER,
            EntityTypeEnum.PROFILE,
            EntityTypeEnum.POINT,
    ...
    public Mono<GroupBO> update(GroupBO group) {
        return Mono.defer(() -> {
            validate(group, true);
            return groupStore.get(group.getTenantId(), group.getId())
    ...
    public Mono<Boolean> delete(Long tenantId, Long id, Long operatorId, String operatorName) {
        ...
        return groupStore.get(tenantId, id)
                .switchIfEmpty(Mono.error(new NotFoundException("Group does not exist")))
                .then(Mono.zip(groupStore.hasChildren(tenantId, id), groupStore.hasActiveBindings(tenantId, id)))
                .flatMap(state -> {
                    if (state.getT1()) {
                        return Mono.error(new AssociatedException("Failed to remove group: there are subgroups under the group"));
                    }
                    if (state.getT2()) {
                        return Mono.error(new AssociatedException("Failed to remove group: the group has been bound by another entity"));
                    }
    ...
    private Mono<Void> normalizeParent(GroupBO group) {
        ...
        return groupStore.get(group.getTenantId(), parentId)
    ...
    private Mono<Void> ensureAcyclic(Long tenantId, Long groupId, GroupBO current, Set<Long> visited) {
        ...
        return groupStore.get(tenantId, current.getParentGroupId())
                .switchIfEmpty(Mono.error(new NotFoundException("Resource does not exist")))
