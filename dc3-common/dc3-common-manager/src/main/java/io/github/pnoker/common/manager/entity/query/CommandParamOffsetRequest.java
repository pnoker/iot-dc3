...
public record CommandParamOffsetRequest(
        ...
        @Schema(description = "Maximum result count between 1 and 200.")
        Integer limit,

        @Schema(description = "Stable sort fields and directions.")
        List<SortSpec> sort,

        @Schema(description = "Partial parameter name filter.")
        String paramName,

        @Schema(description = "Exact stable parameter code filter.")
        String paramCode,

        @Schema(description = "Input or output direction filter.")
        ParamDirectionTypeEnum paramDirection,

        @Schema(description = "Parameter data type filter.") PointTypeEnum paramTypeFlag,

        @Schema(description = "Parent command identifier filter.")
        Long commandId,
        ...
        Integer version) {
    /** Command param offset request compact constructor: normalizes the query record. */
    ...
    /** command param offset request. */
    public CommandParamOffsetRequest() {
        this(0L, PageRequest.DEFAULT_LIMIT, List.of(), null, null, null, null, null, null, null);
    }
}
