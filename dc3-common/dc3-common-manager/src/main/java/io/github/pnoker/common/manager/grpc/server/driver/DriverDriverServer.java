...
public class DriverDriverServer extends DriverApiGrpc.DriverApiImplBase {
    ...
    @Autowired
    public DriverDriverServer(GrpcDriverBuilder grpcDriverBuilder,
    ...
    public void driverRegister(GrpcDriverRegisterDTO request, StreamObserver<GrpcDriverRegistrationDTO> observer) {
        ReactiveGrpcServerSupport.subscribe(reactiveRegisterService.register(request).map(this::registrationResponse), observer);
    }

    @Override
    public void renewLease(GrpcDriverLeaseRequest request, StreamObserver<GrpcDriverLeaseDTO> observer) {
        Flux<GrpcDriverLeaseDTO> response = driverLeaseService.renew(request.getTenantId(), request.getDriverId(), request.getNode(), request.getClient(),
                        request.getHost(), request.getLeaseSeconds(), request.getAssignmentVersion())
    ...
    public void getById(GrpcDriverQuery request, StreamObserver<GrpcDriverRegistrationDTO> observer) {
        Mono<GrpcDriverRegistrationDTO> response = reactiveDriverService.getById(request.getTenantId(), request.getDriverId())
    ...
    private GrpcDriverRegistrationDTO registrationResponse(ReactiveDriverRegisterService.Registration registration) {
        return GrpcDriverRegistrationDTO.newBuilder()
                ...
                .addAllDriverAttributes(registration.driverAttributes().stream().map(grpcDriverAttributeBuilder::buildGrpcDTOByBO).toList())
                .addAllPointAttributes(registration.pointAttributes().stream().map(grpcPointAttributeBuilder::buildGrpcDTOByBO).toList())
                .addAllCommandAttributes(registration.commandAttributes().stream().map(grpcCommandAttributeBuilder::buildGrpcDTOByBO).toList())
                .addAllEventAttributes(registration.eventAttributes().stream().map(grpcEventAttributeBuilder::buildGrpcDTOByBO).toList())
                .build();
    }

    private Mono<GrpcDriverRegistrationDTO> metadataResponse(DriverBO driver) {
        return Mono.zip(reactiveDriverAttributeService.listByDriverId(driver.getTenantId(), driver.getId()).collectList(),
                        reactivePointAttributeService.listByDriverId(driver.getTenantId(), driver.getId()).collectList(),
                        reactiveCommandAttributeService.listByDriverId(driver.getTenantId(), driver.getId()).collectList(),
                        reactiveEventAttributeService.listByDriverId(driver.getTenantId(), driver.getId()).collectList())
                .map(tuple -> GrpcDriverRegistrationDTO.newBuilder()
                        .setDriver(grpcDriverBuilder.buildGrpcDTOByBO(driver))
                        .addAllDriverAttributes(tuple.getT1().stream().filter(value -> Objects.equals(driver.getTenantId(), value.getTenantId())).map(grpcDriverAttributeBuilder::buildGrpcDTOByBO).toList())
                        .addAllPointAttributes(tuple.getT2().stream().filter(value -> Objects.equals(driver.getTenantId(), value.getTenantId())).map(grpcPointAttributeBuilder::buildGrpcDTOByBO).toList())
                        .addAllCommandAttributes(tuple.getT3().stream().filter(value -> Objects.equals(driver.getTenantId(), value.getTenantId())).map(grpcCommandAttributeBuilder::buildGrpcDTOByBO).toList())
                        .addAllEventAttributes(tuple.getT4().stream().filter(value -> Objects.equals(driver.getTenantId(), value.getTenantId())).map(grpcEventAttributeBuilder::buildGrpcDTOByBO).toList())
                        .build());
    }


    private Flux<GrpcDriverLeaseDTO> streamAssignmentSnapshot(GrpcDriverLeaseRequest request, DriverLeaseGrantBO grant) {
        return assertAssignmentVersion(request, grant.assignmentVersion()).thenMany(streamAssignmentPage(request, grant, 0L));
    }

    private Flux<GrpcDriverLeaseDTO> streamAssignmentPage(GrpcDriverLeaseRequest request, DriverLeaseGrantBO grant, long afterDeviceId) {
        return assertAssignmentVersion(request, grant.assignmentVersion()).thenMany(driverLeaseService
                .listOwnedLeases(request.getTenantId(), request.getDriverId(), request.getNode(), afterDeviceId, ASSIGNMENT_BATCH_SIZE + 1)
                .collectList().flatMapMany(page -> {
                    ...
                    long nextCursor = batch.isEmpty() ? afterDeviceId : batch.getLast().deviceId();
                    Flux<GrpcDriverLeaseDTO> current = complete
                            ? assertAssignmentVersion(request, grant.assignmentVersion()).thenMany(Flux.just(leaseResponse(grant, batch, true)))
                            : Flux.just(leaseResponse(grant, batch, false));
                    return complete ? current : Flux.concat(current, streamAssignmentPage(request, grant, nextCursor));
                }));
    }

    private Mono<Void> assertAssignmentVersion(GrpcDriverLeaseRequest request, long expectedVersion) {
        return driverLeaseService.getAssignmentVersion(request.getTenantId(), request.getDriverId())
                .flatMap(current -> current == expectedVersion ? Mono.empty() : Mono.error(new IllegalStateException("Driver assignment changed while streaming snapshot")));
    }

    private GrpcDriverLeaseDTO leaseResponse(DriverLeaseGrantBO grant, List<DeviceLeaseBO> leases, boolean complete) {
        return GrpcDriverLeaseDTO.newBuilder().addAllDeviceLeases(leases.stream()
                        .map(lease -> GrpcDeviceLeaseDTO.newBuilder().setDeviceId(lease.deviceId()).setFencingToken(lease.fencingToken()).build()).toList())
                .setLeaseUntilEpochMillis(grant.leaseUntilEpochMillis()).setAssignmentVersion(grant.assignmentVersion())
                .setAssignmentsChanged(grant.assignmentsChanged()).setSnapshotComplete(complete).build();
    }
}
