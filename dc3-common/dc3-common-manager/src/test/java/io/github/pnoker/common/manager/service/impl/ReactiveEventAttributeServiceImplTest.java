...
class ReactiveEventAttributeServiceImplTest {
    @Mock ReactiveEventAttributeStore store;
    @Mock ReactiveDriverService drivers;
    @Mock MetadataEventPublisher publisher;

    @Test
    void addValidatesDriverAndReturnsSavedAttribute() {
        EventAttributeBO value = value(null);
        EventAttributeBO saved = value(10L);
        ...
        StepVerifier.create(new ReactiveEventAttributeServiceImpl(store, drivers, publisher).add(value))
    ...
    void listDelegatesCanonicalOffsetFilter() {
        var filter = new EventAttributeFilter(7L, null, null, null, 11L, null, null, 20, 10, List.of());
        var page = OffsetPage.of(List.<EventAttributeBO>of(), 20, 10, 20);
        when(store.list(filter)).thenReturn(Mono.just(page));
        StepVerifier.create(new ReactiveEventAttributeServiceImpl(store, drivers, publisher).list(filter))
                .expectNext(page).verifyComplete();
        verify(store).list(filter);
    }

    private EventAttributeBO value(Long id) {
