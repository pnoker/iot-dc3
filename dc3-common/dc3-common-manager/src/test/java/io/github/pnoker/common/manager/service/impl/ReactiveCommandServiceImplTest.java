...
class ReactiveCommandServiceImplTest {
    ...
    @Mock
    ReactiveCommandParamStore params;
    ...
    void rejectsMissingIdentity() {
        StepVerifier.create(new ReactiveCommandServiceImpl(store, profiles, publisher, params, transactions)
                        .getById(null, 1L))
                .expectErrorMessage("Tenant ID and command ID are required")
                .verify();
    }
    ...
    void rejectsInvalidAddWithoutDatabaseAccess() {
        ...
        StepVerifier.create(
                        new ReactiveCommandServiceImpl(store, profiles, publisher, params, transactions).add(command))
                .expectErrorMessage("Tenant ID, profile ID, command name and timeout are required")
                .verify();
