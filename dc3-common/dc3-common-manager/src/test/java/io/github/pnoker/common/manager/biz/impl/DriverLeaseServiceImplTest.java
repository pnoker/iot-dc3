...
class DriverLeaseServiceImplTest {
    @Mock
    private ReactiveDriverLeaseStore store;
    ...
    void setUp() {
        ...
        lenient()
                .when(store.renewInstance(any(), any(), anyString(), anyString(), anyString(), any()))
        ...
        lenient()
                .when(store.listDriverDeviceIds(any(), any(), anyLong(), anyInt()))
                .thenReturn(Flux.empty());
        lenient()
                .when(store.advanceAssignmentVersion(any(), any(), anyString(), anyLong()))
                .thenReturn(Mono.just(1L));
        lenient()
                .when(store.listOwnedLeases(any(), any(), anyString(), anyLong(), anyInt()))
                .thenReturn(Flux.empty());
    ...
    void initialRenewReconcilesAllDevicesAndReturnsChangedGrant() {
        ...
        when(store.listDriverDeviceIds(1L, 2L, 0L, 5000))
                .thenReturn(Flux.fromStream(LongStream.rangeClosed(1, 100).boxed()));
        when(store.advanceAssignmentVersion(eq(1L), eq(2L), anyString(), eq(0L)))
        ...
        assertThat(assignments.getValue())
                .extracting(DeviceLeaseDO::getOwnerNode)
    ...
    void stableHeartbeatSkipsReconciliation() {
        ...
        StepVerifier.create(service.renew(1L, 2L, "node-a", "client-a", "host-a", 30, 9L))
                .assertNext(grant -> assertThat(grant.assignmentsChanged()).isFalse())
                .verifyComplete();
    ...
    void ownedAssignmentsUseValidatedKeysetCursor() {
        when(store.listOwnedLeases(1L, 2L, "node-a", 100L, 1000))
                .thenReturn(Flux.just(new DeviceLeaseDO(1L, 2L, 101L, "node-a", 7L)));
        StepVerifier.create(service.listOwnedLeases(1L, 2L, "node-a", 100L, 1000))
                .assertNext(lease -> assertThat(lease.deviceId()).isEqualTo(101L))
                .verifyComplete();
        StepVerifier.create(service.listOwnedLeases(1L, 2L, "node-a", -1L, 1000))
    ...
    void missingAssignmentStateIsAnError() {
        when(store.getLeaseState(1L, 2L)).thenReturn(Mono.empty());
        StepVerifier.create(service.getAssignmentVersion(1L, 2L))
                .expectError(ServiceException.class)
                .verify();
    }
    ...
    void invalidRenewInputFailsBeforeDatabase() {
        StepVerifier.create(service.renew(1L, 2L, "", "client", "host", 30, 0))
                .expectError(ServiceException.class)
                .verify();
    ...
    void activeOwnerIsTenantScopedAndReactive() {
        ...
        StepVerifier.create(service.getActiveOwner(1L, 10L))
                .assertNext(owner -> assertThat(owner).isEqualTo(new DeviceLeaseBO(2L, 10L, "node-a", 4L)))
                .verifyComplete();
    }
        ...
        try {
            ...
            return java.util.HexFormat.of()
                    .formatHex(digest.digest(
                            String.join("\u0000", nodes).getBytes(java.nio.charset.StandardCharsets.UTF_8)));
