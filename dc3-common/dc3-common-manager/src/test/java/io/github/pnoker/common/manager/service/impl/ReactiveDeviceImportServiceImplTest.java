...
import io.github.pnoker.db.core.operation.OperationRepository;
...
class ReactiveDeviceImportServiceImplTest {
    ...
    @BeforeEach
    void setUp() {
        service = new ReactiveDeviceImportServiceImpl(
                operationRepository,
                jobStore,
                schemaService,
                workbookCodec,
        ...
        when(schemaService.load(7L, 10L, 20L))
                .thenReturn(Mono.just(new DeviceImportManifest(
                        DeviceImportManifest.CURRENT_SCHEMA_VERSION, 10L, 20L, List.of(), List.of())));
    }
    ...
    void sameIdempotencyKeyAndRequestReturnsOriginalOperationWithoutDuplicateJob() {
        ...
        StepVerifier.create(first.zipWith(second))
                .assertNext(tuple -> assertThat(tuple.getT2().operationId())
                        .isEqualTo(tuple.getT1().operationId()))
                .verifyComplete();
    ...
    void sameIdempotencyKeyWithDifferentContentProducesDifferentRequestHash() {
        ...
        assertThat(states.getAllValues())
                .extracting(OperationState::idempotencyKey)
    ...
    void validatesTenantScopedDriverAndProfileBeforeCreatingOperation() {
        when(schemaService.load(7L, 10L, 20L))
                .thenReturn(
                        Mono.error(new io.github.pnoker.common.exception.NotFoundException("Resource does not exist")));
