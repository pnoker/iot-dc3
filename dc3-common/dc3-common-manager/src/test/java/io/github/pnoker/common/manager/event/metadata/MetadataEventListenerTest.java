...
class MetadataEventListenerTest {
    @Mock private ReactiveDriverService driverService;
    @Mock private MessageSender messageSender;
    @InjectMocks private MetadataEventListener listener;
    ...
    void deviceEventNotifiesOwningDriverWithTenant() {
        ...
        listener.onApplicationEvent(new MetadataEvent(this, 1L, 10L, MetadataTypeEnum.DEVICE, MetadataOperateTypeEnum.UPDATE));
        verify(messageSender).send(argThat(m -> m.getTopic() == MqTopic.METADATA
                && "dc3-driver-modbus-tcp".equals(m.getPartitionKey())
                && ((io.github.pnoker.common.entity.dto.MetadataEventDTO) m.getPayload()).getTenantId().equals(1L)));
    }
    ...
    void pointEventFansOutToEveryAffectedDriver() {
        ...
        listener.onApplicationEvent(new MetadataEvent(this, 1L, 20L, MetadataTypeEnum.POINT, MetadataOperateTypeEnum.ADD));
        verify(messageSender, times(2)).send(any());
    }

    @Test
    void eventWithoutTenantIsDropped() {
        listener.onApplicationEvent(new MetadataEvent(this, 10L, MetadataTypeEnum.DEVICE, MetadataOperateTypeEnum.UPDATE));
        verifyNoInteractions(driverService, messageSender);
    }

    @Test
    void eventWithTargetServicesBypassesOwnerLookup() {
        listener.onApplicationEvent(new MetadataEvent(this, 1L, 10L, MetadataTypeEnum.DEVICE,
    ...
    void driverEventNotifiesRegisteredDriverService() {
        ...
        listener.onApplicationEvent(new MetadataEvent(this, 1L, 7L, MetadataTypeEnum.DRIVER, MetadataOperateTypeEnum.UPDATE));
    ...
    void serviceFailureDoesNotPublish() {
        ...
        listener.onApplicationEvent(new MetadataEvent(this, 1L, 10L, MetadataTypeEnum.DEVICE, MetadataOperateTypeEnum.DELETE));
