...
class DeviceImportWorkerTest {
    ...
    @Mock ReactiveDeviceStore deviceStore;
    @Mock ReactiveDriverAttributeConfigStore driverConfigStore;
    @Mock ReactivePointAttributeConfigStore pointConfigStore;
    @Mock MetadataEventPublisher metadataEventPublisher;
    ...
    @BeforeEach
    void setUp() {
        worker = new DeviceImportWorker(jobStore, operationRepository, schemaService, workbookCodec, fileScheduler,
                deviceStore, driverConfigStore, pointConfigStore, metadataEventPublisher, transactionalOperator,
                JsonUtil.getObjectMapper());
        job = new DeviceImportJob(OPERATION_ID, 7L, 10L, 20L, 30L, "operator", "devices.xlsx",
                new byte[]{1}, "", null, 0);
        manifest = new DeviceImportManifest(DeviceImportManifest.CURRENT_SCHEMA_VERSION, 10L, 20L,
                List.of(), List.of());
    }
    ...
    void successfulImportCommitsDevicesOperationAndJobDeletionInOneTransaction() {
        ...
        when(operationRepository.findById(any(), eq(OPERATION_ID)))
                .thenReturn(Mono.just(pending), Mono.just(running));
        ...
        when(fileScheduler.call(any())).thenReturn(Mono.just(List.of(
                new DeviceImportRow(2, "Boiler-A", "", List.of(), List.of()))));
        ...
        verify(operationRepository).transition(any(), eq(OPERATION_ID), eq(OperationState.Status.RUNNING),
                completed.capture());
    ...
    void terminalOperationOnlyDeletesOrphanedJob() {
        OperationState succeeded = pending().transition(OperationState.Status.RUNNING, 5, NOW.plusSeconds(1))
        ...
        verifyNoInteractions(schemaService, workbookCodec, fileScheduler, deviceStore, driverConfigStore,
                pointConfigStore, metadataEventPublisher);
    }
    ...
    void metadataFailureCannotChangeCommittedSuccess() {
        ...
        when(deviceStore.insert(any())).thenReturn(Mono.just(saved));
        when(jobStore.delete(OPERATION_ID, 7L)).thenReturn(Mono.empty());
        doThrow(new IllegalStateException("publisher unavailable")).when(metadataEventPublisher).publishEvent(any());

        StepVerifier.create(worker.processSafely(OPERATION_ID)).verifyComplete();

        verify(operationRepository, never()).transition(any(), eq(OPERATION_ID),
                eq(OperationState.Status.SUCCEEDED), any());
    }

    private OperationState pending() {
        return OperationState.pending(OPERATION_ID, 7L, "request-1", "a".repeat(64), NOW,
