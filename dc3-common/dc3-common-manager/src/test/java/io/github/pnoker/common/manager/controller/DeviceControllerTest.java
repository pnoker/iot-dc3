...
class DeviceControllerTest {
    ...
    void listUsesCanonicalOffsetRequestAndReturnsCanonicalPage() {
        DeviceListRequest request =
                new DeviceListRequest(20, 10, List.of(), "sensor", null, 2L, 3L, null, null, null, null);
    ...
    void importUsesMinimalContextAndProjectsPrincipal() {
        ...
        when(deviceImportService.submit(
                        any(DeviceBO.class),
                        org.mockito.ArgumentMatchers.eq("devices.xlsx"),
                        org.mockito.ArgumentMatchers.any(byte[].class),
                        org.mockito.ArgumentMatchers.eq("import-1")))
                .thenReturn(Mono.just(accepted));

        StepVerifier.create(withPrincipal(
                        controller.importDevice(new DeviceImportRequest(11L, 12L), Mono.just(filePart), "import-1")))
                .assertNext(response -> {
                    assertThat(response.getStatusCode()).isEqualTo(HttpStatus.ACCEPTED);
                    assertThat(response.getHeaders().getLocation()).hasToString("/operations/get_by_id?id=1");
    ...
    private static <T> Mono<T> withPrincipal(Mono<T> publisher) {
        ...
        return publisher.contextWrite(
                ReactiveSecurityContextHolder.withAuthentication(new GatewayAuthenticationToken(principal, Set.of())));
    }
}
