...
class DeviceClientTest {
    ...
    void listUsesOffsetPaginationAndEmitsAttachedDevices() {
        ...
        when(deviceBuilder.buildDTOByGrpcDTO(org.mockito.ArgumentMatchers.any())).thenReturn(device);
        DriverMetadata metadata = new DriverMetadata();
        io.github.pnoker.common.driver.entity.bo.DriverBO driver = new io.github.pnoker.common.driver.entity.bo.DriverBO();
        driver.setId(2L);
        driver.setTenantId(1L);
        metadata.setDriver(driver);
        DeviceClient client = new DeviceClient(stub, metadata, deviceBuilder,
        ...
        GrpcOffsetPageDeviceDTO response = GrpcOffsetPageDeviceDTO.newBuilder()
                ...
                .addItems(GrpcDeviceAttachDTO.newBuilder().setDevice(GrpcDeviceDTO.getDefaultInstance()).build())
                .build();
        doAnswer(invocation -> {
            ...
            return null;
        }).when(stub).list(org.mockito.ArgumentMatchers.any(), org.mockito.ArgumentMatchers.any());
    ...
    void mapsAttachedCommandAndEventRuntimeMetadata() {
        ...
        DeviceBO device = new DeviceBO();
        when(deviceBuilder.buildDTOByGrpcDTO(org.mockito.ArgumentMatchers.any())).thenReturn(device);
        DeviceClient client = new DeviceClient(null, new DriverMetadata(), deviceBuilder,
                mock(GrpcDriverAttributeConfigBuilder.class), mock(GrpcPointAttributeConfigBuilder.class),
                mock(GrpcCommandAttributeConfigBuilder.class), mock(GrpcEventAttributeConfigBuilder.class));
        ...
        assertThat(mapped.getCommandRuntimeIdMap().get(30L))
                .satisfies(runtime -> {
        ...
        assertThat(mapped.getEventRuntimeIdMap().get(20L))
                .satisfies(runtime -> {
