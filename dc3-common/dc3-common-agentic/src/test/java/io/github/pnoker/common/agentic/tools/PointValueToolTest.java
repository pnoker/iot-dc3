...
class PointValueToolTest {
    ...
    void writePointValueAlwaysCreatesPendingAction() {
        ...
        StepVerifier.create(tool.writePointValueReactive(
                        10L,
                        20L,
                        "42",
                        toolContext(Map.of(
                                AgenticConstant.ToolContextKey.CONVERSATION_ID,
                                "conv-1",
    ...
    void getPointValueHistoryReturnsRawValuesAndChartData() {
        ...
        StepVerifier.create(tool.getPointValueHistoryReactive(
                        10L, 20L, 5, toolContext(Map.of(AgenticConstant.ToolContextKey.USER_HEADER, header))))
                .assertNext(value -> {
    ...
    void reactiveLatestValueUsesReactiveFacade() {
        ...
        StepVerifier.create(tool.getLatestPointValueReactive(
                        10L, 20L, toolContext(Map.of(AgenticConstant.ToolContextKey.TENANT_ID, 1L))))
                .assertNext(result -> {
                    assertThat(result.success()).isTrue();
    ...
    void readPointValueSendsReadCommandThroughPointCommandFacade() {
        ...
        AgenticToolResult<PointValueTool.PointCommandResult> result = tool.readPointValueReactive(
                        10L, 20L, toolContext(Map.of(AgenticConstant.ToolContextKey.USER_HEADER, header)))
                .block();
