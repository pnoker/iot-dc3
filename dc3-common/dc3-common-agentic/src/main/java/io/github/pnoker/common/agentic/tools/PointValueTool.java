...
public class PointValueTool {
    ...
    public Mono<AgenticToolResult<FacadePointValueBO>> getLatestPointValueReactive(
        ...
        return Mono.defer(() -> {
            ...
            log.debug(
                    "Agentic reactive tool invoked, tool={}, tenantId={}, deviceId={}, pointId={}",
                    "getLatestPointValue",
                    tenantId,
                    deviceId,
                    pointId);
            return pointValueFacade
                    .lastValue(tenantId, deviceId, pointId)
                    ...
                    .onErrorResume(error -> {
                        log.warn(
                                "Agentic reactive tool failed, tool={}, tenantId={}, deviceId={}, pointId={}",
                                "getLatestPointValue",
                                tenantId,
                                deviceId,
                                pointId,
                                error);
                        return Mono.just(
                                AgenticToolResult.error("Error retrieving latest value: " + error.getMessage()));
                    });
        });
    }
        ...
        return Mono.defer(() -> {
            ...
            return pointValueFacade
                    ...
                    .map(history -> {
                        ...
                        List<String> values = history.items().stream()
                        ...
                        PointValueHistory result = new PointValueHistory(
                                deviceId,
                                pointId,
                                size,
                                values,
                                buildHistoryChart(deviceId, pointId, numericSeries),
                                numericSeries.summary());
                        return AgenticToolResult.ok(
                                "Point value history loaded",
                                result,
                                buildHistoryVisualizations(deviceId, pointId, numericSeries));
                    })
                    .onErrorResume(error -> {
                        log.warn(
                                "Agentic reactive tool failed, tool={}, tenantId={}, deviceId={}, pointId={}, count={}",
    ...
    public Mono<AgenticToolResult<PointCommandResult>> readPointValueReactive(
        ...
        return Mono.defer(() -> {
            ...
            log.debug(
                    "Agentic tool invoked, tool={}, tenantId={}, deviceId={}, pointId={}",
                    "readPointValue",
                    tenantId,
                    deviceId,
                    pointId);
            return pointCommandFacade
                    .submitRead(tenantId, deviceId, pointId, PointCommandSourceEnum.AGENTIC)
                    .map(commandId -> AgenticToolResult.ok(
                            "Read command accepted",
                            new PointCommandResult(deviceId, pointId, commandId, true, false, null)))
                    .onErrorResume(error -> {
                        log.warn(
                                "Agentic tool failed, tool={}, tenantId={}, deviceId={}, pointId={}",
                                "readPointValue",
                                tenantId,
                                deviceId,
                                pointId,
     ...
     */
    public Mono<AgenticToolResult<PointCommandResult>> writePointValueReactive(
            Long deviceId, Long pointId, String value, ToolContext toolContext) {
        return Mono.defer(() -> {
            Long tenantId = AgenticToolContextUtil.requireTenantId(toolContext);
            log.debug(
                    "Agentic tool invoked, tool={}, tenantId={}, deviceId={}, pointId={}, valueLength={}",
                    "writePointValue",
                    tenantId,
                    deviceId,
                    pointId,
                    Objects.isNull(value) ? 0 : value.length());
            if (Objects.isNull(deviceId) || Objects.isNull(pointId)) {
                return Mono.just(
                        AgenticToolResult.invalid("Device ID and point ID are required for point write commands."));
            }
            ...
            return actionService
                    .createWritePointValueAction(conversationId, deviceId, pointId, value, header)
                    .map(actionId -> AgenticToolResult.ok(
                    ...
                    .onErrorResume(error -> {
                        log.warn(
                                ...
                                error);
                        return Mono.just(
                                AgenticToolResult.error("Error preparing write command: " + error.getMessage()));
                    });
        });
    }
     ...
     */
    private HistoryChart buildHistoryChart(
            Long deviceId, Long pointId, AgenticVisualizationUtil.NumericSeries numericSeries) {
        ...
        return new HistoryChart(
                AgenticConstant.Visualization.Type.LINE,
                "Device " + deviceId + " / Point " + pointId,
                "index (oldest to newest)",
     ...
     */
    private List<AgenticVisualizationSpec> buildHistoryVisualizations(
            Long deviceId, Long pointId, AgenticVisualizationUtil.NumericSeries series) {
        ...
        List<AgenticVisualizationSpec.Annotation> annotations = Objects.nonNull(
