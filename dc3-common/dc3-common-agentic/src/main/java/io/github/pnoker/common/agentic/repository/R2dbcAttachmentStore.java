...
public class R2dbcAttachmentStore implements ReactiveAttachmentStore {
    ...
    private static final String COLUMNS = "id, conversation_id, file_name, content_type, size, file_path, tenant_id, user_id,"
    ...
    public Mono<AttachmentBO> save(AttachmentBO attachment) {
        ...
        String sql = "INSERT INTO " + TABLE + " (conversation_id, file_name, content_type, size, file_path, tenant_id, user_id,"
        ...
        return transactionalOperator.transactional(databaseClient.sql(sql)
                        ...
                        .fetch().rowsUpdated()
                        .flatMap(rows -> rows == 1 ? findByPath(attachment.getFilePath(), attachment.getTenantId(), attachment.getUserId())
                                : Mono.error(new IllegalStateException("attachment insert affected " + rows + " rows"))))
    ...
    public Flux<AttachmentBO> findByIds(Collection<Long> ids, RequestHeader.PrincipalHeader header) {
        ...
        String markers = java.util.stream.IntStream.range(0, ids.size())
                .mapToObj(index -> ":id" + index).collect(Collectors.joining(","));
        DatabaseClient.GenericExecuteSpec statement = databaseClient.sql("SELECT " + COLUMNS + " FROM " + TABLE
                        + " WHERE id IN (" + markers + ") AND tenant_id = :tenant_id AND user_id = :user_id"
                        + " AND deleted = 0 ORDER BY create_time DESC, id DESC")
                .bind("tenant_id", header.getTenantId()).bind("user_id", header.getUserId());
        int index = 0;
    ...
    private Mono<AttachmentBO> findByPath(String path, Long tenantId, Long userId) {
        return databaseClient.sql("SELECT " + COLUMNS + " FROM " + TABLE
                ...
                .bind("file_path", path).bind("tenant_id", tenantId).bind("user_id", userId)
                .map(this::map).one();
    }
