 ...
 * {@link PointValueCommandFacade}) so that calls follow the project's
 * local/gRPC dual-mode convention.
 */
@Slf4j
@Component
public class DataToolSet {
    ...
    public DataToolSet(PointValueFacade pointValueFacade,
    ...
    public String getLatestPointValue(
        ...
        try {
            ...
            return String.format("Device %d / Point %d: value=%s, rawValue=%s, time=%d",
                    value.getDeviceId(), value.getPointId(), value.getValue(), value.getRawValue(), value.getCreateTime());
        } catch (Exception e) {
    ...
    public String getPointValueHistory(
            @ToolParam(description = "The tenant ID") Long tenantId,
            @ToolParam(description = "The device ID") Long deviceId,
            @ToolParam(description = "The point (metric) ID") Long pointId,
            @ToolParam(description = "Number of historical records to retrieve") int count) {
        log.debug("Tool: getPointValueHistory(tenantId={}, deviceId={}, pointId={}, count={})", tenantId, deviceId, pointId, count);
        try {
    ...
    public String writePointValue(
        ...
        try {
            ...
            return success ? "Write command sent successfully for device " + deviceId + " point " + pointId + " value=" + value
