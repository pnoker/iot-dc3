...
public class DeviceTool {
    ...
    public Mono<AgenticToolResult<FacadeDeviceBO>> lookupDeviceByIdReactive(Long deviceId, ToolContext context) {
        ...
        return deviceFacade.getByIdReactive(tenantId, deviceId).map(value -> AgenticToolResult.ok("Device loaded", value))
                .defaultIfEmpty(AgenticToolResult.notFound("Device not found for ID: " + deviceId));
    }

    public Mono<AgenticToolResult<List<FacadeDeviceBO>>> lookupDevicesByIdsReactive(List<Long> deviceIds,
                                                                                      ToolContext context) {
        Long tenantId = AgenticToolContextUtil.requireTenantId(context);
        ...
        return deviceFacade.listByIdsReactive(tenantId, ids).collectList().map(values -> values.isEmpty()
                ? AgenticToolResult.empty("No devices found for IDs: " + ids, List.of())
                : AgenticToolResult.ok("Devices loaded", values));
    }

    public Mono<AgenticToolResult<OffsetPage<FacadeDeviceBO>>> searchDevicesReactive(String deviceName,
                                                                                       String deviceCode,
                                                                                       Long driverId, long offset,
        ...
        return deviceFacade.listReactive(new FacadeDeviceOffsetQuery(tenantId, deviceName, deviceCode, driverId,
                        null, null, null, null, null, offset, limit, List.of()))
                .map(page -> page.items().isEmpty() ? AgenticToolResult.empty("No devices found.", page)
                        : AgenticToolResult.ok("Device page loaded", page));
    }

    public Mono<AgenticToolResult<OffsetPage<FacadeDeviceBO>>> listDevicesByDriverIdReactive(Long driverId,
        ...
        if (offset < 0) return Mono.just(AgenticToolResult.invalid("Offset must be non-negative."));
        if (limit < 1 || limit > 200) return Mono.just(AgenticToolResult.invalid("Limit must be between 1 and 200."));
        return deviceFacade.listReactive(new FacadeDeviceOffsetQuery(tenantId, null, null, driverId, null,
                        null, null, null, null, offset, limit, List.of()))
                .map(page -> page.items().isEmpty() ? AgenticToolResult.empty("No devices found for driver ID: " + driverId, page)
                        : AgenticToolResult.ok("Device page loaded for driver " + driverId, page));
    }

    public Mono<AgenticToolResult<OffsetPage<FacadeDeviceBO>>> listDevicesByProfileIdReactive(Long profileId,
                                                                                                long offset, int limit,
                                                                                                ToolContext context) {
        Long tenantId = AgenticToolContextUtil.requireTenantId(context);
        ...
        if (offset < 0) return Mono.just(AgenticToolResult.invalid("Offset must be non-negative."));
        if (limit < 1 || limit > 200) return Mono.just(AgenticToolResult.invalid("Limit must be between 1 and 200."));
        return deviceFacade.listReactive(new FacadeDeviceOffsetQuery(tenantId, null, null, null, profileId,
                        null, null, null, null, offset, limit, List.of()))
                .map(page -> page.items().isEmpty() ? AgenticToolResult.empty("No devices found for profile ID: " + profileId, page)
                        : AgenticToolResult.ok("Device page loaded for profile " + profileId, page));
    }

    public Mono<AgenticToolResult<DeviceLatestPointValues>> getDeviceLatestPointValuesReactive(Long deviceId,
                                                                                                 int limit,
                                                                                                 ToolContext context) {
        Long tenantId = AgenticToolContextUtil.requireTenantId(context);
        if (deviceId == null || deviceId <= 0) return Mono.just(AgenticToolResult.invalid("Device ID must be positive."));
        if (limit < 1 || limit > AgenticConstant.ToolLimit.MAX_IDS) {
            return Mono.just(AgenticToolResult.invalid("Limit must be between 1 and " + AgenticConstant.ToolLimit.MAX_IDS + "."));
        }
        int safeLimit = limit;
        return deviceFacade.getByIdReactive(tenantId, deviceId)
                .flatMap(device -> pointFacade.listReactive(new FacadePointOffsetQuery(tenantId, null, null, null,
                                null, null, null, null, null, null, deviceId, 0, safeLimit, List.of()))
                        .flatMapMany(page -> Flux.fromIterable(page.items()))
                        .flatMap(point -> pointValueFacade.lastValue(tenantId, deviceId, point.getId())
                                .map(value -> new PointLatestValue(point, value))
                                .defaultIfEmpty(new PointLatestValue(point, null)), 8)
                        .collectList()
                        .map(values -> values.isEmpty()
                                ? AgenticToolResult.empty("No points found for device " + deviceId,
                                new DeviceLatestPointValues(device, List.of()))
                                : AgenticToolResult.ok("Device latest point values loaded",
