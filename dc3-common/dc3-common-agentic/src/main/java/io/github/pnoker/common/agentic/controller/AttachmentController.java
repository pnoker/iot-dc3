...
public class AttachmentController implements BaseController {
    ...
    @PostMapping("/upload")
    public Mono<R<AttachmentVO>> upload(@RequestBody AttachmentUploadRequest request) {
        return getUserHeader().flatMap(header -> async(() -> {
            request.setConversationId(AgenticConversationIds.scope(header.getTenantId(), header.getUserId(),
                    request.getConversationId()));
            AttachmentVO attachment = attachmentService.upload(request, header);
    ...
    public Mono<R<List<AttachmentVO>>> list(@PathVariable String conversationId) {
        return getUserHeader().flatMap(header -> async(() -> {
            String scopedConversationId = AgenticConversationIds.scope(header.getTenantId(), header.getUserId(),
                    conversationId);
            List<AttachmentVO> attachments = attachmentService.list(scopedConversationId, header);
