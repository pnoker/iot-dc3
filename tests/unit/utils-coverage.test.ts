...
describe('utility coverage guard', () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  ...
  it('routes dashboard jumps to the expected named routes', async () => {
    ...
    jumpToEntity(router as never, 'driver', 1);
    jumpToEntity(router as never, 'device', 2);
    jumpToEntity(router as never, 'profile', 3);
    jumpToEntity(router as never, 'point', 4);
    jumpToEntity(router as never, 'driver', '');
    jumpToSourceEvents(router as never, 'device', 5);
    jumpToSourceEvents(router as never, 'driver', 6);
