...
import { isEmail, isNull, isPhone, isUrl } from '@/utils/validationUtil';
...
describe('format and validation helpers', () => {
  ...
  it('keeps legacy validation behavior explicit', () => {
    expect(isUrl('https://dc3.example')).toBe(true);
    expect(isUrl('ftp://dc3.example')).toBe(false);
    expect(isEmail('dc3@example.com')).toBe(true);
...
describe('menu title resolution', () => {
