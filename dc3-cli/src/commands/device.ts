...
async function waitForOperation(
  ...
  while (true) {
    const operation = await dc3Client.get<OperationView>(
      operationPath(accepted.statusUri),
    );
...
export function registerDeviceCommand(program: Command): void {
  ...
  device
    ...
    .action(async (id, opts) => {
      ...
      const result = await dc3Client.get(
        `${DEVICE_BASE}/get_by_id?id=${encodeURIComponent(id)}`,
      );
      printAndExit(result, format);
    });
  ...
  device
    .command("import <file>")
    .description(
      "Submit an XLSX device import and wait for its durable operation",
    )
    ...
    .option(
      "--idempotency-key <key>",
      "Stable key for safe retries (defaults to a UUIDv4)",
    )
    ...
    .action(async (file: string, opts) => {
      ...
      if (!file.toLowerCase().endsWith(".xlsx")) {
        printAndExit(
          { ok: false, message: "Import file must use the .xlsx extension" },
          "json",
          1,
        );
        return;
      ...
      if (content.length === 0) {
        printAndExit(
          { ok: false, message: "Import file must not be empty" },
          "json",
          1,
        );
        return;
      ...
      const operation = await waitForOperation(
        accepted,
        Math.max(100, Number(opts.pollInterval)),
      );
