...
class SignedCursorCodecTest {
    ...
    void roundTripsAnAuthenticatedTenantAndQueryBoundCursor() {
        ...
        String token = codec.encode(new CursorState(
                "k1", TENANT_ID, QUERY_DIGEST, position, NOW.plusSeconds(300)));
    ...
    void preservesTheCanonicalMicrosecondExpiryPrecision() {
        ...
        String token = codec.encode(new CursorState(
                "k1", TENANT_ID, QUERY_DIGEST, new byte[]{1}, expiry));
    ...
    void rejectsTamperingCrossTenantReuseQueryReuseAndExpiry() {
        String token = codec.encode(new CursorState(
                "k1", TENANT_ID, QUERY_DIGEST, new byte[]{1}, NOW.plusSeconds(1)));
        ...
        assertThrows(IllegalArgumentException.class,
                () -> codec.decode(token, UUID.randomUUID(), QUERY_DIGEST));
        assertThrows(IllegalArgumentException.class,
                () -> codec.decode(token, TENANT_ID, sha256("another query")));

        SignedCursorCodec expiredCodec = new SignedCursorCodec(
                Map.of("k1", new SecretKeySpec(new byte[32], "HmacSHA256")),
                Clock.fixed(NOW.plusSeconds(1), ZoneOffset.UTC));
        assertThrows(IllegalArgumentException.class,
                () -> expiredCodec.decode(token, TENANT_ID, QUERY_DIGEST));
    }

    @Test
    void mapsMalformedExpiryPayloadsToInvalidCursor() {
        String token = codec.encode(new CursorState(
                "k1", TENANT_ID, QUERY_DIGEST, new byte[]{1}, NOW.plusSeconds(300)));
        ...
        try {
            ...
            assertThrows(IllegalArgumentException.class,
                    () -> codec.decode(malformed, TENANT_ID, QUERY_DIGEST));
    ...
    void rejectsNonCanonicalBase64Representations() {
        ...
        assertThrows(IllegalArgumentException.class,
                () -> codec.decode(padded + "." + parts[1], TENANT_ID, QUERY_DIGEST));
    }
