...
/** Reactive persistence port for rule records. */
@Repository
...
public class R2dbcRuleStore implements ReactiveRuleStore {
    ...
    private static final String COLUMNS =
            "id,alarm_target_type_flag,rule_name,rule_code,entity_id,notify_id,message_id,rule_ext,enable_flag,tenant_id,remark,creator_id,creator_name,create_time,operator_id,operator_name,operate_time,deleted";
    ...
    public Flux<RuleBO> listEnabledCandidates(long tenantId, AlarmTargetTypeEnum targetType, long entityId) {
        ...
        return databaseClient
                ...
                .bind("target_type", targetType.getIndex())
                .bind("entity_id", entityId)
                .map((row, metadata) -> ruleBuilder.buildBOByDO(map(row)))
                .all();
    }
    ...
    @Override
    public Mono<OffsetPage<RuleDO>> list(
            long tenantId,
            String ruleName,
            String ruleCode,
            Long entityId,
        ...
        DatabaseClient.GenericExecuteSpec count = databaseClient
                .sql("SELECT COUNT(*) AS total FROM " + TABLE + predicates)
                .bind("tenant_id", tenantId);
        DatabaseClient.GenericExecuteSpec rows = databaseClient
                .sql("SELECT " + COLUMNS + " FROM " + TABLE + predicates + " ORDER BY " + orderBy(pageRequest.sort())
        ...
        return total.flatMap(totalCount -> itemRows.map((row, metadata) -> map(row))
                        .all()
                        .collectList()
    ...
    public Mono<RuleDO> insert(RuleDO rule) {
        if (rule == null || !valid(rule.getTenantId()))
        ...
        String sql = "INSERT INTO " + TABLE + " (" + COLUMNS
                + ") VALUES (:id,:alarm_target_type_flag,:rule_name,:rule_code,:entity_id,:notify_id,:message_id,"
                + dialect.jsonWriteExpression(":rule_ext")
        ...
        DatabaseClient.GenericExecuteSpec spec = databaseClient
                .sql(sql)
                .bind("id", rule.getId())
                .bind("alarm_target_type_flag", value(rule.getAlarmTargetTypeFlag()))
                .bind("rule_name", text(rule.getRuleName()))
                .bind("rule_code", text(rule.getRuleCode()))
                .bind("entity_id", value(rule.getEntityId()))
                .bind("notify_id", value(rule.getNotifyId()))
                .bind("message_id", value(rule.getMessageId()))
                .bind("enable_flag", value(rule.getEnableFlag()))
                .bind("tenant_id", rule.getTenantId())
                .bind("remark", text(rule.getRemark()))
                .bind("creator_id", value(rule.getCreatorId()))
                .bind("creator_name", text(rule.getCreatorName()))
                .bind("create_time", rule.getCreateTime())
                .bind("operator_id", value(rule.getOperatorId()))
                .bind("operator_name", text(rule.getOperatorName()))
                .bind("operate_time", rule.getOperateTime());
        spec = bindNullable(
                spec,
                "rule_ext",
                rule.getRuleExt() == null ? null : JsonUtil.toJsonString(rule.getRuleExt()),
                String.class);
        return transactionalOperator
                .transactional(spec.fetch().rowsUpdated())
                .flatMap(rows -> rows == 1
                        ? get(rule.getTenantId(), rule.getId())
    ...
    public Mono<RuleDO> update(RuleDO rule) {
        ...
        String sql = "UPDATE " + TABLE
                + " SET alarm_target_type_flag=:alarm_target_type_flag,rule_name=:rule_name,rule_code=:rule_code,entity_id=:entity_id,notify_id=:notify_id,message_id=:message_id,rule_ext="
                + dialect.jsonWriteExpression(":rule_ext")
    ...
    public Mono<Boolean> existsActiveCode(long tenantId, String ruleCode, Long excludedId) {
        ...
        DatabaseClient.GenericExecuteSpec spec =
                databaseClient.sql(sql).bind("tenant_id", tenantId).bind("rule_code", ruleCode);
    ...
    private RuleDO map(io.r2dbc.spi.Row row) {
        ...
        value.setRuleExt(
                ruleExt == null
                        ? null
                        : JsonUtil.parseObject(ruleExt, io.github.pnoker.common.entity.ext.JsonExt.class));
        ...
        for (SortSpec spec : sort) {
            String column =
                    switch (spec.field()) {
                        ...
                        default ->
                            throw new IllegalArgumentException("rule sort field is not allowed: " + spec.field());
                    };
    ...
    private byte value(AlarmTargetTypeEnum value) {
        return value == null ? 0 : value.getIndex();
    }

    private byte value(EnableFlagEnum value) {
        return value == null ? 0 : value.getIndex();
    }
