...
public class R2dbcCommandHistoryStore implements ReactiveCommandHistoryStore {
    ...
    public Mono<CommandHistoryDO> insert(CommandHistoryDO history) {
        if (history == null
                || history.getTenantId() == null
                || history.getRecordId() == null
        ...
        DatabaseClient.GenericExecuteSpec spec = databaseClient
                ...
                .bind("record_id", history.getRecordId())
                .bind("tenant_id", history.getTenantId())
                .bind("device_id", history.getDeviceId())
                .bind("command_id", history.getCommandId())
    ...
    @Override
    public Mono<OffsetPage<CommandHistoryDO>> list(
            Long tenantId,
            Long deviceId,
            Long commandId,
            String commandCode,
        ...
        if (tenantId == null || tenantId <= 0)
            return Mono.just(OffsetPage.of(List.of(), 0, PageRequest.DEFAULT_LIMIT, 0));
        ...
        DatabaseClient.GenericExecuteSpec count = bindFilters(
                databaseClient.sql("SELECT COUNT(*) AS total" + base),
                tenantId,
                deviceId,
                commandId,
                commandCode,
        ...
        DatabaseClient.GenericExecuteSpec rows = bindFilters(
                        databaseClient.sql("SELECT " + COLUMNS + base + " ORDER BY " + orderBy(sort)
    ...
    private Mono<Boolean> update(
        ...
        DatabaseClient.GenericExecuteSpec spec = databaseClient
                ...
                .bind("record_id", recordId);
        if (bindStates)
            spec = spec.bind("pending", PointCommandStatusEnum.PENDING.getIndex())
    ...
    private CommandHistoryDO map(io.r2dbc.spi.Row row, io.r2dbc.spi.RowMetadata metadata) {
        ...
        value.setResultValues(text(row.get("result_values", String.class)));
    ...
    private Byte status(PointCommandStatusEnum value) {
        return value == null ? PointCommandStatusEnum.PENDING.getIndex() : value.getIndex();
    }

    private Byte source(CommandHistorySourceEnum value) {
