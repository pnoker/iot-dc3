...
public class R2dbcNotifyHistoryStore implements ReactiveNotifyHistoryStore {
    ...
    private static final String COLUMNS =
            "id,rule_id,notify_id,message_id,channel_id,alarm_id,dedupe_key,channel_type_flag,target,"
    ...
    @Override
    public Mono<OffsetPage<NotifyHistoryDO>> list(
            long tenantId,
            Long ruleId,
            Long notifyId,
            Long messageId,
            Long channelId,
            Long alarmId,
            io.github.pnoker.common.enums.NotifyChannelTypeEnum channelTypeFlag,
            String target,
            io.github.pnoker.common.enums.NotifyHistoryStatusEnum statusFlag,
            PageRequest page) {
        ...
        if (notifyId != null) {
            where.append(" AND notify_id=:notify_id");
            names.add("notify_id");
        }
        if (messageId != null) {
            where.append(" AND message_id=:message_id");
            names.add("message_id");
        }
        if (channelId != null) {
            where.append(" AND channel_id=:channel_id");
            names.add("channel_id");
        }
        if (alarmId != null) {
            where.append(" AND alarm_id=:alarm_id");
            names.add("alarm_id");
        }
        if (channelTypeFlag != null) {
            where.append(" AND channel_type_flag=:channel_type");
            names.add("channel_type");
        }
        if (target != null && !target.isBlank()) {
            where.append(" AND target LIKE :target");
            names.add("target");
        }
        if (statusFlag != null) {
            where.append(" AND status_flag=:status");
            names.add("status");
        }
    ...
    public Mono<NotifyHistoryInsertResult> insertIdempotent(NotifyHistoryDO history) {
        ...
        String baseSql = "INSERT INTO " + TABLE + " (" + COLUMNS
                + ") VALUES (:id,:rule_id,:notify_id,:message_id,:channel_id,:alarm_id,:dedupe_key,:channel_type_flag,:target,:status_flag,"
                + dialect.jsonWriteExpression(":request_ext") + "," + dialect.jsonWriteExpression(":response_ext")
    ...
    @Override
    public Mono<Boolean> updateDelivery(
            long tenantId,
            long historyId,
            byte statusFlag,
            String target,
        ...
        String sql = "UPDATE " + TABLE + " SET status_flag=:status_flag,target=:target,response_ext="
                + dialect.jsonWriteExpression(":response_ext")
                + ",error_message=:error_message,retry_count=:retry_count"
                + " WHERE tenant_id=:tenant_id AND id=:id";
        DatabaseClient.GenericExecuteSpec spec = databaseClient
                .sql(sql)
                .bind("status_flag", statusFlag)
                .bind("target", text(target))
                .bind("error_message", text(errorMessage))
                .bind("retry_count", retryCount)
                .bind("tenant_id", tenantId)
                .bind("id", historyId);
    ...
    private NotifyHistoryDO map(io.r2dbc.spi.Row row, io.r2dbc.spi.RowMetadata metadata) {
        ...
        value.setNotifyId(row.get("notify_id", Long.class));
        value.setMessageId(row.get("message_id", Long.class));
        value.setChannelId(row.get("channel_id", Long.class));
    ...
    private String serializeJsonOrEmpty(Object value) {
        return value == null ? "{}" : serializeJson(value);
    }
    ...
    private DatabaseClient.GenericExecuteSpec insertSpec(NotifyHistoryDO history, String sql) {
        DatabaseClient.GenericExecuteSpec spec = databaseClient
                .sql(sql)
                .bind("id", history.getId())
                .bind("rule_id", value(history.getRuleId()))
                .bind("notify_id", value(history.getNotifyId()))
                .bind("message_id", value(history.getMessageId()))
                .bind("channel_id", value(history.getChannelId()))
                .bind("alarm_id", value(history.getAlarmId()))
                .bind("channel_type_flag", value(history.getChannelTypeFlag()))
                .bind("target", text(history.getTarget()))
                .bind("status_flag", value(history.getStatusFlag()))
                .bind("error_message", text(history.getErrorMessage()))
                .bind("retry_count", history.getRetryCount())
                .bind("tenant_id", history.getTenantId())
                .bind("remark", text(history.getRemark()))
                .bind("creator_id", value(history.getCreatorId()))
                .bind("creator_name", text(history.getCreatorName()))
                .bind("operator_id", value(history.getOperatorId()))
                .bind("operator_name", text(history.getOperatorName()));
