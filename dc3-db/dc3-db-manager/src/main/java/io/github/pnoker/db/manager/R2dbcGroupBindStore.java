...
public class R2dbcGroupBindStore implements ReactiveGroupBindStore {
    private static final String T = "dc3_manager.dc3_group_bind",
            C =
                    "id,entity_type_flag,group_id,entity_id,tenant_id,remark,creator_id,creator_name,create_time,operator_id,operator_name,operate_time";
    ...
    public Mono<OffsetPage<GroupBindBO>> list(BindingFilter f) {
        ...
        if (f.ownerId() != null) w.append(" AND group_id=:owner");
        if (f.entityId() != null) w.append(" AND entity_id=:entity");
        Mono<List<GroupBindBO>> i = bind(
    ...
    public Mono<GroupBindBO> getByEntity(Long t, byte type, Long entity) {
        return c.sql(
                        "SELECT " + C + " FROM " + T
                                + " WHERE tenant_id=:tenant AND entity_type_flag=:type AND entity_id=:entity AND deleted=0 LIMIT 1")
                .bind("tenant", t)
                .bind("type", type)
                .bind("entity", entity)
                .map(this::map)
    ...
    public Mono<GroupBindBO> insert(GroupBindBO v) {
        ...
        String s = "INSERT INTO " + T
                + " (id,entity_type_flag,group_id,entity_id,tenant_id,remark,creator_id,creator_name,create_time,operator_id,operator_name,operate_time,deleted) VALUES (:id,:type,:owner,:entity,:tenant,:remark,:creator_id,:creator_name,:create_time,:operator_id,:operator_name,:operate_time,0)";
    ...
    public Mono<GroupBindBO> update(GroupBindBO v) {
        ...
        String s = "UPDATE " + T
                + " SET entity_type_flag=:type,group_id=:owner,entity_id=:entity,remark=:remark,operator_id=:operator_id,operator_name=:operator_name,operate_time=:operate_time WHERE tenant_id=:tenant AND id=:id AND deleted=0";
    ...
    private DatabaseClient.GenericExecuteSpec write(String s, GroupBindBO v, boolean in) {
        DatabaseClient.GenericExecuteSpec q = c.sql(s)
                .bind("id", v.getId())
                .bind("type", v.getEntityTypeFlag().getIndex())
                .bind("owner", v.getGroupId())
    ...
    private GroupBindBO map(io.r2dbc.spi.Row r, io.r2dbc.spi.RowMetadata m) {
        GroupBindBO v = new GroupBindBO();
        v.setId(r.get("id", Long.class));
        v.setEntityTypeFlag(
                EntityTypeEnum.ofIndex(num(r.get("entity_type_flag")).byteValue()));
        v.setGroupId(r.get("group_id", Long.class));
