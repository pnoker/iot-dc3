...
/** Explicit SQL adapter for manager point attributes. */
@Repository
@ConditionalOnClass({DatabaseClient.class, TransactionalOperator.class})
@RequiredArgsConstructor
public class R2dbcPointAttributeStore implements ReactivePointAttributeStore {
    private static final String TABLE = "dc3_manager.dc3_point_attribute";
    ...
    @Override
    public Mono<PointAttributeBO> get(Long tenantId, Long id) {
    ...
    @Override
    public Mono<PointAttributeBO> getByCodeAndDriver(Long tenantId, String attributeCode, Long driverId) {
    ...
    @Override
    public Flux<PointAttributeBO> listByDriverId(Long tenantId, Long driverId) {
    ...
    @Override
    public Mono<PointAttributeBO> insert(PointAttributeBO value) {
        if (value == null
    ...
    @Override
    public Mono<PointAttributeBO> update(PointAttributeBO value, int expectedVersion) {
        DatabaseClient.GenericExecuteSpec spec = databaseClient
                .sql(
                        "UPDATE " + TABLE
    ...
    @Override
    public Mono<OffsetPage<PointAttributeBO>> list(PointAttributeFilter filter) {
    ...
    private DatabaseClient.GenericExecuteSpec bind(
            DatabaseClient.GenericExecuteSpec spec, PointAttributeFilter filter) {
            ...
            for (SortSpec spec : sort) {
                String column =
                        switch (spec.field()) {
                            ...
                            default -> throw new IllegalArgumentException("unsupported point attribute sort field");
                        };
    ...
    private PointAttributeBO map(io.r2dbc.spi.Row row, io.r2dbc.spi.RowMetadata metadata) {
            ...
            try {
                ...
                ext.setContent(
                        json.getContent() == null
                                ? null
                                : JsonUtil.parseObject(json.getContent(), PointAttributeExt.Content.class));
    ...
    private String serialize(PointAttributeExt value) {
        try {
