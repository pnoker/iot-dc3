...
public class R2dbcCommandAttributeConfigStore implements ReactiveCommandAttributeConfigStore {
    ...
    private static final String COLUMNS =
            "id, attribute_id, config_value, device_id, config_ext, command_id, enable_flag, tenant_id, remark, signature, version, creator_id, creator_name, create_time, operator_id, operator_name, operate_time";
    ...
    @Override
    public Mono<CommandAttributeConfigBO> get(Long tenantId, Long id) {
    ...
    @Override
    public Mono<CommandAttributeConfigBO> getByAttributeDeviceCommand(
        ...
        return databaseClient
                .sql(
                        "SELECT " + COLUMNS + " FROM " + TABLE
                                + " WHERE tenant_id=:tenant_id AND attribute_id=:attribute_id AND device_id=:device_id AND command_id=:command_id AND deleted=0 LIMIT 1")
                .bind("tenant_id", tenantId)
    ...
    public Flux<CommandAttributeConfigBO> listByDeviceIdAndCommandId(Long tenantId, Long deviceId, Long commandId) {
        if (tenantId == null
                ...
                || tenantId <= 0
                || deviceId <= 0
                || commandId <= 0) return Flux.empty();
        return databaseClient
                .sql(
                        "SELECT " + COLUMNS + " FROM " + TABLE
                                + " WHERE tenant_id=:tenant_id AND device_id=:device_id AND command_id=:command_id AND deleted=0 ORDER BY id ASC")
                .bind("tenant_id", tenantId)
                .bind("device_id", deviceId)
                .bind("command_id", commandId)
                .map(this::map)
    ...
    @Override
    public Mono<CommandAttributeConfigBO> insert(CommandAttributeConfigBO value) {
        if (value == null
            ...
            return Mono.error(
                    new IllegalArgumentException("tenantId, attributeId, deviceId and commandId are required"));
        ...
        return transactionalOperator.transactional(query.fetch()
                ...
                .flatMap(rows -> rows == 1
                        ...
                        : Mono.error(new IllegalStateException(
                                "command attribute config insert affected " + rows + " rows"))));
    }

    @Override
    public Mono<CommandAttributeConfigBO> update(CommandAttributeConfigBO value, int expectedVersion) {
        if (value == null || value.getTenantId() == null || value.getId() == null)
            return Mono.error(new IllegalArgumentException("tenantId and config id are required"));
        DatabaseClient.GenericExecuteSpec query = databaseClient
                .sql(
                        "UPDATE " + TABLE
    ...
    public Mono<OffsetPage<CommandAttributeConfigBO>> list(CommandAttributeConfigFilter filter) {
        ...
        if (filter.commandId() != null) where.append(" AND c.command_id=:command_id");
    ...
    private DatabaseClient.GenericExecuteSpec bind(
            DatabaseClient.GenericExecuteSpec spec, CommandAttributeConfigFilter filter) {
            ...
            for (SortSpec spec : sort) {
                String column =
                        switch (spec.field()) {
                            ...
                            case "commandId" -> "c.command_id";
                            case "createTime" -> "c.create_time";
                            case "operateTime" -> "c.operate_time";
                            case "version" -> "c.version";
                            default ->
                                throw new IllegalArgumentException("unsupported command attribute config sort field");
                        };
                clauses.add(column + (spec.direction() == SortSpec.Direction.DESC ? " DESC" : " ASC"));
            }
        if (clauses.stream().noneMatch(v -> v.startsWith("c.id"))) clauses.add("c.id ASC");
    ...
    private CommandAttributeConfigBO map(io.r2dbc.spi.Row row, io.r2dbc.spi.RowMetadata metadata) {
