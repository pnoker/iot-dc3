...
public class R2dbcPointAttributeConfigStore implements ReactivePointAttributeConfigStore {
    ...
    private static final String COLUMNS =
            "id, attribute_id, config_value, device_id, config_ext, point_id, enable_flag, tenant_id, remark, signature, version, creator_id, creator_name, create_time, operator_id, operator_name, operate_time";
    ...
    @Override
    public Mono<PointAttributeConfigBO> get(Long tenantId, Long id) {
    ...
    @Override
    public Mono<PointAttributeConfigBO> getByAttributeDevicePoint(
        ...
        return databaseClient
                .sql(
                        "SELECT " + COLUMNS + " FROM " + TABLE
                                + " WHERE tenant_id=:tenant_id AND attribute_id=:attribute_id AND device_id=:device_id AND point_id=:point_id AND deleted=0 LIMIT 1")
                .bind("tenant_id", tenantId)
    ...
    @Override
    public Flux<PointAttributeConfigBO> listByDeviceId(Long tenantId, Long deviceId) {
    ...
    @Override
    public Flux<PointAttributeConfigBO> listByDeviceIdAndPointId(Long tenantId, Long deviceId, Long pointId) {
        if (!valid(tenantId) || !valid(deviceId) || !valid(pointId)) return Flux.empty();
        return databaseClient
                .sql(
                        "SELECT " + COLUMNS + " FROM " + TABLE
                                + " WHERE tenant_id=:tenant_id AND device_id=:device_id AND point_id=:point_id AND deleted=0 ORDER BY id ASC")
                .bind("tenant_id", tenantId)
                .bind("device_id", deviceId)
                .bind("point_id", pointId)
                .map(this::map)
    ...
    @Override
    public Mono<PointAttributeConfigBO> insert(PointAttributeConfigBO value) {
        if (value == null
                || !valid(value.getTenantId())
                || !valid(value.getAttributeId())
                || !valid(value.getDeviceId())
                || !valid(value.getPointId())) {
        ...
        String sql = "INSERT INTO " + TABLE + " (" + COLUMNS
                ...
                + ",:point_id,:enable_flag,:tenant_id,:remark,:signature,:version,:creator_id,:creator_name,:create_time,:operator_id,:operator_name,:operate_time,0)";
        ...
        return transactionalOperator.transactional(query.fetch()
                ...
                .flatMap(rows -> rows == 1
                        ...
                        : Mono.error(new IllegalStateException(
                                "point attribute config insert affected " + rows + " rows"))));
    }

    @Override
    public Mono<PointAttributeConfigBO> update(PointAttributeConfigBO value, int expectedVersion) {
        if (value == null
                || !valid(value.getTenantId())
                || !valid(value.getId())
                || !valid(value.getAttributeId())
                ...
                || !valid(value.getPointId())) {
            return Mono.error(
                    new IllegalArgumentException("tenantId, id, attributeId, deviceId and pointId are required"));
        }
        String sql = "UPDATE " + TABLE
                + " SET attribute_id=:attribute_id, config_value=:config_value, device_id=:device_id, config_ext="
                + dialect.jsonWriteExpression(":config_ext")
                + ", point_id=:point_id, enable_flag=:enable_flag, remark=:remark, signature=:signature, version=version+1, operator_id=:operator_id, operator_name=:operator_name, operate_time=:operate_time WHERE tenant_id=:tenant_id AND id=:id AND version=:expected_version AND deleted=0";
        DatabaseClient.GenericExecuteSpec query = databaseClient
                .sql(sql)
                .bind("attribute_id", value.getAttributeId())
                .bind("config_value", empty(value.getConfigValue()))
    ...
    @Override
    public Mono<OffsetPage<PointAttributeConfigBO>> list(PointAttributeConfigFilter filter) {
    ...
    private DatabaseClient.GenericExecuteSpec bind(
            DatabaseClient.GenericExecuteSpec spec, PointAttributeConfigFilter filter) {
            ...
            for (SortSpec spec : sort) {
                String column =
                        switch (spec.field()) {
                            ...
                            case "pointId" -> "c.point_id";
                            case "createTime" -> "c.create_time";
                            case "operateTime" -> "c.operate_time";
                            case "version" -> "c.version";
                            default ->
                                throw new IllegalArgumentException("unsupported point attribute config sort field");
                        };
