...
public class R2dbcLabelStore implements ReactiveLabelStore {
    ...
    private static final String COLUMNS =
            "id,label_name,label_code,label_color,entity_type_flag,enable_flag,tenant_id,remark,creator_id,creator_name,create_time,operator_id,operator_name,operate_time";
    ...
    public Mono<OffsetPage<LabelBO>> list(LabelFilter filter) {
        ...
        if (text(filter.labelName()))
            where.append(" AND ").append(dialect.caseInsensitiveLike("label_name", ":label_name"));
    ...
    public Mono<LabelBO> getByName(Long tenantId, String name, byte entityType) {
        ...
        return client.sql(
                        "SELECT " + COLUMNS + " FROM " + TABLE
                                + " WHERE tenant_id=:tenant_id AND label_name=:label_name AND entity_type_flag=:entity_type AND deleted=0 LIMIT 1")
                .bind("tenant_id", tenantId)
                .bind("label_name", name)
    ...
    public Mono<Boolean> hasActiveBindings(Long tenantId, Long labelId) {
        ...
        return client.sql(
                        "SELECT 1 FROM dc3_manager.dc3_label_bind WHERE tenant_id=:tenant_id AND label_id=:label_id AND deleted=0 LIMIT 1")
                .bind("tenant_id", tenantId)
    ...
    public Mono<LabelBO> insert(LabelBO value) {
        ...
        return transactionalOperator.transactional(write(sql, value)
                .fetch()
                .rowsUpdated()
    ...
    public Mono<LabelBO> update(LabelBO value) {
        ...
        String sql = "UPDATE " + TABLE
                + " SET label_name=:label_name,label_code=:label_code,label_color=:label_color,entity_type_flag=:entity_type,enable_flag=:enable_flag,remark=:remark,operator_id=:operator_id,operator_name=:operator_name,operate_time=:operate_time WHERE tenant_id=:tenant_id AND id=:id AND deleted=0";
        return transactionalOperator.transactional(write(sql, value)
                .fetch()
                .rowsUpdated()
                .flatMap(rows -> rows == 1 ? get(value.getTenantId(), value.getId()) : Mono.empty()));
    ...
    private DatabaseClient.GenericExecuteSpec bind(DatabaseClient.GenericExecuteSpec spec, LabelFilter filter) {
        ...
        if (text(filter.labelName()))
            spec = spec.bind("label_name", "%" + filter.labelName().trim() + "%");
        if (text(filter.color())) spec = spec.bind("color", filter.color().trim());
        if (filter.entityTypeFlag() != null)
            spec = spec.bind("entity_type", filter.entityTypeFlag().getIndex());
    ...
    private DatabaseClient.GenericExecuteSpec write(String sql, LabelBO value) {
        DatabaseClient.GenericExecuteSpec spec = client.sql(sql)
                .bind("id", value.getId())
                .bind("label_name", value.getLabelName())
                .bind("label_code", value.getLabelCode())
                .bind("label_color", value.getLabelColor())
                .bind("entity_type", value.getEntityTypeFlag().getIndex())
                .bind("enable_flag", flag(value.getEnableFlag()))
                ...
                .bind("operate_time", dialect.bindInstant(toInstant(value.getOperateTime())));
    ...
    private LabelBO map(io.r2dbc.spi.Row row, io.r2dbc.spi.RowMetadata metadata) {
        LabelBO value = new LabelBO();
        value.setId(row.get("id", Long.class));
        value.setLabelName(row.get("label_name", String.class));
        value.setLabelCode(row.get("label_code", String.class));
        value.setLabelColor(row.get("label_color", String.class));
        value.setEntityTypeFlag(
                EntityTypeEnum.ofIndex(number(row.get("entity_type_flag")).byteValue()));
        value.setEnableFlag(
                EnableFlagEnum.ofIndex(number(row.get("enable_flag")).byteValue()));
        ...
        for (SortSpec spec : sort) {
            String column =
                    switch (spec.field()) {
                        ...
                        case "id" -> "id";
                        default -> throw new IllegalArgumentException("label sort field is not allowed");
                    };
            clauses.add(column + (spec.direction() == SortSpec.Direction.ASC ? " ASC" : " DESC"));
        }
        if (clauses.stream().noneMatch(v -> v.startsWith("id "))) clauses.add("id DESC");
