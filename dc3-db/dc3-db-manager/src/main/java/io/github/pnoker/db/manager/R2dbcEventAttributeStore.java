...
/** Explicit SQL adapter for manager event attributes. */
@Repository
@ConditionalOnClass({DatabaseClient.class, TransactionalOperator.class})
@RequiredArgsConstructor
public class R2dbcEventAttributeStore implements ReactiveEventAttributeStore {
    private static final String TABLE = "dc3_manager.dc3_event_attribute";
    ...
    @Override
    public Mono<EventAttributeBO> get(Long tenantId, Long id) {
    ...
    @Override
    public Mono<EventAttributeBO> getByCodeAndDriver(Long tenantId, String attributeCode, Long driverId) {
    ...
    @Override
    public Mono<EventAttributeBO> insert(EventAttributeBO value) {
        if (value == null
    ...
    @Override
    public Mono<EventAttributeBO> update(EventAttributeBO value, int expectedVersion) {
        DatabaseClient.GenericExecuteSpec spec = databaseClient
                .sql(
                        "UPDATE " + TABLE
    ...
    private DatabaseClient.GenericExecuteSpec bind(
            DatabaseClient.GenericExecuteSpec spec, EventAttributeFilter filter) {
            ...
            for (SortSpec spec : sort) {
                String column =
                        switch (spec.field()) {
                            ...
                            default -> throw new IllegalArgumentException("unsupported event attribute sort field");
                        };
    ...
    private EventAttributeBO map(io.r2dbc.spi.Row row, io.r2dbc.spi.RowMetadata metadata) {
            ...
            try {
                ...
                ext.setContent(
                        json.getContent() == null
                                ? null
                                : JsonUtil.parseObject(json.getContent(), EventAttributeExt.Content.class));
    ...
    private String serialize(EventAttributeExt value) {
        try {
