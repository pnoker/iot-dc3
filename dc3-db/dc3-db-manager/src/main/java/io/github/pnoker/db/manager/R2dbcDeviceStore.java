...
public class R2dbcDeviceStore implements ReactiveDeviceStore {
    ...
    private static final String COLUMNS =
            "id, device_name, device_code, driver_id, profile_id, device_ext, enable_flag,"
    ...
    public Mono<OffsetPage<DeviceBO>> list(DeviceFilter filter) {
        ...
        if (filter.deviceName() != null && !filter.deviceName().isBlank())
            optional.add(" AND d.device_name LIKE :device_name");
        if (filter.deviceCode() != null && !filter.deviceCode().isBlank())
            optional.add(" AND d.device_code = :device_code");
        ...
        if (filter.groupId() != null)
            optional.add(" AND EXISTS (SELECT 1 FROM dc3_manager.dc3_group_bind dgb"
        ...
        if (filter.labelId() != null)
            optional.add(" AND EXISTS (SELECT 1 FROM dc3_manager.dc3_label_bind dlb"
        ...
        DatabaseClient.GenericExecuteSpec count = databaseClient
                .sql("SELECT COUNT(*) AS total FROM " + from + predicates)
                .bind("tenant_id", filter.tenantId());
        DatabaseClient.GenericExecuteSpec rows = databaseClient
                .sql("SELECT " + COLUMNS + " FROM " + from + predicates + " ORDER BY " + orderBy(filter.sort())
    ...
    public Mono<DeviceBO> insert(DeviceBO device) {
        DeviceBO value = prepare(device, device.getId() == null ? UuidV7.nextLong() : device.getId(), false);
        DatabaseClient.GenericExecuteSpec insert = databaseClient
                .sql("INSERT INTO " + TABLE
                        ...
                        + " " + dialect.jsonWriteExpression(":device_ext")
                ...
                .bind("id", value.getId())
                .bind("device_name", value.getDeviceName())
                .bind("device_code", value.getDeviceCode())
                ...
                .bind("signature", value.getSignature())
        ...
        Mono<DeviceBO> write = insert.fetch()
                .rowsUpdated()
                .flatMap(rows -> rows == 1
                        ? get(value.getTenantId(), value.getId())
                        : Mono.error(new IllegalStateException("device insert affected " + rows + " rows")));
    ...
    public Mono<DeviceBO> update(DeviceBO device, int expectedVersion) {
        ...
        DatabaseClient.GenericExecuteSpec update = databaseClient
                .sql("UPDATE " + TABLE + " SET device_name = :device_name, driver_id = :driver_id,"
                        + " profile_id = :profile_id, device_ext = " + dialect.jsonWriteExpression(":device_ext")
        ...
        Mono<DeviceBO> write = update.fetch()
                .rowsUpdated()
    ...
    public Mono<Boolean> delete(Long tenantId, Long id, int expectedVersion, Long operatorId, String operatorName) {
        ...
        DatabaseClient.GenericExecuteSpec write = databaseClient
    ...
    private DatabaseClient.GenericExecuteSpec bindOptional(
            DatabaseClient.GenericExecuteSpec spec, DeviceFilter filter) {
        if (filter.deviceName() != null && !filter.deviceName().isBlank())
            spec = spec.bind("device_name", "%" + filter.deviceName().trim() + "%");
        if (filter.deviceCode() != null && !filter.deviceCode().isBlank())
            spec = spec.bind("device_code", filter.deviceCode().trim());
    ...
    private DatabaseClient.GenericExecuteSpec bindNull(
    ...
    private Mono<Void> validateRelations(DeviceBO value) {
        Mono<Boolean> driver = databaseClient
                .sql("SELECT 1 FROM dc3_manager.dc3_driver"
        ...
        Mono<Boolean> profile = value.getProfileId() == null
                ? Mono.just(true)
                : databaseClient
                        .sql("SELECT 1 FROM dc3_manager.dc3_profile"
                        ...
                        .one()
                        .defaultIfEmpty(false);
        return Mono.zip(driver, profile)
        ...
        for (SortSpec spec : sort) {
            String column =
                    switch (spec.field()) {
                        ...
                        case "id" -> "d.id";
                        default ->
                            throw new IllegalArgumentException("device sort field is not allowed: " + spec.field());
                    };
    ...
    private Mono<DeviceBO> one(String predicate, Long tenantId, Long id) {
        ...
        return databaseClient
                .sql("SELECT " + COLUMNS + " FROM " + TABLE + predicate + " LIMIT 1")
                .bind("tenant_id", tenantId)
                .bind("id", id)
                .map(this::map)
                .one();
    ...
    private Flux<DeviceBO> listSimple(String suffix, Long tenantId, String key, Long value) {
        ...
        return databaseClient
                .sql("SELECT " + COLUMNS + " FROM " + TABLE + " WHERE tenant_id = :tenant_id AND deleted = 0" + suffix)
                .bind("tenant_id", tenantId)
                .bind(key, value)
                .map(this::map)
                .all();
    ...
    private DeviceBO prepare(DeviceBO source, Long id, boolean update) {
        if (source == null
                || source.getTenantId() == null
                || source.getDeviceName() == null
                || source.getDeviceCode() == null
                || source.getDriverId() == null) {
        ...
        source.setCreatorId(
                source.getCreatorId() == null
                        ? (source.getOperatorId() == null ? 0L : source.getOperatorId())
                        : source.getCreatorId());
        source.setCreatorName(
                source.getCreatorName() == null
                        ? (source.getOperatorName() == null ? "" : source.getOperatorName())
