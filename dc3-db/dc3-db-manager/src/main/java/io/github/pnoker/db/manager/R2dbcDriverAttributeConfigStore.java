...
public class R2dbcDriverAttributeConfigStore implements ReactiveDriverAttributeConfigStore {
    ...
    @Override
    public Mono<DriverAttributeConfigBO> get(Long tenantId, Long id) {
    ...
    @Override
    public Mono<DriverAttributeConfigBO> getByAttributeAndDevice(Long tenantId, Long attributeId, Long deviceId) {
    ...
    public Mono<DriverAttributeConfigBO> insert(DriverAttributeConfigBO value) {
        ...
        String sql = "INSERT INTO " + TABLE + " (" + COLUMNS + ", deleted) VALUES "
    ...
    public Mono<DriverAttributeConfigBO> update(DriverAttributeConfigBO value, int expectedVersion) {
        ...
        String sql = "UPDATE " + TABLE + " SET attribute_id=:attribute_id, config_value=:config_value"
                + ", device_id=:device_id, config_ext=" + dialect.jsonWriteExpression(":config_ext")
    ...
    public Mono<OffsetPage<DriverAttributeConfigBO>> list(DriverAttributeConfigFilter filter) {
        if (filter == null) {
            return Mono.error(new IllegalArgumentException("filter is required"));
        }
        StringBuilder where = new StringBuilder(" WHERE c.tenant_id=:tenant_id AND c.deleted=0");
        if (filter.attributeId() != null) {
            where.append(" AND c.attribute_id=:attribute_id");
        }
        if (filter.deviceId() != null) {
            where.append(" AND c.device_id=:device_id");
        }
        if (filter.enableFlag() != null) {
            where.append(" AND c.enable_flag=:enable_flag");
        }
    ...
    private DatabaseClient.GenericExecuteSpec bind(
        ...
        if (filter.deviceId() != null) {
            spec = spec.bind("device_id", filter.deviceId());
        }
        if (filter.enableFlag() != null) {
            spec = spec.bind("enable_flag", filter.enableFlag().getIndex());
        }
        if (filter.version() != null) {
            spec = spec.bind("version", filter.version());
        }
        return spec;
    }
    ...
    private String orderBy(List<SortSpec> sort) {
        ...
        if (clauses.stream().noneMatch(value -> value.startsWith("c.id"))) {
            clauses.add("c.id ASC");
        }
    ...
    private DriverAttributeConfigBO map(io.r2dbc.spi.Row row, io.r2dbc.spi.RowMetadata metadata) {
