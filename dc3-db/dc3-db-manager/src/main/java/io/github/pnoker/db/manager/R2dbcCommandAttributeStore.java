 ...
 */
package io.github.pnoker.db.manager;
...
/** Explicit SQL adapter for manager command attributes. */
@Repository
@ConditionalOnClass({DatabaseClient.class, TransactionalOperator.class})
@RequiredArgsConstructor
public class R2dbcCommandAttributeStore implements ReactiveCommandAttributeStore {
    private static final String TABLE = "dc3_manager.dc3_command_attribute";
    private static final String COLUMNS =
            "id, attribute_name, attribute_code, attribute_type_flag, default_value, driver_id, attribute_ext, enable_flag, tenant_id, remark, signature, version, creator_id, creator_name, create_time, operator_id, operator_name, operate_time";
    ...
    @Override
    public Mono<CommandAttributeBO> get(Long tenantId, Long id) {
    ...
    @Override
    public Mono<CommandAttributeBO> getByCodeAndDriver(Long tenantId, String attributeCode, Long driverId) {
        if (tenantId == null || driverId == null || attributeCode == null || attributeCode.isBlank())
            return Mono.empty();
        return databaseClient
                .sql(
    ...
    @Override
    public Flux<CommandAttributeBO> listByDriverId(Long tenantId, Long driverId) {
    ...
    @Override
    public Mono<CommandAttributeBO> insert(CommandAttributeBO value) {
        if (value == null
            ...
            return Mono.error(
                    new IllegalArgumentException("tenantId, driverId, attributeName and attributeCode are required"));
        }
        ...
        DatabaseClient.GenericExecuteSpec spec = databaseClient
                .sql(
                        "INSERT INTO " + TABLE + " (" + COLUMNS
                                + ", deleted) VALUES (:id,:attribute_name,:attribute_code,:attribute_type_flag,:default_value,:driver_id,"
                                + dialect.jsonWriteExpression(":attribute_ext")
                ...
                .bind("id", value.getId())
                .bind("attribute_name", value.getAttributeName().trim())
                .bind("attribute_code", value.getAttributeCode().trim())
                .bind("attribute_type_flag", index(value.getAttributeTypeFlag()))
                .bind("default_value", valueOrEmpty(value.getDefaultValue()))
                .bind("driver_id", value.getDriverId())
                .bind("attribute_ext", serialize(value.getAttributeExt()))
    ...
    @Override
    public Mono<CommandAttributeBO> update(CommandAttributeBO value, int expectedVersion) {
        DatabaseClient.GenericExecuteSpec spec = databaseClient
                .sql(
                        "UPDATE " + TABLE
                                + " SET attribute_name=:attribute_name, attribute_code=:attribute_code, attribute_type_flag=:attribute_type_flag, default_value=:default_value, driver_id=:driver_id, attribute_ext="
                                + dialect.jsonWriteExpression(":attribute_ext")
                ...
                .bind("attribute_name", valueOrEmpty(value.getAttributeName()))
                .bind("attribute_code", valueOrEmpty(value.getAttributeCode()))
                .bind("attribute_type_flag", index(value.getAttributeTypeFlag()))
                .bind("default_value", valueOrEmpty(value.getDefaultValue()))
                .bind("driver_id", value.getDriverId())
                .bind("attribute_ext", serialize(value.getAttributeExt()))
    ...
    private Mono<Boolean> deleteSql(Long tenantId, List<Long> ids, Long operatorId, String operatorName) {
        ...
        DatabaseClient.GenericExecuteSpec spec = databaseClient
                .sql("UPDATE " + TABLE
                        + " SET deleted=1, operator_id=:operator_id, operator_name=:operator_name, operate_time=:operate_time WHERE tenant_id=:tenant_id AND id IN ("
                        + placeholders + ") AND deleted=0")
                .bind("tenant_id", tenantId)
                .bind("operator_id", operatorId == null ? 0L : operatorId)
                .bind("operator_name", valueOrEmpty(operatorName))
                .bind("operate_time", LocalDateTime.now(ZoneOffset.UTC));
    ...
    @Override
    public Mono<OffsetPage<CommandAttributeBO>> list(CommandAttributeFilter filter) {
        ...
        DatabaseClient.GenericExecuteSpec count = databaseClient
                .sql("SELECT COUNT(*) AS total FROM " + TABLE + " a" + where)
                .bind("tenant_id", filter.tenantId());
        DatabaseClient.GenericExecuteSpec items = databaseClient
                .sql("SELECT " + COLUMNS + " FROM " + TABLE + " a" + where + orderBy(filter.sort())
                        + " LIMIT :limit OFFSET :offset")
                .bind("tenant_id", filter.tenantId())
                .bind("limit", filter.limit())
                .bind("offset", filter.offset());
        count = bind(count, filter);
        items = bind(items, filter);
        Mono<Long> total =
                count.map((row, metadata) -> row.get("total", Long.class)).one().defaultIfEmpty(0L);
    ...
    private DatabaseClient.GenericExecuteSpec bind(
            DatabaseClient.GenericExecuteSpec spec, CommandAttributeFilter filter) {
        if (present(filter.attributeName()))
            spec = spec.bind("attribute_name", "%" + filter.attributeName().trim() + "%");
        if (present(filter.attributeCode()))
            spec = spec.bind("attribute_code", filter.attributeCode().trim());
        if (filter.attributeTypeFlag() != null)
            spec = spec.bind("attribute_type_flag", filter.attributeTypeFlag().getIndex());
        ...
        if (sort != null)
            for (SortSpec spec : sort) {
                String column =
                        switch (spec.field()) {
                            case "id" -> "a.id";
                            case "attributeName" -> "a.attribute_name";
                            case "attributeCode" -> "a.attribute_code";
                            case "driverId" -> "a.driver_id";
                            case "createTime" -> "a.create_time";
                            case "operateTime" -> "a.operate_time";
                            case "version" -> "a.version";
                            default -> throw new IllegalArgumentException("unsupported command attribute sort field");
                        };
    ...
    private CommandAttributeBO map(io.r2dbc.spi.Row row, io.r2dbc.spi.RowMetadata metadata) {
        CommandAttributeBO value = new CommandAttributeBO();
        value.setId(row.get("id", Long.class));
        value.setAttributeName(row.get("attribute_name", String.class));
        value.setAttributeCode(row.get("attribute_code", String.class));
            ...
            try {
                ...
                ext.setContent(
                        json.getContent() == null
                                ? null
                                : JsonUtil.parseObject(json.getContent(), CommandAttributeExt.Content.class));
                value.setAttributeExt(ext);
            } catch (Exception exception) {
                throw new IllegalStateException("attribute_ext contains invalid JSON", exception);
            }
        return value;
    }
    ...
    private Byte index(AttributeTypeEnum value) {
        return value == null ? AttributeTypeEnum.STRING.getIndex() : value.getIndex();
    }
    ...
    private String serialize(CommandAttributeExt value) {
        try {
            ...
            throw new IllegalArgumentException("attribute_ext is not valid JSON", exception);
        }
    }
