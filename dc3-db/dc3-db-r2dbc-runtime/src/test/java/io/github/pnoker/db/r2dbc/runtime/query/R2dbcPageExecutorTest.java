...
class R2dbcPageExecutorTest {
    ...
    void bindsPageParametersAndBuildsCanonicalResult() {
        ...
        when(mapped.<String>map((java.util.function.BiFunction<io.r2dbc.spi.Row, io.r2dbc.spi.RowMetadata, String>)
                any())).thenReturn(itemRows);
        ...
        StepVerifier.create(new R2dbcPageExecutor(client, pageTransaction).execute(
                        new PageRequest(2, 2),
                        "SELECT count(*)",
                        "SELECT name",
                        Map.of("tenant", 7L),
    ...
    void rejectsAnInvalidParameterNameBeforeQueryExecution() {
        ...
        StepVerifier.create(new R2dbcPageExecutor(client, pageTransaction).execute(
                        PageRequest.firstPage(), "SELECT count(*)", "SELECT name",
                        Map.of("tenant;drop", 1L), (row, metadata) -> "x"))
    ...
    void rejectsReservedPaginationParametersFromRepositoryFilters() {
        ...
        StepVerifier.create(new R2dbcPageExecutor(client, pageTransaction).execute(
                        PageRequest.firstPage(), "COUNT", "ITEMS", Map.of("offset", 99L),
                        (row, metadata) -> "x"))
    ...
    void bindsNullParametersWithoutUsingMapCopyOf() {
        ...
        when(itemStatement.<String>map((java.util.function.BiFunction<io.r2dbc.spi.Row, io.r2dbc.spi.RowMetadata, String>) any()))
                .thenReturn(itemRows);
        ...
        StepVerifier.create(new R2dbcPageExecutor(client, pageTransaction).execute(
                        new PageRequest(0, 5), "COUNT", "ITEMS", parameters,
                        (row, metadata) -> "x"))
    ...
    void keepsCountAndItemsInsideTheConfiguredReactiveTransaction() {
        ...
        StepVerifier.create(new R2dbcPageExecutor(client, pageTransaction).execute(
                        new PageRequest(0, 5), "COUNT", "ITEMS", Map.of(), (row, metadata) -> "one"))
