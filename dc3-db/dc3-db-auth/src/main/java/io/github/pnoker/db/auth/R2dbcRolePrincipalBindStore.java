...
public class R2dbcRolePrincipalBindStore implements ReactiveRolePrincipalBindStore {
    ...
    private static final String COLUMNS =
            "id,tenant_id,role_id,principal_id,principal_type,remark,creator_id,creator_name,create_time,operator_id,operator_name,operate_time,deleted";
    ...
    public Mono<RolePrincipalBindDO> insert(RolePrincipalBindBO binding) {
        if (binding == null || !valid(binding.getTenantId(), binding.getRoleId()) || !valid(binding.getPrincipalId()))
        ...
        return transactionalOperator
                .transactional(databaseClient
                        .sql(sql)
                        .bind("id", id)
                        .bind("tenant_id", binding.getTenantId())
                        .bind("role_id", binding.getRoleId())
                        .bind("principal_id", binding.getPrincipalId())
                        .bind("principal_type", principalType(binding.getPrincipalType()))
    ...
    public Mono<Boolean> exists(Long tenantId, Long roleId, Long principalId, Long excludedId) {
        ...
        StringBuilder sql = new StringBuilder("SELECT 1 FROM ")
                .append(TABLE)
                .append(
                        " WHERE tenant_id=:tenant_id AND role_id=:role_id AND principal_id=:principal_id AND deleted=0");
    ...
    public Flux<Long> listPrincipalIds(Long tenantId, Long roleId, String principalType) {
        ...
        StringBuilder sql = new StringBuilder("SELECT principal_id FROM ")
                .append(TABLE)
        ...
        DatabaseClient.GenericExecuteSpec query =
                databaseClient.sql(sql.toString()).bind("tenant_id", tenantId).bind("role_id", roleId);
    ...
    private DatabaseClient.GenericExecuteSpec bind(
            DatabaseClient.GenericExecuteSpec query, RolePrincipalBindFilter filter) {
        ...
        if (filter.principalType() != null)
            query = query.bind("principal_type", filter.principalType().getValue());
        return query;
    }
                    ...
                    switch (spec.field()) {
                        ...
                        default ->
                            throw new IllegalArgumentException(
                                    "unsupported role principal bind sort field: " + spec.field());
                    };
    ...
    private String principalType(PrincipalTypeEnum value) {
        return value == null ? PrincipalTypeEnum.USER.getValue() : value.getValue();
    }
