...
public class R2dbcApiStore implements ReactiveApiStore {
    ...
    @Override
    public Mono<ApiDO> getById(Long id) {
    ...
    @Override
    public Mono<OffsetPage<ApiDO>> list(ApiFilter filter) {
    ...
    @Override
    public Mono<ApiDO> insert(ApiBO api) {
        ...
        String sql = "INSERT INTO " + TABLE + " (id,service_name,api_type_flag,api_name,api_code,api_group,api_ext,"
                ...
                + ",:enable_flag,:remark,:creator_id,:creator_name,:create_time,"
        ...
        DatabaseClient.GenericExecuteSpec query = databaseClient
                ...
                .bind("api_type_flag", index(api.getApiTypeFlag()))
                ...
                .bind("enable_flag", index(api.getEnableFlag()))
                .bind("remark", text(api.getRemark()))
                .bind("creator_id", value(api.getCreatorId()))
                .bind("creator_name", text(api.getCreatorName()))
                .bind("create_time", now)
                .bind("operator_id", value(api.getOperatorId()))
    ...
    @Override
    public Mono<ApiDO> update(ApiBO api) {
        if (api == null || !valid(api.getId())) return Mono.empty();
        String sql =
                "UPDATE " + TABLE + " SET service_name=:service_name,api_type_flag=:api_type_flag,api_name=:api_name,"
    ...
    @Override
    public Mono<Boolean> existsDuplicate(ApiBO api) {
    ...
    private String where(ApiFilter filter) {
        ...
        if (filter.apiName() != null)
            where.append(" AND ").append(dialect.caseInsensitiveLike("api_name", ":api_name"));
    ...
    private DatabaseClient.GenericExecuteSpec bind(DatabaseClient.GenericExecuteSpec spec, ApiFilter filter) {
        ...
        if (filter.apiTypeFlag() != null)
            spec = spec.bind("api_type_flag", filter.apiTypeFlag().getIndex());
            ...
            for (SortSpec spec : sort) {
                String column =
                        switch (spec.field()) {
                            ...
                            case "apiGroup" -> "api_group";
                            case "apiTypeFlag" -> "api_type_flag";
                            case "enableFlag" -> "enable_flag";
                            default ->
                                throw new IllegalArgumentException("unsupported API sort field: " + spec.field());
                        };
    ...
    private byte index(Object value) {
        ...
        if (value instanceof ApiTypeEnum flag) return flag.getIndex();
        return 0;
    }
