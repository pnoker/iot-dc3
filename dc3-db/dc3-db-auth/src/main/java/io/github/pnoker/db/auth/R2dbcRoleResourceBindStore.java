 ...
 */
package io.github.pnoker.db.auth;
...
public class R2dbcRoleResourceBindStore implements ReactiveRoleResourceBindStore {
    ...
    private static final String COLUMNS =
            "rr.id,rr.role_id,rr.resource_id,rr.remark,rr.creator_id,rr.creator_name,rr.create_time,rr.operator_id,rr.operator_name,rr.operate_time,rr.deleted";
    ...
    public Mono<RoleResourceBindDO> insert(RoleResourceBindBO binding) {
        if (binding == null || !valid(binding.getRoleId()) || !valid(binding.getResourceId()))
            return Mono.error(new IllegalArgumentException("binding identifiers are required"));
        long id = id();
        LocalDateTime now = now();
        String sql = "INSERT INTO " + TABLE
                + " (id,role_id,resource_id,remark,creator_id,creator_name,create_time,operator_id,operator_name,operate_time,deleted) VALUES (:id,:role_id,:resource_id,:remark,:creator_id,:creator_name,:create_time,:operator_id,:operator_name,:operate_time,0)";
        return transactionalOperator
                .transactional(databaseClient
                        .sql(sql)
                        .bind("id", id)
                        .bind("role_id", binding.getRoleId())
                        .bind("resource_id", binding.getResourceId())
                        .bind("remark", text(binding.getRemark()))
                        .bind("creator_id", value(binding.getCreatorId()))
                        .bind("creator_name", text(binding.getCreatorName()))
                        .bind("create_time", now)
                        .bind("operator_id", value(binding.getOperatorId()))
                        .bind("operator_name", text(binding.getOperatorName()))
                        .bind("operate_time", now)
                        .fetch()
                        .rowsUpdated())
                .then(getById(binding.getRoleId(), id));
    }

    private Mono<RoleResourceBindDO> getById(Long roleId, Long id) {
        return databaseClient
                .sql("SELECT " + COLUMNS + " FROM " + TABLE
                        + " rr WHERE rr.role_id=:role_id AND rr.id=:id AND rr.deleted=0 LIMIT 1")
                .bind("role_id", roleId)
                .bind("id", id)
                .map(this::map)
                .one();
    ...
    public Mono<Boolean> delete(Long tenantId, Long id, Long operatorId, String operatorName) {
        ...
        String sql = "UPDATE " + TABLE
                + " rr SET deleted=1,operator_id=:operator_id,operator_name=:operator_name,operate_time=:operate_time WHERE rr.id=:id AND rr.deleted=0 AND EXISTS (SELECT 1 FROM "
                + ROLE + " r WHERE r.id=rr.role_id AND r.tenant_id=:tenant_id AND r.deleted=0)";
        return transactionalOperator
                .transactional(databaseClient
                        .sql(sql)
    ...
    public Mono<Boolean> exists(Long tenantId, Long roleId, Long resourceId) {
        ...
        return databaseClient
                .sql(
                        "SELECT 1 FROM " + TABLE + " rr JOIN " + ROLE
                                + " r ON r.id=rr.role_id AND r.tenant_id=:tenant_id AND r.deleted=0 WHERE rr.role_id=:role_id AND rr.resource_id=:resource_id AND rr.deleted=0 LIMIT 1")
                .bind("tenant_id", tenantId)
                .bind("role_id", roleId)
    ...
    @Override
    public Flux<Long> listResourceIds(Long tenantId, Long roleId) {
        if (!valid(tenantId) || !valid(roleId)) return Flux.empty();
        return databaseClient
                .sql(
                        "SELECT rr.resource_id FROM " + TABLE + " rr JOIN " + ROLE
                                + " r ON r.id=rr.role_id AND r.tenant_id=:tenant_id AND r.deleted=0 WHERE rr.role_id=:role_id AND rr.deleted=0 ORDER BY rr.resource_id")
                .bind("tenant_id", tenantId)
                .bind("role_id", roleId)
                .map((row, m) -> row.get("resource_id", Long.class))
                .all();
    }

    @Override
    public Flux<Long> listResourceIdsByPrincipal(Long tenantId, Long principalId) {
        if (!valid(tenantId) || !valid(principalId)) return Flux.empty();
        return databaseClient
                .sql(
                        "SELECT DISTINCT rr.resource_id FROM " + PRINCIPAL_BIND + " rp JOIN " + ROLE
                                + " r ON r.id=rp.role_id AND r.tenant_id=rp.tenant_id AND r.deleted=0 AND r.enable_flag=0 JOIN "
                                + TABLE
                                + " rr ON rr.role_id=r.id AND rr.deleted=0 WHERE rp.tenant_id=:tenant_id AND rp.principal_id=:principal_id AND rp.deleted=0 ORDER BY rr.resource_id")
                .bind("tenant_id", tenantId)
    ...
    @Override
    public Flux<Long> listRoleIdsByResource(Long tenantId, Long resourceId) {
        if (!valid(tenantId) || !valid(resourceId)) return Flux.empty();
        return databaseClient
                .sql(
                        "SELECT rr.role_id FROM " + TABLE + " rr JOIN " + ROLE
                                + " r ON r.id=rr.role_id AND r.tenant_id=:tenant_id AND r.deleted=0 AND r.enable_flag=0 WHERE rr.resource_id=:resource_id AND rr.deleted=0 ORDER BY rr.role_id")
                .bind("tenant_id", tenantId)
                .bind("resource_id", resourceId)
                .map((row, m) -> row.get("role_id", Long.class))
                .all();
    }

    private DatabaseClient.GenericExecuteSpec bind(
            DatabaseClient.GenericExecuteSpec query, RoleResourceBindFilter filter) {
        query = query.bind("tenant_id", filter.tenantId());
        if (filter.roleId() != null) query = query.bind("role_id", filter.roleId());
        if (filter.resourceId() != null) query = query.bind("resource_id", filter.resourceId());
        return query;
    }

    private RoleResourceBindDO map(io.r2dbc.spi.Row row, io.r2dbc.spi.RowMetadata metadata) {
        RoleResourceBindDO value = new RoleResourceBindDO();
        value.setId(row.get("id", Long.class));
        value.setRoleId(row.get("role_id", Long.class));
    ...
    private String orderBy(List<SortSpec> sort) {
        if (sort == null || sort.isEmpty()) return "rr.role_id ASC,rr.resource_id ASC,rr.id ASC";
        List<String> clauses = new ArrayList<>();
        for (SortSpec spec : sort) {
            String column =
                    switch (spec.field()) {
                        case "id" -> "rr.id";
                        case "roleId" -> "rr.role_id";
                        case "resourceId" -> "rr.resource_id";
                        case "createTime" -> "rr.create_time";
                        case "operateTime" -> "rr.operate_time";
                        default ->
                            throw new IllegalArgumentException(
                                    "unsupported role resource bind sort field: " + spec.field());
                    };
            clauses.add(column + " " + spec.direction().name());
        }
        if (clauses.stream().noneMatch(v -> v.startsWith("rr.id "))) clauses.add("rr.id ASC");
