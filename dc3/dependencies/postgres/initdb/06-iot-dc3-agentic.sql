...
(
    id              BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, -- Primary key ID
    conversation_id TEXT     DEFAULT ''::TEXT          NOT NULL, -- Conversation ID
    title           TEXT     DEFAULT ''::TEXT          NOT NULL, -- Session title
    session_ext     JSONB     DEFAULT '{}'::JSONB        NOT NULL, -- Session extension metadata and chat preferences
    ...
    CONSTRAINT uq_session_tenant_user_conversation UNIQUE (tenant_id, user_id, conversation_id)
);
...
(
    ...
    content         JSONB     DEFAULT '{}'::JSONB        NOT NULL, -- Structured message content
    ...
    CONSTRAINT fk_message_session FOREIGN KEY (tenant_id, user_id, conversation_id)
        REFERENCES dc3_session (tenant_id, user_id, conversation_id)
);

CREATE UNIQUE INDEX idx_message_tenant_user_conversation_order_active
    ON dc3_message (tenant_id, user_id, conversation_id, message_index) WHERE deleted = 0;
CREATE INDEX idx_message_tenant_user_conversation_time
    ON dc3_message (tenant_id, user_id, conversation_id, create_time DESC, id DESC) WHERE deleted = 0;
...
(
    ...
    CONSTRAINT fk_attachment_session FOREIGN KEY (tenant_id, user_id, conversation_id)
        REFERENCES dc3_session (tenant_id, user_id, conversation_id)
);

CREATE INDEX idx_attachment_tenant_user_conversation
    ON dc3_attachment (tenant_id, user_id, conversation_id, create_time DESC, id DESC) WHERE deleted = 0;
...
(
    ...
    payload         JSONB     DEFAULT '{}'::JSONB        NOT NULL, -- Action payload, free-form structured data
    ...
    CONSTRAINT chk_action_status CHECK (status IN (0, 1, 2, 3, 4)),
    CONSTRAINT fk_action_session FOREIGN KEY (tenant_id, user_id, conversation_id)
        REFERENCES dc3_session (tenant_id, user_id, conversation_id)
);

CREATE UNIQUE INDEX idx_action_tenant_user_id_active
    ON dc3_action (tenant_id, user_id, action_id) WHERE deleted = 0;
