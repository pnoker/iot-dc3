...
(
    ...
    attribute_code      VARCHAR(191)     DEFAULT ''          NOT NULL, -- Attribute code
...
(
    ...
    profile_code       VARCHAR(191)     DEFAULT ''          NOT NULL, -- Profile code
...
(
    ...
    point_code      VARCHAR(191)     DEFAULT ''          NOT NULL, -- Point code
...
(
    ...
    device_code   VARCHAR(191)     DEFAULT ''          NOT NULL,   -- Device code
    driver_id     BIGINT   DEFAULT 0 NOT NULL,                   -- Driver ID
...
(
    ...
    command_code      VARCHAR(191)     DEFAULT ''       NOT NULL,  -- Command code
...
(
    ...
    param_code           VARCHAR(191)     DEFAULT ''       NOT NULL, -- Param code
...
(
    ...
    event_code       VARCHAR(191)     DEFAULT ''       NOT NULL,   -- Event code
...
ALTER TABLE dc3_driver ADD COLUMN idx_driver_tenant_code_active_unique_guard TINYINT GENERATED ALWAYS AS (CASE WHEN deleted = 0 AND driver_code <> '' THEN 1 ELSE NULL END) STORED;
CREATE UNIQUE INDEX idx_driver_tenant_code_active_unique ON dc3_driver (tenant_id, driver_code, idx_driver_tenant_code_active_unique_guard);
ALTER TABLE dc3_driver_attribute ADD COLUMN idx_driver_attribute_active_unique_guard TINYINT GENERATED ALWAYS AS (CASE WHEN deleted = 0 AND attribute_code <> '' THEN 1 ELSE NULL END) STORED;
CREATE UNIQUE INDEX idx_driver_attribute_active_unique ON dc3_driver_attribute (tenant_id, driver_id, attribute_code, idx_driver_attribute_active_unique_guard);
ALTER TABLE dc3_point_attribute ADD COLUMN idx_point_attribute_active_unique_guard TINYINT GENERATED ALWAYS AS (CASE WHEN deleted = 0 AND attribute_code <> '' THEN 1 ELSE NULL END) STORED;
CREATE UNIQUE INDEX idx_point_attribute_active_unique ON dc3_point_attribute (tenant_id, driver_id, attribute_code, idx_point_attribute_active_unique_guard);
ALTER TABLE dc3_command_attribute ADD COLUMN idx_command_attribute_active_unique_guard TINYINT GENERATED ALWAYS AS (CASE WHEN deleted = 0 AND attribute_code <> '' THEN 1 ELSE NULL END) STORED;
CREATE UNIQUE INDEX idx_command_attribute_active_unique ON dc3_command_attribute (tenant_id, driver_id, attribute_code, idx_command_attribute_active_unique_guard);
ALTER TABLE dc3_event_attribute ADD COLUMN idx_event_attribute_active_unique_guard TINYINT GENERATED ALWAYS AS (CASE WHEN deleted = 0 AND attribute_code <> '' THEN 1 ELSE NULL END) STORED;
CREATE UNIQUE INDEX idx_event_attribute_active_unique ON dc3_event_attribute (tenant_id, driver_id, attribute_code, idx_event_attribute_active_unique_guard);
ALTER TABLE dc3_profile ADD COLUMN idx_profile_tenant_code_active_unique_guard TINYINT GENERATED ALWAYS AS (CASE WHEN deleted = 0 AND profile_code <> '' THEN 1 ELSE NULL END) STORED;
CREATE UNIQUE INDEX idx_profile_tenant_code_active_unique ON dc3_profile (tenant_id, profile_code, idx_profile_tenant_code_active_unique_guard);
ALTER TABLE dc3_point ADD COLUMN idx_point_profile_code_active_unique_guard TINYINT GENERATED ALWAYS AS (CASE WHEN deleted = 0 AND point_code <> '' THEN 1 ELSE NULL END) STORED;
CREATE UNIQUE INDEX idx_point_profile_code_active_unique ON dc3_point (tenant_id, profile_id, point_code, idx_point_profile_code_active_unique_guard);
ALTER TABLE dc3_device ADD COLUMN idx_device_tenant_code_active_unique_guard TINYINT GENERATED ALWAYS AS (CASE WHEN deleted = 0 AND device_code <> '' THEN 1 ELSE NULL END) STORED;
CREATE UNIQUE INDEX idx_device_tenant_code_active_unique ON dc3_device (tenant_id, device_code, idx_device_tenant_code_active_unique_guard);
ALTER TABLE dc3_driver_attribute_config ADD COLUMN idx_driver_attribute_config_active_unique_guard TINYINT GENERATED ALWAYS AS (CASE WHEN deleted = 0 THEN 1 ELSE NULL END) STORED;
CREATE UNIQUE INDEX idx_driver_attribute_config_active_unique ON dc3_driver_attribute_config (tenant_id, device_id, attribute_id, idx_driver_attribute_config_active_unique_guard);
ALTER TABLE dc3_point_attribute_config ADD COLUMN idx_point_attribute_config_active_unique_guard TINYINT GENERATED ALWAYS AS (CASE WHEN deleted = 0 THEN 1 ELSE NULL END) STORED;
CREATE UNIQUE INDEX idx_point_attribute_config_active_unique ON dc3_point_attribute_config (tenant_id, device_id, point_id, attribute_id, idx_point_attribute_config_active_unique_guard);
ALTER TABLE dc3_command_attribute_config ADD COLUMN idx_command_attribute_config_active_unique_guard TINYINT GENERATED ALWAYS AS (CASE WHEN deleted = 0 THEN 1 ELSE NULL END) STORED;
CREATE UNIQUE INDEX idx_command_attribute_config_active_unique ON dc3_command_attribute_config (tenant_id, device_id, command_id, attribute_id, idx_command_attribute_config_active_unique_guard);
ALTER TABLE dc3_event_attribute_config ADD COLUMN idx_event_attribute_config_active_unique_guard TINYINT GENERATED ALWAYS AS (CASE WHEN deleted = 0 THEN 1 ELSE NULL END) STORED;
CREATE UNIQUE INDEX idx_event_attribute_config_active_unique ON dc3_event_attribute_config (tenant_id, device_id, event_id, attribute_id, idx_event_attribute_config_active_unique_guard);
ALTER TABLE dc3_command ADD COLUMN idx_command_tenant_profile_code_active_unique_guard TINYINT GENERATED ALWAYS AS (CASE WHEN deleted = 0 AND command_code <> '' THEN 1 ELSE NULL END) STORED;
CREATE UNIQUE INDEX idx_command_tenant_profile_code_active_unique ON dc3_command (tenant_id, profile_id, command_code, idx_command_tenant_profile_code_active_unique_guard);
ALTER TABLE dc3_command_param ADD COLUMN idx_command_param_active_unique_guard TINYINT GENERATED ALWAYS AS (CASE WHEN deleted = 0 AND param_code <> '' THEN 1 ELSE NULL END) STORED;
CREATE UNIQUE INDEX idx_command_param_active_unique ON dc3_command_param (tenant_id, command_id, param_code, idx_command_param_active_unique_guard);
ALTER TABLE dc3_event ADD COLUMN idx_event_tenant_profile_code_active_unique_guard TINYINT GENERATED ALWAYS AS (CASE WHEN deleted = 0 AND event_code <> '' THEN 1 ELSE NULL END) STORED;
CREATE UNIQUE INDEX idx_event_tenant_profile_code_active_unique ON dc3_event (tenant_id, profile_id, event_code, idx_event_tenant_profile_code_active_unique_guard);
ALTER TABLE dc3_event_param ADD COLUMN idx_event_param_active_unique_guard TINYINT GENERATED ALWAYS AS (CASE WHEN deleted = 0 AND param_code <> '' THEN 1 ELSE NULL END) STORED;
CREATE UNIQUE INDEX idx_event_param_active_unique ON dc3_event_param (tenant_id, event_id, param_code, idx_event_param_active_unique_guard);
