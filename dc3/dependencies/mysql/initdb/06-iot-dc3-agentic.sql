...
(
    ...
    conversation_id VARCHAR(191)     DEFAULT ''          NOT NULL, -- Conversation ID
...
(
    id              BIGINT PRIMARY KEY NOT NULL,                 -- Primary key ID
    action_id       VARCHAR(191)     DEFAULT ''          NOT NULL, -- Stable action identifier exposed to clients
    conversation_id TEXT     DEFAULT ('')          NOT NULL, -- Conversation ID
...
(
    id            BIGINT PRIMARY KEY NOT NULL,                   -- Primary key ID
    name          VARCHAR(191)     DEFAULT ''          NOT NULL,   -- Provider name, unique
...
(
    id            BIGINT PRIMARY KEY             NOT NULL,             -- Primary key ID
    model         VARCHAR(191)             DEFAULT ''          NOT NULL, -- Model identifier
...
ALTER TABLE dc3_session ADD COLUMN idx_session_conversation_id_active_guard TINYINT GENERATED ALWAYS AS (CASE WHEN deleted = 0 THEN 1 ELSE NULL END) STORED;
CREATE UNIQUE INDEX idx_session_conversation_id_active ON dc3_session (conversation_id, idx_session_conversation_id_active_guard);
ALTER TABLE dc3_action ADD COLUMN idx_action_id_active_guard TINYINT GENERATED ALWAYS AS (CASE WHEN deleted = 0 THEN 1 ELSE NULL END) STORED;
CREATE UNIQUE INDEX idx_action_id_active ON dc3_action (action_id, idx_action_id_active_guard);
ALTER TABLE dc3_model_provider ADD COLUMN idx_model_provider_tenant_name_active_guard TINYINT GENERATED ALWAYS AS (CASE WHEN deleted = 0 THEN 1 ELSE NULL END) STORED;
CREATE UNIQUE INDEX idx_model_provider_tenant_name_active ON dc3_model_provider (tenant_id, name, idx_model_provider_tenant_name_active_guard);
ALTER TABLE dc3_model_config ADD COLUMN idx_model_config_tenant_model_active_guard TINYINT GENERATED ALWAYS AS (CASE WHEN deleted = 0 THEN 1 ELSE NULL END) STORED;
CREATE UNIQUE INDEX idx_model_config_tenant_model_active ON dc3_model_config (tenant_id, model, idx_model_config_tenant_model_active_guard);
