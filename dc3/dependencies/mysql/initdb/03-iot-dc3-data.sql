...
(
    ...
    notify_code       VARCHAR(191)     DEFAULT ''          NOT NULL, -- Notification code
...
(
    ...
    channel_code      VARCHAR(191)     DEFAULT ''          NOT NULL, -- Notification channel code
...
(
    ...
    message_code  VARCHAR(191)     DEFAULT ''          NOT NULL,   -- Message code
...
(
    ...
    rule_code              VARCHAR(191)     DEFAULT ''          NOT NULL, -- Rule code
...
ALTER TABLE dc3_notify ADD COLUMN idx_notify_tenant_code_active_unique_guard TINYINT GENERATED ALWAYS AS (CASE WHEN deleted = 0 AND notify_code <> '' THEN 1 ELSE NULL END) STORED;
CREATE UNIQUE INDEX idx_notify_tenant_code_active_unique ON dc3_notify (tenant_id, notify_code, idx_notify_tenant_code_active_unique_guard);
ALTER TABLE dc3_notify_channel ADD COLUMN idx_notify_channel_tenant_code_active_unique_guard TINYINT GENERATED ALWAYS AS (CASE WHEN deleted = 0 AND channel_code <> '' THEN 1 ELSE NULL END) STORED;
CREATE UNIQUE INDEX idx_notify_channel_tenant_code_active_unique ON dc3_notify_channel (tenant_id, channel_code, idx_notify_channel_tenant_code_active_unique_guard);
ALTER TABLE dc3_notify_channel_bind ADD COLUMN idx_notify_channel_bind_active_unique_guard TINYINT GENERATED ALWAYS AS (CASE WHEN deleted = 0 THEN 1 ELSE NULL END) STORED;
CREATE UNIQUE INDEX idx_notify_channel_bind_active_unique ON dc3_notify_channel_bind (tenant_id, notify_id, channel_id, idx_notify_channel_bind_active_unique_guard);
ALTER TABLE dc3_message ADD COLUMN idx_message_tenant_code_active_unique_guard TINYINT GENERATED ALWAYS AS (CASE WHEN deleted = 0 AND message_code <> '' THEN 1 ELSE NULL END) STORED;
CREATE UNIQUE INDEX idx_message_tenant_code_active_unique ON dc3_message (tenant_id, message_code, idx_message_tenant_code_active_unique_guard);
ALTER TABLE dc3_rule ADD COLUMN idx_rule_tenant_code_active_unique_guard TINYINT GENERATED ALWAYS AS (CASE WHEN deleted = 0 AND rule_code <> '' THEN 1 ELSE NULL END) STORED;
CREATE UNIQUE INDEX idx_rule_tenant_code_active_unique ON dc3_rule (tenant_id, rule_code, idx_rule_tenant_code_active_unique_guard);
