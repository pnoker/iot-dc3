...
import {localizeAlertRows, localizeStreamRows, localizeTopology} from '../locale';
...
const withConfirmationOverrides = (rows: MockAlertRow[]): MockAlertRow[] =>
  rows.map((row) => ({...row, confirmFlag: confirmationOverrides.get(row.id) ?? row.confirmFlag}));

const rangeStart = (body: Record<string, any>, now = new Date()): number | null => {
  ...
  const hours = rangeKey === '24h' ? 24 : rangeKey === '7d' ? 24 * 7 : rangeKey === '30d' ? 24 * 30 : Number(body.rangeHours);
  return Number.isFinite(hours) && hours > 0 ? now.getTime() - hours * 3_600_000 : null;
};
...
const filteredAlerts = (body: Record<string, any>): MockAlertRow[] => {
  ...
  return withConfirmationOverrides(alertRecords()).filter((row) => {
    ...
    if (body.eventTypeFlag !== null && body.eventTypeFlag !== undefined && body.eventTypeFlag !== '') {
      if (row.eventTypeFlag !== Number(body.eventTypeFlag)) return false;
    }
    if (body.confirmFlag !== null && body.confirmFlag !== undefined && body.confirmFlag !== '') {
      const expected = Number(body.confirmFlag) === 1 ? 'CONFIRMED' : 'UNCONFIRMED';
...
export function registerDashboardHandlers(): void {
  ...
  on('post', 'api/v3/data/dashboard/alert/page', (ctx) => {
    ...
    return responseOf(ctx.config, okPage(rows.slice(start, start + size), rows.length));
  });
  ...
  on('post', 'api/v3/data/dashboard/alert/bulk_confirm', (ctx) => {
    const value: MockAlertRow['confirmFlag'] = ctx.body.confirm ? 'CONFIRMED' : 'UNCONFIRMED';
  ...
  on('get', 'api/v3/data/dashboard/alert/top_sources', (ctx) =>
    responseOf(ctx.config, ok(alertTopSourceRows(Number(ctx.params.limit) || 10))),
  );
  ...
  on('get', 'api/v3/data/dashboard/alert/storm_sources', (ctx) =>
    responseOf(
      ctx.config,
      ok(alertStormRows(Number(ctx.params.min_count) || 10, Number(ctx.params.limit) || 10)),
    ),
  );
  on('get', 'api/v3/data/dashboard/alert/flapping', (ctx) =>
    responseOf(
      ctx.config,
      ok(alertFlappingRows(Number(ctx.params.min_count) || 5, Number(ctx.params.limit) || 20)),
    ),
  );
  on('get', 'api/v3/data/dashboard/alert/correlation', (ctx) =>
    responseOf(
      ctx.config,
      ok(alertCorrelationRows(Number(ctx.params.hours) || 24, Number(ctx.params.limit) || 15)),
    ),
  );
  on('get', 'api/v3/data/dashboard/alert/peer_deviation', (ctx) =>
    responseOf(ctx.config, ok(alertPeerDeviationRows(Number(ctx.params.days) || 7))),
  );
  ...
  on('get', 'api/v3/data/dashboard/alert/change_impact', (ctx) =>
    responseOf(
      ctx.config,
      ok(alertChangeImpactRows(Number(ctx.params.days) || 30, Number(ctx.params.limit) || 30)),
    ),
  );
  ...
  on('get', 'api/v3/data/dashboard/coverage/gap', (ctx) =>
    responseOf(ctx.config, ok(coverageGapReport(Number(ctx.params.limit) || 100))),
  );
  ...
  on('get', 'api/v3/data/dashboard/silent/sources', (ctx) =>
    responseOf(
      ...
      ok(
        silentSources()
          .filter((row) => row.silentSeconds >= (Number(ctx.params.silent_minutes) || 15) * 60)
  ...
  on('get', 'api/v3/data/dashboard/stats/activity', (ctx) =>
    responseOf(ctx.config, ok(statsActivity(String(ctx.params.range_key ?? '7d')))),
  );
}
